...
function parseAimsTokensIntoEventsV3(tokens, dayNum, month, year, base) {
 ...
 // apresentação, o código antigo tratava a coluna inteira como uma única
 // jornada: pegava o primeiro horário antes de qualquer "LA" (mesmo vindo do
 // resíduo "(...)" do dia anterior) como dutyReport, e agrupava todos os "LA"
 // da coluna como pernas de um só dia — contaminando a apresentação (#510,
 // ex. LA3730 herdando 08:29 da DR anterior) e fundindo jornadas distintas
 // (ex. LA4712 + LA3246 no mesmo "dia"). O sinal para uma NOVA jornada é
 // estrutural no próprio formato: um "LA" com apresentação própria impressa
 // tem dois horários antes do primeiro aeroporto (apresentação + partida); um
 // "LA" que é apenas continuação da jornada anterior tem só um (a partida).
 const laIndexes = [];
 for (let i = 0; i < upperTokens.length; i++) {
 ...
 // se nenhum "LA" da coluna tiver apresentação própria (ex.: perna isolada
 // que só continua uma jornada iniciada ontem), ainda assim segmentar a
 // partir do primeiro "LA" real evita perder o dia inteiro.
 ...
 for (let s = 0; s < segmentStarts.length; s++) {
  ...
  const beforeFirst = segNormalized.slice(0, firstLaIndex).filter(isTimeToken).map(normalizeTimeToken);
