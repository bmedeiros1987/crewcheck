    ...
    String text(Element font, String slot) {
        ...
        for (Element p : children(template)) {
            String value = switch(p.getAttribute("expression")) {
                case "[DAY_OF_WEEK_S]" -> "SEX";
                case "[DAY_Z]" -> "25";
                case "[MONTH_S]" -> "SET";
                case "[COMPLICATION.TITLE]" -> EXAMPLES.get(slot)[0];
                case "[COMPLICATION.TEXT]" -> EXAMPLES.get(slot)[1];
                default -> throw new IllegalArgumentException("Unsupported preview expression: " + p.getAttribute("expression"));
            };
    ...
    void paint(Element e, Graphics2D parent, String slot) {
        ...
        try {
            ...
            switch(e.getTagName()) {
                ...
                case "PartImage" -> {
                    if (!child(e, "Image").getAttribute("resource").equals("crewcheck_logo_neon"))
                        throw new IllegalArgumentException("Unexpected image; requalify preview");
                    g.drawImage(logo, 0, 0, (int)w, (int)h, null);
                }
                case "TimeText" -> {
