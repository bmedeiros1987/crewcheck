...
Owner: **CrewCheck Peripherals**. Build-only tooling; no code in the WFF APK and no Mobile-owned files.
...
CI generates before/after canonical preparation and requires byte-identical PNGs within the same environment. It builds the Watch Face debug APK, verifies the three icons and metadata are packaged and rejects executable code. The existing pinned official WFF v1 schema and consolidated signed-build gates remain required, unchanged.
