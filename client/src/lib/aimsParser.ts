...
export function parseAimsRoster(fullText: string, visualRows?: AimsVisualRow[]): CrewRoster {
  ...
  // Segurança premium: nunca aceitar uma leitura visual parcial só porque ela
  // encontrou vários marcadores de data. Em PDFs AIMS/SL escala com muitas colunas,
  // especialmente no iOS/WebView, alguns dias aparecem como marcadores vazios.
  // Mesclamos as fontes e escolhemos, por data, o bloco com mais pernas/horários.
  if (visualParsed?.days?.length) {
    return mergeAimsParsedRosters(visualParsed, legacyParsed);
  }
  return legacyParsed;
}
...
function parseAimsRosterFromVisualRows(fullText: string, visualRows: AimsVisualRow[]): CrewRoster | null {
  ...
  for (const [page, markers] of byPage.entries()) {
    ...
    sorted.forEach((entry, index) => {
      ...
      // AIMS pode trazer transbordos publicados do mês anterior/posterior.
      // Mantemos esses dias quando há programação, pois podem completar pernoites e voos já publicados.
      ...
      const current = byDate.get(dateFormatted);
      if (!current || aimsDayQualityScore(candidate) > aimsDayQualityScore(current)) {
        byDate.set(dateFormatted, candidate);
      }
    });
  }

  days.push(...Array.from(byDate.values()));
