...
export function detectAndMarkLayovers(roster: CrewRoster): CrewRoster {
  ...
  // v11.1.80: não inserir cartões de pernoite no mesmo dia da chegada.
  // Eles poluíam a timeline com LAYOVER entre pernas e até com repouso < 12h.
  // Mantemos apenas dias inteiros de inativo entre uma chegada fora de base e
  // a próxima programação saindo da mesma estação.
  ...
  // v10.8.77: não cria mais calendário completo artificial.
  // Mantém somente dias realmente lidos/gerados por continuidade operacional
  // (incluindo dias do próximo mês já publicados na escala) e remove vazios.
  const officialOrOperationalDays = output.filter(day => {
    if (isEmptyDay(day)) return false;
    if (/Dia sem programação publicada no PDF/i.test(day.rawText || '')) return false;
  ...
  return {
    ...roster,
    days: officialOrOperationalDays.sort(compareRosterDays),
  };
}



function addArrivalLayoverCards(output: RosterDay[], officialDays: RosterDay[], base: string): void {
  for (let i = 0; i < officialDays.length; i += 1) {
    const day = officialDays[i];
    if (!isFlightDay(day)) continue;
    const lastLeg = day.legs[day.legs.length - 1];
    const station = cleanAirport(lastLeg?.destination);
    const home = cleanAirport(base || day.base);
    if (!station || station === home) continue;
    const currentDate = parseDate(day.date);
    if (!currentDate) continue;
    const arrivalTime = cleanTime(lastLeg.arrivalTime || day.dutyDebrief || '23:59');
    const startDate = new Date(startOfDay(currentDate));
    if (lastLeg?.isNextDay || day.isNextDay) startDate.setDate(startDate.getDate() + 1);
    const nextFlight = findNextFlightFromStation(officialDays, i + 1, station);
    const nextPresentation = nextFlight?.dutyReport || nextFlight?.legs?.[0]?.departureTime || null;
    const rawEnd = nextPresentation ? cleanTime(nextPresentation) : '23:59';
    const hours = nextPresentation ? timeDiffHours(arrivalTime, rawEnd) + ((parseDate(nextFlight!.date)!.getTime() - startOfDay(startDate).getTime()) / DAY_MS) * 24 : 24;
    const key = formatDate(startDate);
    const exists = output.some((candidate) => candidate.date === key && candidate.type === 'LAYOVER' && String(candidate.hotel || '').toUpperCase() === station && /PERNOITE/.test(candidate.rawText || ''));
    if (exists) continue;
    output.push({
      date: key,
      ...
      type: 'LAYOVER',
      pairingCode: `PERNOITE ${station}`,
      dutyReport: arrivalTime,
      dutyDebrief: rawEnd,
      legs: [],
      dutyHours: Math.round(Math.max(0, hours) * 100) / 100,
      flyingHours: 0,
      isNextDay: false,
      hotel: station,
      base: home,
      rawText: `PERNOITE ${station}: corte dos motores ${arrivalTime}; próxima apresentação ${nextPresentation || 'a confirmar'}; almoço e jantar previstos`,
    });
  }
}

function cleanAirport(value?: string | null): string {
...
function addOvernightContinuationMarkers(output: RosterDay[], officialDays: RosterDay[], base: string, rosterMonth: number, rosterYear: number): void {
  for (const day of officialDays) {
    ...
    // v10.8.77: se a programação cruza para o próximo mês, mantém a continuação real.
    // Não bloqueamos mais por mês de referência, para preservar escalas que já trazem dias do mês seguinte.
