...
export async function getLatestSavedRoster(): Promise<{ summary: SavedRosterSummary; roster: CrewRoster; compliance: ComplianceResult; gym: GymRecommendation[] } | null> {
  const local = getLatestLocalRoster();
  try {
    const payload = await jsonFetch<{ ok: boolean; roster: SavedRosterSummary; data: { roster: CrewRoster; compliance: ComplianceResult; gym: GymRecommendation[] } }>('/api/rosters/latest');
    if (payload?.ok && payload.roster && payload.data?.roster) {
      return {
        summary: payload.roster,
        roster: payload.data.roster,
        compliance: payload.data.compliance,
        gym: payload.data.gym || [],
      };
    }
  } catch {
    // Usa fallback local quando a base online estiver indisponível.
  }
  if (!local) return null;
  return {
    summary: localSummary(local),
    roster: local.roster,
...
function readLocalHistory(): LocalHistoryItem[] {
  try {
    return JSON.parse(localStorage.getItem(localHistoryKey()) || '[]') as LocalHistoryItem[];
  } catch {
    return [];
  }
}
...
function deleteLocalRoster(id: string): void {
  try {
    const next = readLocalHistory().filter((item) => item.id !== id && item.checksum !== id);
...
function localSummary(item: LocalHistoryItem): SavedRosterSummary {
  return {
    id: item.id,
...
function getLatestLocalRoster(): LocalHistoryItem | null {
  const history = readLocalHistory();
  if (!history.length) return null;
  return [...history].sort((a, b) => new Date(b.createdAt || 0).getTime() - new Date(a.createdAt || 0).getTime())[0] || null;
}
...
function getLocalRosterSummaries(limit: number): SavedRosterSummary[] {
  ...
  return unique.map(localSummary);
}
