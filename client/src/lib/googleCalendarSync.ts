...
const GOOGLE_CLIENT_ID_FALLBACK = '811142401548-da2kf1qmqn8629ik689oaf3acsa8c3ai.apps.googleusercontent.com';
const GOOGLE_SCOPES = [
...
function rosterPeriodBounds(roster: CrewRoster): { timeMin: string; timeMax: string } {
  const first = roster.days[0]?.date || `${roster.year}-${String(roster.month).padStart(2, '0')}-01`;
  const last = roster.days[roster.days.length - 1]?.date || first;
  const min = new Date(`${first}T00:00:00-03:00`);
  min.setDate(min.getDate() - 3);
  const max = new Date(`${last}T23:59:59-03:00`);
