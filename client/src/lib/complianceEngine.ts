...
const COMPLIANCE_ENGINE_VERSION = '11.0.91';
...
function hasMadrugadaDuty(day: RosterDay): boolean {
  ...
  const legs = day.legs || [];
  if (legs.length > 0) {
    // Para reduzir falso positivo: madrugada regulatória só é marcada por voo que
    // efetivamente toca a janela 00:00–06:00. Apresentação cedo sem voo nessa faixa,
    // sobreaviso sem acionamento, reserva sem voo e pernoite não contam.
...
export function analyzeCompliance(roster: CrewRoster, roleSelection: CrewRoleSelection = 'auto'): ComplianceResult {
  ...
  sortedDays.forEach((day, index) => {
    ...
    // v11.0.91: tempo em solo entre etapas deixou de gerar alerta automático.
  ...
  // v11.0.91: jornada semanal é mantida como métrica interna. Não vira alerta
  ...
  // v11.0.91: limite mensal de jornada/trabalho fica como métrica, não como
  ...
  if (maxNightOpsWindow > limits.maxNightOps168h) {
    pushAlert(alerts, {
      ...
      details: 'O sistema conta apenas voo que toca 00:00–06:00; folga, sobreaviso sem acionamento, reserva sem voo e voo sem madrugada quebram sequência. Mantido como atenção para evitar falso positivo.',
  ...
  if (metrics.maxConsecutiveNights > limits.maxConsecutiveNightOps) {
    pushAlert(alerts, {
      ...
      details: 'Folga, pernoite/inativo, sobreaviso sem acionamento, reserva sem voo e voo sem madrugada quebram a sequência.',
      legalReference: actRules.nightOps.legalReference,
      confidence: 'media',
      classification: 'atencao',
    });
