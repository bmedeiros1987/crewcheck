...
const COMPLIANCE_ENGINE_VERSION = '11.0.95';
...
function getFlightHours(day: RosterDay): number {
  ...
  if (legs.length) {
    ...
    // v11.0.95: o total de flyingHours vindo do PDF convertido pode herdar
...
function isSameOperationalContinuation(prev: RosterDay, next: RosterDay): boolean {
  ...
  if (rest <= 3 && nextStartsEarly && (prev.legs?.length || 0) > 0 && (next.legs?.length || 0) > 0) return true;
...
export function analyzeCompliance(roster: CrewRoster, roleSelection: CrewRoleSelection = 'auto'): ComplianceResult {
  ...
  sortedDays.forEach((day, index) => {
    ...
    // v11.0.95: tempo em solo entre etapas deixou de gerar alerta automático.
    ...
    if (day.type === 'VOO') {
      ...
      // v11.0.95: se o parser precisou estimar porque a janela foi contaminada
      // por pernoite/continuação visual, não criar card de irregularidade diária.
      // O usuário pode conferir manualmente na calculadora de Regulamentação.
      if (breakdown.mode !== 'estimada_parser' && dutyHours > limits.absoluteDailyDutyLimit) {
        pushAlert(alerts, {
          severity: 'warning',
      ...
      } else if (breakdown.mode !== 'estimada_parser' && dutyHours > limits.maxDailyDutyHoursComposite) {
        pushAlert(alerts, {
          severity: 'warning',
  ...
  // v11.0.95: jornada semanal é mantida como métrica interna. Não vira alerta
  ...
  // v11.0.95: limite mensal de jornada/trabalho fica como métrica, não como
