 ...
 return (
  <div className="min-h-screen bg-[#061425] px-4 py-6 pb-32 text-white sm:px-6 lg:px-8">
   <div className="mx-auto max-w-6xl">
    <HeaderBack title="Assinatura" subtitle="Premium Unlimited · planos · cobrança" onBack={onBack} />
    <div className="mt-6">
     <BillingSettingsCard
      status={status}
      busy={busy}
 ...
 useEffect(() => {
  try {
   ...
   for (const day of roster.days.slice(0, 45)) {
    ...
    const ok = scheduleCrewCheckNotification('CrewCheck · Check-in da escala', `Lembrete premium: preencher o formulário de Check-in da escala para ${reserve ? 'reserva/ASB' : 'Voo de Extra/PS'} às ${start}.`, reminder);
   ...
   <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
    <div className="flex items-start gap-3">
     ...
     <div>
      ...
      <h3 className="mt-0.5 text-base font-black leading-tight sm:text-xl">Teste Saída Inteligente, Voo de Extra, Radar, Diárias e Salário sem cartão de crédito.</h3>
...
function CrewCheckMobileMenu({ active, onNavigate }: { active: CrewCheckBottomActive; onNavigate: (view: string) => void }) {
 ...
 const items: { id: 'home' | 'roster' | 'vivo' | 'flightboard' | 'settings'; label: string; fullLabel: string; icon: LucideIcon; view: string }[] = [
  ...
  { id: 'vivo', label: 'Extra', fullLabel: 'Voo de Extra na Saída Inteligente', icon: Plane, view: 'departureVivo' },
...
function CrewCheckFloatingSpeedMenu({ active, onNavigate, canAccessC32FAcademy, hasRoster }: { active: 'home' | 'roster' | 'iflight' | 'departure' | 'settings' | 'more'; onNavigate: (view: string) => void; canAccessC32FAcademy: boolean; hasRoster: boolean }) {
 ...
 const quickItems: Array<{ label: string; hint: string; icon: LucideIcon; view: string; premium?: boolean }> = [
  ...
  { label: 'Voo de Extra', hint: 'grátis quando usar dados locais', icon: Radar, view: 'departureVivo' },
...
function buildExtraFlightOptionsForPresentation(originAirport: string, targetAirport: string, targetIso?: string, selectedDate?: string): ExtraFlightOptionEstimate[] {
 ...
 if (directBlock) {
  ...
  options.push({
   ...
   legs: [buildEstimatedLeg('Trecho direto', from, to, depart, directBlock, 'LATAM/GOL/Azul', 'Validar disponibilidade real e política de Voo de Extra.')],
  });
 }
 for (const hub of CREWCHECK_EXTRA_FLIGHT_HUBS) {
  ...
  options.push({
   kind: 'connection',
   title: 'Voo de Extra com conexão',
   route: `${from} → ${hub} → ${to}`,
...
function SmartDepartureScreen({ nextEvent, bundle, userBase, hasRoster, canAccessAdmin = false, onBack, onOpenRoster }: { nextEvent?: DashboardNextEvent; bundle?: SessionRosterBundle | null; userBase?: string; hasRoster: boolean; canAccessAdmin?: boolean; onBack: () => void; onOpenRoster: () => void }) {
 ...
 const scheduleCurrentDepartureAlert = useCallback(() => {
  ...
  if (!targetIso) {
   toast.info('Carregue uma escala ou selecione uma opção de Voo de Extra para agendar o alerta.');
   return;
  }
  ...
  const targetLabel = selectedExtraOption ? `Voo de Extra ${selectedExtraOption.route}` : (nextEvent?.title || 'sua próxima programação');
 ...
 const runGeolocationCalculation = useCallback(async () => {
  if (!canCalculate) {
   setError('Importe uma escala, abra uma escala salva ou selecione uma opção de Voo de Extra para calcular a hora de sair.');
   return;
  }
      ...
      <div className="p-6 md:p-8">
       ...
       <div ref={vivoSectionRef} className="mt-5 rounded-[1.7rem] border border-fuchsia-200/20 bg-fuchsia-300/10 p-4 shadow-xl shadow-black/10">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
         <div>
          <p className="text-[10px] font-black uppercase tracking-[0.22em] text-fuchsia-100/70">Voo de Extra</p>
 ...
 return cleanAirportMonitorRows(rows)
  ...
  .map((row) => {
   ...
   return {
    ...
    title: 'Voo de Extra direto · monitor/API',
    route: `${origin} → ${target}`,
    ...
    advisory: 'Confirme aceitação do Voo de Extra, valor da passagem, assento, check-in, bagagem e eventual troca de terminal.',
    legs: [{
  ...
  <div role="button" tabIndex={0} onClick={onToggle} onKeyDown={(event) => { if (event.key === 'Enter' || event.key === ' ') { event.preventDefault(); onToggle(); } }} className={`group cursor-pointer rounded-[2rem] border bg-white p-4 text-slate-950 shadow-xl shadow-slate-950/5 transition hover:-translate-y-0.5 hover:shadow-2xl dark:border-white/10 dark:bg-white/[0.07] dark:text-white ${selected ? 'ring-2 ring-cyan-300/70 border-cyan-300/40' : 'border-slate-200/80'}`}>
   <div className="flex items-start justify-between gap-3">
    <div className="min-w-0">
     ...
     <h4 className="mt-2 truncate text-3xl font-black leading-none tracking-tight text-slate-950 dark:text-white">{primaryFlights || item.title.replace(/^Voo de Extra\s*/i, '').replace(/·.*$/, '').trim() || 'Voo a validar'}</h4>
  ...
  <div className="min-h-screen bg-[#08111f] px-5 py-8 text-white">
   <div className="mx-auto max-w-3xl">
    ...
    <div className="mt-6 rounded-3xl border border-white/10 bg-white/[0.05] p-5">
     <textarea value={note} onChange={(event) => setNote(event.target.value)} placeholder="Ex.: observar pernoite, descanso, Voo de Extra, treinamento, pendência pessoal..." className="min-h-[18rem] w-full rounded-2xl border border-white/10 bg-[#07111f] p-4 text-sm leading-6 text-white outline-none focus:border-cyan-300" />
    ...
    <SettingsCategoryCard eyebrow="Perfil local" title="Quem aparece no CrewCheck" description="Esses dados ficam salvos apenas neste dispositivo e ajudam a identificar chefia, base e exportações.">
     ...
     <div className="mt-4 rounded-2xl border border-white/10 bg-white/[0.045] p-4">
      <button type="button" onClick={() => setProfileForm({ ...profileForm, hasVirtualBase: !profileForm.hasVirtualBase })} className={`flex w-full items-center justify-between gap-3 rounded-2xl border px-4 py-4 text-left transition active:scale-[0.99] ${profileForm.hasVirtualBase ? 'border-cyan-300/30 bg-cyan-300/12 text-cyan-50' : 'border-white/10 bg-white/10 text-slate-100'}`}>
       <span><b>Possuo base virtual</b><p className="mt-1 text-xs font-semibold opacity-75">A base virtual aparece como destino no Voo de Extra e é considerada nas diárias.</p></span>
...
function BillingSettingsCard({ status, busy, onRefresh, onStartTrial, onSubscribe, onRegularizeTrial, onCancel }: {
 ...
 const monthly = plans.find((plan) => plan.id === 'premium_monthly') || { id: 'premium_monthly', name: 'Premium mensal', value: 19.9, currency: 'BRL', cycle: 'MONTHLY', description: 'Premium completo com radar, Voo de Extra e e-mail interno.' };
...
function BillingComparisonTable({ free, monthly, annual, trialDays }: { free: CrewCheckPlanLike; monthly: CrewCheckPlanLike; annual: CrewCheckPlanLike; trialDays: number }) {
 const rows = [
  ...
  { label: 'Voo de Extra', free: 'Cálculo local e simulações', monthly: 'Fontes externas quando houver custo', annual: 'Fontes externas quando houver custo' },
...
function CrewCheckColorSettingsCard() {
 ...
 const rows: Array<[keyof typeof colors, string]> = [
  ['flightBg','Fundo voo normal'], ['flightFg','Fonte voo normal'], ['extraBg','Fundo Voo de Extra'], ['extraFg','Fonte Voo de Extra'],
  ...
  <div className="min-h-screen bg-[#08111f] px-5 py-8 text-white">
   <div className="mx-auto max-w-4xl">
    ...
    <div className="mt-6 grid gap-3 sm:grid-cols-2">
     ...
     <MenuAction icon={Radar} title="Voo de Extra" text="Melhor opção, voos reais e conexões cabíveis" onClick={() => onNavigate('departureVivo')} />
   ...
   <div className="mx-auto max-w-6xl">
    ...
    <section className="mt-6 grid gap-4 lg:grid-cols-[1fr_0.9fr]">
     ...
     <div className="crewcheck-light-card rounded-[2rem] border border-amber-300/20 bg-amber-400/10 p-5 shadow-2xl shadow-black/20">
      ...
      <p className="mt-3 text-sm leading-6 text-amber-50/85">O CrewCheck trata C32F como Check de Competência A32F, separado de voo, folga e Voo de Extra. Ele deve aparecer como treinamento/check, entrar no calendário como atividade de solo e servir como gatilho para revisão desta academia.</p>
