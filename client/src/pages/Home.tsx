...
type HomeView = 'home' | 'import' | 'history' | 'calendar' | 'iflight' | 'flightboard' | 'departure' | 'routine' | 'perdiem' | 'salary' | 'c32f' | 'settings' | 'support' | 'notes' | 'chief' | 'medical' | 'bids' | 'admin' | 'systemstatus' | 'more';
...
function loadLocalProfileSettings(): LocalProfileSettings {
 try {
  ...
  return { displayName: '', company: '', base: '', rank: '', isInstructor: false, hasVirtualBase: false, virtualBase: '' };
 }
}
...
function dashboardRelationForTarget(target: Date, endTarget: Date, todayStart: Date, now: Date, afterRest: boolean): { relation: DashboardNextEvent['relation']; relationLabel: string; dayContext: string; isToday: boolean; isTomorrow: boolean; isInProgress: boolean; isAfterRest: boolean } {
 ...
 if (afterRest) {
  ...
  return { relation: 'rest_followup', relationLabel: isTomorrow ? 'Amanhã · não é hoje' : `Dia ${target.toLocaleDateString('pt-BR', { day: '2-digit', month: '2-digit' })} · não é hoje`, dayContext: `Hoje consta folga/monofolga. A programação exibida é ${when}; o CrewCheck antecipa a leitura para você se organizar sem confundir com hoje.`, isToday, isTomorrow, isInProgress, isAfterRest: true };
 }
 if (isToday) return { relation: 'today', relationLabel: 'Hoje', dayContext: 'Programação de hoje ainda não finalizada.', isToday, isTomorrow, isInProgress, isAfterRest: false };
 if (isTomorrow) return { relation: 'tomorrow', relationLabel: 'Amanhã · não é hoje', dayContext: 'A próxima programação é amanhã. Hoje não há programação operacional pendente para deslocamento no cockpit.', isToday, isTomorrow, isInProgress, isAfterRest: false };
 return { relation: 'future', relationLabel: `Dia ${target.toLocaleDateString('pt-BR', { day: '2-digit', month: '2-digit' })} · não é hoje`, dayContext: `A próxima programação operacional está em ${daysAway} dias.`, isToday, isTomorrow, isInProgress, isAfterRest: false };
}
...
function buildDashboardNextEvent(bundle: SessionRosterBundle | null, saved?: SavedRosterSummary): DashboardNextEvent {
 ...
 const candidates = [...roster.days]
  .map((day, index) => {
   ...
   const displayRoute = buildDashboardDisplayRoute(day, firstLeg, lastLeg);
   const route = firstLeg ? (displayRoute || `${firstLeg.origin} → ${firstLeg.destination || lastLeg?.destination || ''}`) : (activation?.title || day.pairingCode || day.type || 'Programação');
   ...
   return { day, firstLeg, lastLeg, range: { ...range, start }, route, title, target, endTarget, smartTarget: smart.target, smartRule: smart.rule, index, base, requiresDeparture, activation, isRestOnly };
  })
   ...
   <div className="mx-auto max-w-5xl">
    ...
    <section className="mt-6 rounded-[2rem] border border-cyan-300/18 bg-[linear-gradient(135deg,rgba(34,211,238,.13),rgba(255,255,255,.055),rgba(37,99,235,.10))] p-5 shadow-2xl shadow-cyan-950/20">
     ...
     <div className="mt-5 flex flex-col gap-3 sm:flex-row sm:flex-wrap">
      <button onClick={exportCalendar} className="rounded-2xl bg-cyan-300 px-5 py-4 text-sm font-black text-[#061426]"><DownloadIcon className="mr-2 inline h-4 w-4" /> Exportar para calendário</button>
...
export default function Home() {
 ...
 useEffect(() => {
  try {
   ...
   if (syncedAvatar && syncedAvatar !== profileAvatar) setProfileAvatar(syncedAvatar);
  } catch {}
 }, [user?.email]);
 ...
 useEffect(() => {
  try {
   ...
   for (const day of roster.days.slice(0, 45)) {
    ...
    const ok = scheduleCrewCheckNotification('CrewCheck · Check-in da escala', `Lembrete premium: preencher o formulário de Check-in da escala para ${reserve ? 'reserva/ASB' : 'Vivo de Extra/PS'} às ${start}.`, reminder);
 ...
 const publishRosterSyncBundle = useCallback((roster: CrewRoster, compliance: ComplianceResult, gym: GymRecommendation[], sourceFileName: string) => {
  const payload = { roster, compliance, gym, sourceFileName, updatedAt: new Date().toISOString(), source: 'crewcheck-unified-sync-v108134' };
  try {
   localStorage.setItem('crewcheck_roster_sync_latest_v108134', JSON.stringify(payload));
   localStorage.setItem('crewcheck_latest_roster_bundle', JSON.stringify(payload));
   window.dispatchEvent(new CustomEvent('crewcheck:roster-sync-updated', { detail: payload }));
   if ('BroadcastChannel' in window) {
    const channel = new BroadcastChannel('crewcheck-roster-sync');
 ...
 useEffect(() => {
  ...
  const storageHandler = (event: StorageEvent) => {
   if (event.key === 'crewcheck_roster_sync_latest_v108134' && event.newValue) {
  ...
  try {
   if ('BroadcastChannel' in window) {
    channel = new BroadcastChannel('crewcheck-roster-sync');
 ...
 const handleDashboardNavigate = useCallback((view: string) => {
  const map: Record<string, () => void> = {
   ...
   iflight: () => canAccessC32FAcademy ? setCurrentView('iflight') : (toast.info('iFlight AutoPull é restrito ao administrador. Use a importação manual de PDF oficial.'), setCurrentView('import')),
   flightboard: () => setCurrentView('flightboard'),
   departureVivo: () => { try { localStorage.setItem('crewcheck_departure_focus', 'vivo'); } catch {} setCurrentView('departure'); },
   ...
   systemstatus: () => canAccessC32FAcademy ? setCurrentView('systemstatus') : toast.info('Status do sistema restrito ao administrador.'),
   ...
   billing: () => { try { localStorage.setItem('crewcheck_settings_category', 'billing'); } catch {} setCurrentView('settings'); },
   subscription: () => { try { localStorage.setItem('crewcheck_settings_category', 'billing'); } catch {} setCurrentView('settings'); },
   assinatura: () => { try { localStorage.setItem('crewcheck_settings_category', 'billing'); } catch {} setCurrentView('settings'); },
 ...
 const commitRoster = useCallback(async (inputRoster: CrewRoster, sourceFileName: string, targetView: ResultsView = 'summary') => {
  ...
  setInitialResultsView(targetView);
  void forceSyncRosterEverywhere(roster, compliance, gym, sourceFileName);
   ...
   <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
    <div className="flex items-start gap-3">
     ...
     <div>
      ...
      <h3 className="mt-0.5 text-base font-black leading-tight sm:text-xl">Teste Saída Inteligente, Vivo de Extra, Radar, Diárias e Salário sem cartão de crédito.</h3>
...
function CrewCheckMobileMenu({ active, onNavigate }: { active: CrewCheckBottomActive; onNavigate: (view: string) => void }) {
 ...
 const items: { id: 'home' | 'roster' | 'vivo' | 'flightboard' | 'settings'; label: string; fullLabel: string; icon: LucideIcon; view: string }[] = [
  ...
  { id: 'vivo', label: 'Vivo', fullLabel: 'Vivo de Extra na Saída Inteligente', icon: Plane, view: 'departureVivo' },
  { id: 'flightboard', label: 'Radar', fullLabel: 'Radar de Voos', icon: Radar, view: 'flightboard' },
  { id: 'settings', label: 'Conta', fullLabel: 'Conta e Configurações', icon: Settings, view: 'settings' },
 ];
 return (
...
function CrewCheckFloatingSpeedMenu({ active, onNavigate, canAccessC32FAcademy, hasRoster }: { active: 'home' | 'roster' | 'iflight' | 'departure' | 'settings' | 'more'; onNavigate: (view: string) => void; canAccessC32FAcademy: boolean; hasRoster: boolean }) {
 ...
 const quickItems: Array<{ label: string; hint: string; icon: LucideIcon; view: string; premium?: boolean }> = [
  { label: 'Chefe de Cabine', hint: 'caderno, água, horários', icon: ClipboardList, view: 'chief', premium: true },
  { label: 'Checklist Médico', hint: 'ocorrência e PDF', icon: FileText, view: 'chief', premium: true },
  ...
  { label: 'Vivo de Extra', hint: 'melhor opção e conexões', icon: Radar, view: 'departureVivo', premium: true },
  ...(canAccessC32FAcademy ? [
   { label: 'Admin geral', hint: 'saúde, financeiro e gestão', icon: BarChart3, view: 'admin', premium: true },
   { label: 'Status do sistema', hint: 'APIs, banco e radar', icon: Activity, view: 'systemstatus', premium: true },
    ...
    <div className="crewcheck-speed-dial-panel mb-3 w-[min(21rem,calc(100vw-2rem))] rounded-[1.55rem] border p-3 shadow-2xl backdrop-blur-2xl">
     <div className="mb-2 flex items-center justify-between px-2">
      <div>
       ...
       <p className="text-xs font-semibold opacity-70">Acesso rápido aos módulos premium</p>
      </div>
      <span className="rounded-full px-2 py-1 text-[9px] font-black uppercase tracking-[0.12em]">v11.0.3</span>
     </div>
...
function buildExtraFlightOptionsForPresentation(originAirport: string, targetAirport: string, targetIso?: string, selectedDate?: string): ExtraFlightOptionEstimate[] {
 ...
 if (directBlock) {
  ...
  options.push({
   ...
   legs: [buildEstimatedLeg('Trecho direto', from, to, depart, directBlock, 'LATAM/GOL/Azul', 'Validar disponibilidade real e política de Vivo de Extra.')],
  });
 }
 for (const hub of CREWCHECK_EXTRA_FLIGHT_HUBS) {
  ...
  options.push({
   kind: 'connection',
   title: 'Vivo de Extra com conexão',
   route: `${from} → ${hub} → ${to}`,
...
function SmartDepartureScreen({ nextEvent, bundle, userBase, hasRoster, canAccessAdmin = false, onBack, onOpenRoster }: { nextEvent?: DashboardNextEvent; bundle?: SessionRosterBundle | null; userBase?: string; hasRoster: boolean; canAccessAdmin?: boolean; onBack: () => void; onOpenRoster: () => void }) {
 ...
 const scheduleCurrentDepartureAlert = useCallback(() => {
  ...
  if (!targetIso) {
   toast.info('Carregue uma escala ou selecione uma opção de Vivo de Extra para agendar o alerta.');
   return;
  }
  ...
  const targetLabel = selectedExtraOption ? `Vivo de Extra ${selectedExtraOption.route}` : (nextEvent?.title || 'sua próxima programação');
 ...
 const runGeolocationCalculation = useCallback(async () => {
  if (!canCalculate) {
   setError('Importe uma escala, abra uma escala salva ou selecione uma opção de Vivo de Extra para calcular a hora de sair.');
   return;
  }
  ...
  try {
   ...
   const calculationMargin = selectedExtraOption ? Math.max(55, margin) : margin;
   ...
   if (selectedExtraOption) params.set('extraFlight', `${selectedExtraOption.route} · ${selectedExtraOption.leaveAt}`);
 ...
 }, [airport, base2Address, base2Cep, base2Label, canCalculate, homeAddress, homeCep, margin, mode, nextEvent?.targetIso, nextEvent?.smartDepartureTargetIso, originMode, permission, selectedExtraDepartureAirport, selectedExtraDepartureIso, selectedExtraOption, validateSelectedAddress]);
 ...
 const smartDepartureHeroDescription = isRestOnlyProgram
  ...
  : 'O CrewCheck compara carro, transporte público e corrida por app com a localização real autorizada, calcula a hora limite de saída e mostra a fonte do cálculo para você confiar antes de sair de casa.';
      ...
      <div className="p-6 md:p-8">
       ...
       <div ref={vivoSectionRef} className="mt-5 rounded-[1.7rem] border border-fuchsia-200/20 bg-fuchsia-300/10 p-4 shadow-xl shadow-black/10">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
         <div>
          <p className="text-[10px] font-black uppercase tracking-[0.22em] text-fuchsia-100/70">Vivo de Extra</p>
        ...
        <div className="mt-4 grid gap-3 md:grid-cols-2 xl:grid-cols-4">
         <label className="rounded-2xl border border-white/10 bg-black/20 p-3">
          ...
          <input value={originAirportForExtra} onChange={(event) => { const value = event.target.value.toUpperCase().replace(/[^A-Z]/g, '').slice(0, 3); setOriginAirportForExtra(value); localStorage.setItem('crewcheck_extra_origin_airport', value); }} placeholder="Ex.: GRU" className="mt-2 w-full rounded-xl border border-white/10 bg-white/90 px-3 py-3 text-sm font-black text-slate-950 outline-none placeholder:text-slate-500" />
         </label>
         <label className="rounded-2xl border border-white/10 bg-black/20 p-3">
        ...
        {selectedExtraOption ? (
         <div className="mt-3 rounded-2xl border border-emerald-200/30 bg-emerald-300/12 px-4 py-3 text-xs font-black text-emerald-50">
          Opção selecionada para Saída Inteligente: {selectedExtraOption.route} · embarque {cleanExtraClock(selectedExtraOption.legs?.[0]?.departure || selectedExtraOption.leaveAt) || selectedExtraOption.leaveAt} · aeroporto {selectedExtraDepartureAirport || 'a confirmar'}.
         </div>
        ) : null}
...
function ImportScreen({ userLabel, roleSelection, onRoleSelectionChange, isDragging, isProcessing, error, fileName, fileInputRef, isAdmin, onOpenIFlight, onBack, onDrop, onDragOver, onDragLeave, onFileInput, onLogout }: {
 userLabel: string;
      ...
      <Card className="crewcheck-light-card relative overflow-hidden rounded-[2rem] border-white/10 bg-white/[0.08] p-4 text-white shadow-2xl shadow-black/30 backdrop-blur-2xl">
       ...
       <div className="crewcheck-light-card rounded-[1.5rem] border border-white/10 bg-[#0B1730]/70 p-5 md:p-7">
        ...
        <div className="mb-5 grid gap-3 md:grid-cols-2">
         <div className="rounded-[1.2rem] border border-emerald-300/20 bg-emerald-300/10 p-4"><p className="text-sm font-black text-emerald-100">Importação de PDF oficial</p><p className="mt-1 text-xs leading-5 text-emerald-50/80">Fluxo padrão restaurado: selecione o PDF exportado pelo CrewTopia, CrewRosterReport/AIMS ou fonte oficial disponível.</p></div>
         {isAdmin ? <button type="button" onClick={onOpenIFlight} className="rounded-[1.2rem] border border-orange-300/20 bg-orange-300/10 p-4 text-left transition hover:-translate-y-0.5 hover:border-orange-300/45"><p className="text-sm font-black text-orange-100">iFlight Admin · em desenvolvimento</p><p className="mt-1 text-xs leading-5 text-orange-50/80">Laboratório restrito para validar captura automática. Usuários finais continuam somente com PDF manual.</p><span className="mt-3 inline-flex rounded-full bg-orange-200 px-3 py-1 text-xs font-black text-orange-950">Abrir área admin</span></button> : <div className="rounded-[1.2rem] border border-cyan-300/15 bg-cyan-300/10 p-4"><p className="text-sm font-black text-cyan-100">iFlight automático em desenvolvimento</p><p className="mt-1 text-xs leading-5 text-cyan-50/80">Nesta versão, envie o PDF oficial. A captura automática aparecerá somente quando estiver validada.</p></div>}
        </div>
        ...
        <div onDragOver={onDragOver} onDragLeave={onDragLeave} onDrop={onDrop} className={`crewcheck-light-card relative rounded-[1.35rem] border-2 border-dashed p-8 text-center transition-all duration-300 ${isDragging ? 'scale-[1.015] border-cyan-300 bg-cyan-300/10' : 'border-white/15 bg-white/[0.04] hover:border-fuchsia-300/60 hover:bg-white/[0.07]'} ${isProcessing ? 'pointer-events-none opacity-70' : ''}`}>
         {isProcessing ? (
          ...
          <div className="flex flex-col items-center gap-5 py-4">
           ...
           <div><p className="text-xl font-black">Arraste sua escala em PDF aqui</p><p className="mt-2 text-sm text-slate-300">CrewTopia, CrewRosterReport ou AIMS — PDF original exportado do sistema</p></div>
           ...
           <p className="max-w-md text-xs leading-5 text-slate-400">No iPhone/iPad, prefira Arquivos → No iPhone/iCloud Drive. Se veio do WhatsApp, salve em Arquivos antes. A captura automática do iFlight está em desenvolvimento e fica restrita ao administrador.</p>
          </div>
         )}
        </div>
 ...
 return cleanAirportMonitorRows(rows)
  ...
  .map((row) => {
   ...
   return {
    ...
    title: 'Vivo de Extra direto · monitor/API',
    route: `${origin} → ${target}`,
    carriers: `${carrier}${row.flightNumber ? ` · ${row.flightNumber}` : ''}`,
    leaveAt: departure,
    arriveAt: row.arrivalTime || 'validar no bilhete',
    durationMinutes: 0,
    confidence: `Monitor/API · ${selectedDate} · portão ${row.gate || 'não informado'} · ${status}`,
    ...
    details: `Voo retornado por monitor/API para ${origin} → ${target}. Portão e terminal só aparecem quando o provedor informar. Preço exibido apenas quando o provedor retornar valor; caso contrário, confirme no canal da companhia.`,
    advisory: 'Confirme aceitação do Vivo de Extra, valor da passagem, assento, check-in, bagagem e eventual troca de terminal.',
    legs: [{
     ...
     status,
     source: row.source || 'Radar CrewCheck',
     note: 'Informação vinda do monitor/API. Horário de chegada pode não estar disponível no painel de partidas.',
    }],
   };
  });
}
  ...
  <div role="button" tabIndex={0} onClick={onToggle} onKeyDown={(event) => { if (event.key === 'Enter' || event.key === ' ') { event.preventDefault(); onToggle(); } }} className={`group cursor-pointer rounded-[2rem] border bg-white p-4 text-slate-950 shadow-xl shadow-slate-950/5 transition hover:-translate-y-0.5 hover:shadow-2xl dark:border-white/10 dark:bg-white/[0.07] dark:text-white ${selected ? 'ring-2 ring-cyan-300/70 border-cyan-300/40' : 'border-slate-200/80'}`}>
   <div className="flex items-start justify-between gap-3">
    <div className="min-w-0">
     ...
     <h4 className="mt-2 truncate text-3xl font-black leading-none tracking-tight text-slate-950 dark:text-white">{primaryFlights || item.title.replace(/^Vivo de Extra\s*/i, '').replace(/·.*$/, '').trim() || 'Voo a validar'}</h4>
...
function FlightBoardScreen({ onBack, isAdmin = false }: { onBack: () => void; isAdmin?: boolean }) {
 ...
 const loadBoard = useCallback(async (forceRefresh = false) => {
  ...
  try {
   ...
   const params = new URLSearchParams({ airport, type, limit: '100', diagnostics: '1', date: today, apiFirst: '1' });
   ...
   setBoardMessage(apiRows.length ? String(payload.message || 'Dados carregados da API de voos/radar CrewCheck.') : 'Sem retorno confiável da API para estes filtros. O CrewCheck não exibe fallback de escala no radar.');
 ...
 }, [airport, type, airline, boardDate]);
 ...
 const filteredRows = segmentedRows.filter((row) => {
  ...
  return matchesQuery && matchesTraffic;
 });
    ...
    <section className="mt-6 overflow-hidden rounded-[2.2rem] border border-cyan-300/20 bg-[radial-gradient(circle_at_20%_0%,rgba(14,165,233,.35),transparent_35%),linear-gradient(135deg,rgba(8,47,73,.92),rgba(2,6,23,.98))] shadow-2xl shadow-black/35">
     <div className="border-b border-cyan-200/10 px-5 py-5 sm:px-7">
      ...
      <div className="mt-5 flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
       ...
       <div className="flex items-center gap-2 rounded-2xl border border-white/10 bg-white/8 px-4 py-3 text-xs font-bold text-cyan-50/75"><SlidersHorizontal className="h-4 w-4 text-cyan-200" /> {trafficScopeLabel(trafficFilter)} · {filteredRows.length} voo(s) · atualizado {updatedAt ? new Date(updatedAt).toLocaleTimeString('pt-BR', { hour: '2-digit', minute: '2-digit' }) : '—'}{boardCacheLabel ? ` · ${boardCacheLabel}` : ''}</div>
      </div>
     </div>
  ...
  <div className="min-h-screen bg-[#08111f] px-5 py-8 text-white">
   <div className="mx-auto max-w-3xl">
    ...
    <div className="mt-6 rounded-3xl border border-white/10 bg-white/[0.05] p-5">
     <textarea value={note} onChange={(event) => setNote(event.target.value)} placeholder="Ex.: observar pernoite, descanso, Vivo de Extra, treinamento, pendência pessoal..." className="min-h-[18rem] w-full rounded-2xl border border-white/10 bg-[#07111f] p-4 text-sm leading-6 text-white outline-none focus:border-cyan-300" />
...
function SettingsHomePanel({ userLabel, userEmail, profileSettings, avatar, pendingOffline, hasRoster, canAccessAdmin, themeMode, onThemeModeChange, onProfileSettingsChange, onAvatarChange, onBack, onOpenIFlight, onOpenImport, onOpenResultsSettings, onOpenPerDiem, onOpenSalary, onExportPdf, onExportIcs, onSyncPending, onLogout }: {
 ...
 type SettingsCategoryKey = 'profile' | 'billing' | 'calendar' | 'preferences' | 'notifications' | 'finance' | 'exports' | 'privacy' | 'manual';
 const settingsCategoryKeys: SettingsCategoryKey[] = ['profile', 'billing', 'calendar', 'preferences', 'notifications', 'finance', 'exports', 'privacy', 'manual'];
 ...
 useEffect(() => {
  try {
   ...
   if (requested && settingsCategoryKeys.includes(requested)) {
    ...
    setSettingsPulse(requested);
    localStorage.removeItem('crewcheck_settings_category');
    window.setTimeout(() => {
    ...
    <SettingsCategoryCard eyebrow="Perfil local" title="Quem aparece no CrewCheck" description="Esses dados ficam salvos apenas neste dispositivo e ajudam a identificar chefia, base e exportações.">
     ...
     <div className="mt-4 rounded-2xl border border-white/10 bg-white/[0.045] p-4">
      <button type="button" onClick={() => setProfileForm({ ...profileForm, hasVirtualBase: !profileForm.hasVirtualBase })} className={`flex w-full items-center justify-between gap-3 rounded-2xl border px-4 py-4 text-left transition active:scale-[0.99] ${profileForm.hasVirtualBase ? 'border-cyan-300/30 bg-cyan-300/12 text-cyan-50' : 'border-white/10 bg-white/10 text-slate-100'}`}>
       <span><b>Possuo base virtual</b><p className="mt-1 text-xs font-semibold opacity-75">A base virtual aparece como destino no Vivo de Extra e é considerada nas diárias.</p></span>
     ...
     <div className="mt-5 rounded-[1.6rem] border border-cyan-200/20 bg-white/[0.055] p-4 shadow-xl shadow-black/10">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
       <div>
        ...
        <p className="mt-1 text-sm leading-6 text-slate-300">Use o servidor de e-mail configurado no Render. O destinatário padrão é o e-mail cadastrado do usuário.</p>
       </div>
      ...
      <p className="mt-3 text-xs leading-5 text-slate-400">Para anexos completos, o backend já aceita anexos Base64 em <code>/api/email/share</code>. Esta tela envia o resumo e também aciona a geração local do PDF/ICS quando selecionado.</p>
     </div>
  ...
  if (activeCategory === 'privacy') {
   return (
    <SettingsCategoryCard eyebrow="LGPD" title="Privacidade sem ruído técnico" description="Informações corporativas sensíveis não ficam salvas no CrewCheck. A senha e MFA sempre permanecem no portal oficial.">
     <div className="grid gap-3 sm:grid-cols-2"><StatusTile icon={Lock} title="Senha corporativa" text="não armazenada" /><StatusTile icon={Shield} title="Perfil" text="salvo localmente" /><StatusTile icon={Database} title="Histórico" text="pode funcionar offline" /><StatusTile icon={ExternalLink} title="iFlight" text="AutoPull admin" /></div>
  ...
  return (
   <SettingsCategoryCard eyebrow="Ajuda premium" title="Manuais, suporte e Android" description="O CrewCheck está focado em tripulantes LATAM neste lançamento e está aberto a expansão para outras empresas. O plano gratuito mantém manuais e FAQ; suporte direto é Premium.">
    ...
    <div className="grid gap-3 sm:grid-cols-2">
     ...
     <button onClick={() => window.open('/manuals/funcionalidades-crewcheck-v11-premium.pdf', '_blank', 'noopener,noreferrer')} className="rounded-2xl bg-violet-200 px-5 py-4 text-sm font-black text-[#07111f]"><Sparkles className="mr-2 inline h-4 w-4" /> Funcionalidades para vender</button>
...
function BillingSettingsCard({ status, busy, onRefresh, onStartTrial, onSubscribe, onRegularizeTrial, onCancel }: {
 ...
 const monthly = plans.find((plan) => plan.id === 'premium_monthly') || { id: 'premium_monthly', name: 'Premium mensal', value: 19.9, currency: 'BRL', cycle: 'MONTHLY', description: 'Premium completo com radar, Vivo de Extra e e-mail interno.' };
 ...
 const paymentMethodOptions: Array<{ type: BillingCheckoutBillingType; title: string; description: string; badge: string }> = [
  { type: 'UNDEFINED', title: 'Asaas completo', description: 'Mantém as formas habilitadas na sua conta: Pix, boleto e cartão quando disponíveis.', badge: 'Recomendado' },
  { type: 'PIX', title: 'Pix dentro do app', description: 'Mostra QR Code e Pix copia e cola no CrewCheck quando o Asaas retornar os dados.', badge: 'Rápido' },
  { type: 'CREDIT_CARD', title: 'Cartão recorrente', description: 'Cliente informa cartão no ambiente seguro do Asaas. Não coletamos dados do cartão no CrewCheck.', badge: 'Crédito/débito' },
  { type: 'BOLETO', title: 'Boleto', description: 'Gera assinatura com boleto e página segura do Asaas para conclusão.', badge: 'Alternativo' },
 ];
 ...
 const paymentActionLabel = checkoutIsTrial ? 'Iniciar teste grátis' : checkoutIsRegularization ? 'Regularizar assinatura' : checkoutBillingType === 'PIX' ? 'Criar cobrança Pix' : checkoutBillingType === 'CREDIT_CARD' ? 'Criar assinatura no cartão' : checkoutBillingType === 'BOLETO' ? 'Criar cobrança boleto' : 'Criar checkout Asaas';
 ...
 const checkoutSubtitle = checkoutIsRegularization
  ...
  : checkoutIsTrial
   ...
   : 'Escolha a forma de cobrança. O CrewCheck mostra as informações retornadas pelo Asaas e só exibe Pix se o usuário tocar nessa opção.';
  ...
  <SettingsCategoryCard eyebrow="Assinatura" title="Premium CrewCheck" description={`Mais do que ver escala: organização de rotina, descanso priorizado e tempo de qualidade. Mensal promocional por ${promoMonths} meses para contratações até 19 de julho.`}>
   <div className="rounded-[2rem] border border-amber-200/25 bg-gradient-to-br from-amber-200/16 via-white/[0.055] to-cyan-300/10 p-5 shadow-2xl shadow-black/20">
    <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
     <div>
      ...
      <p className="mt-2 text-sm leading-6 text-slate-300">{isLifetime ? 'Premium Unlimited liberado. Esta conta não gera cobrança nem precisa iniciar teste.' : premium ? 'Recursos premium liberados nesta conta.' : 'Você está no plano básico gratuito.'} {endLabel ? `Validade prevista: ${endLabel}.` : ''}</p>
     </div>
     <div className="flex flex-wrap gap-2">
   ...
   <div className="mt-5 rounded-[2rem] border border-cyan-200/25 bg-[#07111f]/70 p-5 shadow-xl shadow-cyan-950/20">
    <div className="grid gap-4 lg:grid-cols-[1.15fr_.85fr] lg:items-center">
     <div>
      ...
      <p className="mt-2 text-sm leading-6 text-slate-300">O CrewCheck cria a cobrança, mantém a tela com status claro e deixa cartão, débito, boleto ou Pix organizados em opções. Dados do cartão ficam somente no ambiente seguro do Asaas.</p>
     </div>
     <div className="grid gap-2 sm:grid-cols-3 lg:grid-cols-1 xl:grid-cols-3">
      ...
      <StatusTile icon={CreditCard} title="Opções Asaas" text="cartão, boleto e Pix" />
     </div>
    </div>
   </div>
   ...
   <div className="mt-5 grid gap-4 lg:grid-cols-3">
    <PlanCard
     ...
     caption="Para manter o app acessível"
     features={["Importação manual da escala", "Programação a seguir", "Legenda de códigos", "Consulta local essencial", "Manual e FAQ", "Teste Premium sem cartão"]}
     footer="Sem cartão e sem cobrança."
    />
    <PlanCard
     highlight
     title={monthly.name}
     ...
     caption="Premium mensal"
     features={["Saída Inteligente com foco em descanso", "Escala organizada por dia", "Radar e status de voos", "Diárias, salário e calendário", "Rotina para reduzir desgaste"]}
    ...
    <PlanCard
     ...
     features={["Tudo do Premium mensal", "Melhor custo-benefício", "Ajuda a manter servidor e APIs", "Prioridade em melhorias premium", "Menos cobranças durante o ano"]}
   ...
   <div className="mt-5 grid gap-4 xl:grid-cols-2">
    <Version11FeatureBox
     eyebrow="Free Trial"
     title="Teste sem risco e sem cartão"
     description="O usuário experimenta o valor real antes de pagar: escala organizada, rotina, descanso e previsibilidade. O trial segue com cobrança programada somente após 7 dias."
     items={[
    ...
    <Version11FeatureBox
     ...
     title="Premium para qualidade de vida"
     description="Posicionamento comercial focado em tempo de qualidade: rotina planejada, descanso priorizado e menos energia gasta tentando entender a escala."
     items={[
      'Saída Inteligente com deslocamento',
      'Radar de voos e portões quando disponível',
      'Diárias, salário e histórico',
      'Rotina com foco em descanso',
      'Mapa premium da escala',
    ...
    <div className="max-h-[92vh] w-full max-w-2xl overflow-y-auto rounded-[2rem] border border-cyan-200/25 bg-[#07111f] p-5 shadow-2xl shadow-black/45">
     ...
     {checkoutPhase === 'form' && <>
      ...
      {checkoutIsPaidPlan && <div className="mt-5 rounded-[1.5rem] border border-cyan-200/20 bg-cyan-300/10 p-4">
       <div className="flex flex-col gap-1 sm:flex-row sm:items-end sm:justify-between">
        <div>
         ...
         <p className="mt-1 text-xs font-semibold leading-5 text-cyan-50/80">Mantém as opções do Asaas. Nenhum dado de cartão é digitado no CrewCheck.</p>
        </div>
      ...
      <label className="mt-5 block">
       <span className="text-xs font-black uppercase tracking-[0.18em] text-cyan-100/60">CPF para cobrança</span>
       <input
        autoFocus
        type="tel"
        inputMode="numeric"
      ...
      <div className="mt-4 rounded-2xl border border-cyan-200/20 bg-cyan-300/10 p-4 text-xs font-semibold leading-5 text-cyan-50/90">O CPF fica visível apenas enquanto você digita para facilitar conferência. Depois, não aparece no status nem em mensagens técnicas; é usado somente para emissão da cobrança.</div>
     ...
     {checkoutPhase === 'creating' && <div className="mt-6 rounded-[1.8rem] border border-cyan-200/25 bg-cyan-300/10 p-5 text-center">
      ...
      <div className="mt-5 grid gap-3 sm:grid-cols-3">
       <StatusTile icon={Shield} title="1. Proteção" text="sem payload exposto" />
     ...
     {checkoutPhase === 'ready' && checkoutIsPaidPlan && <div className="mt-6 space-y-4">
      <div className="rounded-[1.8rem] border border-emerald-200/25 bg-emerald-300/10 p-5">
       <p className="text-xs font-black uppercase tracking-[0.22em] text-emerald-100/75">Informações do Asaas recebidas</p>
      ...
      {checkoutReadyView === 'options' && <div className="grid gap-3 md:grid-cols-3">
       <button type="button" onClick={() => checkoutUrl && window.open(checkoutUrl, '_blank', 'noopener,noreferrer')} disabled={!checkoutUrl} className="crewcheck-payment-option-card rounded-[1.5rem] border border-cyan-200/25 bg-cyan-300/10 p-5 text-left transition hover:bg-cyan-300/15 disabled:opacity-50">
        ...
        <p className="mt-2 text-xs font-semibold leading-5 text-slate-300">Abre manualmente o checkout seguro do Asaas. O CrewCheck não exibe nem coleta dados de cartão.</p>
       ...
       <button type="button" onClick={() => pixAvailable ? setCheckoutReadyView('pix') : checkoutUrl && window.open(checkoutUrl, '_blank', 'noopener,noreferrer')} className="crewcheck-payment-option-card rounded-[1.5rem] border border-emerald-200/25 bg-emerald-300/10 p-5 text-left transition hover:bg-emerald-300/15">
        ...
        <p className="mt-2 text-xs font-semibold leading-5 text-slate-300">{pixAvailable ? 'Mostra QR Code e copia e cola somente se o usuário escolher Pix.' : 'Pix disponível no checkout Asaas quando habilitado na conta.'}</p>
       ...
       <button type="button" onClick={() => checkoutUrl && window.open(checkoutUrl, '_blank', 'noopener,noreferrer')} disabled={!checkoutUrl} className="crewcheck-payment-option-card rounded-[1.5rem] border border-white/10 bg-white/[0.055] p-5 text-left transition hover:bg-white/[0.09] disabled:opacity-50">
        ...
        <p className="mt-2 text-xs font-semibold leading-5 text-slate-300">Mantém as opções habilitadas no Asaas sem abrir pop-up automático.</p>
      ...
      <div className="rounded-[1.5rem] border border-white/10 bg-white/[0.055] p-4">
       <div className="flex items-start gap-3">
        ...
        <div>
         ...
         <p className="mt-1 text-xs font-semibold leading-5 text-slate-400">O checkout interno mostra apenas valor, vencimento e opções. Dados cadastrais do Asaas não são embutidos em iframe dentro do sistema.</p>
        </div>
       </div>
      </div>
     </div>}
...
function PremiumTrustPromise() {
 const items = [
  ...
  { icon: CreditCard, title: 'Sem cartão no CrewCheck', text: 'cartão/débito ficam no ambiente seguro do Asaas' },
...
function BillingComparisonTable({ free, monthly, annual, trialDays }: { free: CrewCheckPlanLike; monthly: CrewCheckPlanLike; annual: CrewCheckPlanLike; trialDays: number }) {
 const rows = [
  ...
  { label: 'Saída Inteligente', free: 'Consulta básica', monthly: 'Deslocamento com foco em chegar sem desgaste', annual: 'Deslocamento com foco em chegar sem desgaste' },
  { label: 'Escala organizada', free: 'Visualização básica', monthly: 'Cards premium por dia e detalhes claros', annual: 'Cards premium por dia e detalhes claros' },
  { label: 'Rotina e descanso', free: 'Recomendações simples', monthly: 'Sugestões para encaixar atividade sem piorar cansaço', annual: 'Sugestões para encaixar atividade sem piorar cansaço' },
  { label: 'Vivo de Extra', free: 'Limitado', monthly: 'Melhor opção + conexões reais', annual: 'Melhor opção + conexões reais' },
  { label: 'Radar de voos', free: 'Resumo do dia', monthly: 'Portão, status e cache premium', annual: 'Portão, status e cache premium' },
  { label: 'Diárias e salário', free: 'Resumo simples', monthly: 'Cálculo detalhado', annual: 'Cálculo detalhado' },
  { label: 'Agenda e histórico', free: 'Mês atual salvo', monthly: 'Histórico e integrações', annual: 'Histórico e integrações' },
  ...
  <div className="mt-6 rounded-[1.8rem] border border-cyan-200/18 bg-white/[0.055] p-4 shadow-2xl shadow-black/15 backdrop-blur-2xl sm:p-5">
   <div className="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
    <div>
     ...
     <h4 className="mt-1 text-2xl font-black text-white">Escolha pelo tempo que você recupera, não só pelo preço</h4>
     <p className="mt-2 max-w-3xl text-sm leading-6 text-slate-400">O plano gratuito mantém o CrewCheck acessível. O Premium sustenta servidor, APIs, radar, rotas, suporte e melhorias — com foco em previsibilidade, descanso e qualidade de vida.</p>
    </div>
   ...
   <div className="mt-4 grid gap-3 md:grid-cols-3">
    ...
    <StatusTile icon={CreditCard} title="CPF protegido" text="CPF usado somente para cobrança e não exibido no status." />
    <StatusTile icon={Sparkles} title="Premium transparente" text="Teste antes da cobrança e preço pensado para sustentar o projeto." />
   </div>
  </div>
 );
}
...
function CrewCheckColorSettingsCard() {
 ...
 const rows: Array<[keyof typeof colors, string]> = [
  ['flightBg','Fundo voo normal'], ['flightFg','Fonte voo normal'], ['extraBg','Fundo Vivo de Extra'], ['extraFg','Fonte Vivo de Extra'],
...
function SystemStatusScreen({ onBack }: { onBack: () => void }) {
 ...
 const tiles = [
  ...
  { title: 'Lucro estimado', ok: Boolean(systemDashboard?.finance), detail: systemDashboard?.finance ? `${formatMoney(systemDashboard.finance.monthlyProfit || 0)} / mês · margem ${systemDashboard.finance.profitMarginPercent === null || systemDashboard.finance.profitMarginPercent === undefined ? '—' : `${Number(systemDashboard.finance.profitMarginPercent).toFixed(1)}%`}` : 'configure custos no Render' },
  ...
  { title: 'Asaas / cobrança', ok: Boolean(billingHealth?.ok), detail: billingHealth?.message || (billingHealth?.configured ? 'configurado, aguardando teste' : 'ASAAS_API_KEY ausente') },
 ...
 return (
  <div className="min-h-screen bg-[#08111f] px-5 py-8 text-white">
   <div className="mx-auto max-w-5xl">
    <HeaderBack title="Status do Sistema" subtitle="Painel administrativo: APIs, banco, radar e segurança" onBack={onBack} />
    ...
    <section className="mt-5 grid gap-4 lg:grid-cols-3">
     ...
     <div className="rounded-[1.7rem] border border-cyan-200/20 bg-cyan-300/10 p-5 shadow-xl shadow-black/20">
      ...
      <p className="mt-2 text-sm font-semibold text-cyan-50/75">Inclui Render, voos, e-mail, Maps, domínio e outros custos informados no Render.</p>
     </div>
  ...
  <div className="min-h-screen bg-[#08111f] px-5 py-8 text-white">
   <div className="mx-auto max-w-4xl">
    ...
    <div className="mt-6 grid gap-3 sm:grid-cols-2">
     ...
     <MenuAction icon={Radar} title="Vivo de Extra" text="Melhor opção, voos reais e conexões cabíveis" onClick={() => onNavigate('departureVivo')} />
     ...
     {canAccessC32FAcademy && <MenuAction icon={Activity} title="Status do Sistema" text="APIs, banco, e-mail, Maps e radar" onClick={() => onNavigate('systemstatus')} />}
   ...
   <div className="mx-auto max-w-6xl">
    ...
    <section className="mt-6 grid gap-4 lg:grid-cols-[1fr_0.9fr]">
     ...
     <div className="crewcheck-light-card rounded-[2rem] border border-amber-300/20 bg-amber-400/10 p-5 shadow-2xl shadow-black/20">
      ...
      <p className="mt-3 text-sm leading-6 text-amber-50/85">O CrewCheck trata C32F como Check de Competência A32F, separado de voo, folga e Vivo de Extra. Ele deve aparecer como treinamento/check, entrar no calendário como atividade de solo e servir como gatilho para revisão desta academia.</p>
...
function buildExplicitLayoverDayPerDiemSegments(roster: CrewRoster): PerDiemOperationalSegment[] {
 const base = normalizedAirportCode(roster.base || 'BSB') || 'BSB';
 const out: PerDiemOperationalSegment[] = [];
 for (const day of roster.days || []) {
  if (String(day.type || '').toUpperCase() !== 'LAYOVER') continue;
  const away = normalizedAirportCode(day.hotel || day.pairingCode || '');
  if (!away) continue;
  ...
  const suffix = directed ? ' Pernoite dirigido habilitado pelo usuário: gerar almoço/jantar se cruzar janela, mantendo revisão manual.' : '';
  const reason = internationalRule
   ? `Pernoite publicado fora do Brasil em ${away}. Café e ceia não são gerados por pernoite puro; almoço/jantar seguem janelas de diárias internacionais.${suffix}`
   : `Pernoite publicado fora da base em ${away}. Gerar almoço/jantar quando o pernoite cobre as janelas; café e ceia não são gerados por pernoite puro.${suffix}`;
  out.push(...splitAbsoluteSegment(startAbs, endAbs, `PERNOITE ${away}`, 'layover', directed ? 'estimada' : 'alta', reason, { ...meta, flightKey: perDiemFlightKeyForDay(day) || `PERNOITE ${away}` }));
 }
 return out;
}
...
function buildLayoverPerDiemSegments(roster: CrewRoster): PerDiemOperationalSegment[] {
 ...
 return [...out, ...buildExplicitLayoverDayPerDiemSegments(roster)];
}
...
function AdminDashboardScreen({ bundle, savedRosters, dbStatus, pendingOffline, onBack }: { bundle: SessionRosterBundle | null; savedRosters: SavedRosterSummary[]; dbStatus: DatabaseStatus | null; pendingOffline: number; onBack: () => void }) {
 ...
 const financeCards = [
  ...
  { label: 'Custos mensais', value: formatMoney(finance.fixedCosts || billingDashboard?.forecast?.fixedCosts || 0), hint: 'custos configurados no Render' },
   ...
   <div className="mx-auto max-w-7xl">
    ...
    <section className="mt-6 rounded-[2rem] border border-cyan-200/20 bg-gradient-to-br from-cyan-300/15 via-white/[0.06] to-blue-500/10 p-5 shadow-2xl shadow-black/25">
     <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
      <div>
       ...
       <p className="mt-2 max-w-3xl text-sm leading-6 text-slate-300">Painel agregado para acompanhar APIs, banco, e-mail, cobrança, assinaturas, inadimplência, receita prevista e lucro estimado. Sem expor segredos, tokens ou dados sensíveis.</p>
      </div>
    ...
    <section className="mt-6 grid gap-4 lg:grid-cols-3">
     ...
     <div className="rounded-[2rem] border border-cyan-300/20 bg-cyan-300/10 p-5">
      <h3 className="text-xl font-black">Saúde das APIs</h3>
    ...
    <section className="mt-6 overflow-hidden rounded-[2rem] border border-cyan-300/20 bg-white/[0.065] shadow-2xl shadow-cyan-950/20 backdrop-blur-2xl">
     ...
     <div className="grid gap-4 p-5 lg:grid-cols-[1.2fr_.8fr] sm:p-7">
      <div className="rounded-3xl border border-cyan-200/20 bg-cyan-300/10 p-5">
       ...
       <p className="mt-2 text-sm leading-6 text-slate-300">O backend envia para o contato interno configurado no Render, sem revelar o destinatário.</p>
