...
import { inferCrewRole, type CrewRoleSelection } from '@/lib/actRules';
...
type HomeView = 'home' | 'import' | 'history' | 'calendar' | 'billing' | 'iflight' | 'flightboard' | 'departure' | 'meteorology' | 'routine' | 'perdiem' | 'salary' | 'currency' | 'parking' | 'regulation' | 'c32f' | 'settings' | 'support' | 'notes' | 'chief' | 'medical' | 'bids' | 'admin' | 'systemstatus' | 'more';
...
function crewcheckIsPilotRoleFromText(value?: string | null): boolean {
 const text = String(value || '').normalize('NFD').replace(/[\u0300-\u036f]/g, '').toUpperCase();
 return /\b(PILOTO|COMANDANTE|COPILOTO|CAPTAIN|FIRST\s*OFFICER|FO|CP|CMTE|CMD|CMT)\b/.test(text) && !/COMISS|CABIN|CCM|CMS/.test(text);
}

function crewcheckIataToIcaoClient(value?: string | null): string {
 const raw = String(value || '').trim().toUpperCase().replace(/[^A-Z0-9]/g, '');
 const map: Record<string, string> = { BSB:'SBBR', GRU:'SBGR', CGH:'SBSP', VCP:'SBKP', GIG:'SBGL', SDU:'SBRJ', CNF:'SBCF', POA:'SBPA', CWB:'SBCT', FLN:'SBFL', GYN:'SBGO', SSA:'SBSV', REC:'SBRF', FOR:'SBFZ', NAT:'SBSG', JPA:'SBJP', MCZ:'SBMO', AJU:'SBAR', SLZ:'SBSL', THE:'SBTE', IOS:'SBIL', BPS:'SBPS', MAO:'SBEG', BEL:'SBBE', MAB:'SBMA', PMW:'SBPJ', PVH:'SBPV', RBR:'SBRB', BVB:'SBBV', MCP:'SBMQ', MIA:'KMIA', JFK:'KJFK', MCO:'KMCO', EZE:'SAEZ', AEP:'SABE', SCL:'SCEL', LIM:'SPJC', LIS:'LPPT', MAD:'LEMD', CDG:'LFPG', FCO:'LIRF', LHR:'EGLL', FRA:'EDDF' };
 if (raw.length === 4) return raw;
 if (raw.length >= 3 && map[raw.slice(0,3)]) return map[raw.slice(0,3)];
     ...
     <section className="rounded-[2rem] border border-white bg-white p-5 shadow-[0_18px_60px_rgba(20,54,84,0.08)]">
      ...
      <div className="grid gap-3 md:grid-cols-2">
       ...
       <RegulationField label="Etapas">
        <div className="grid grid-cols-5 gap-2">{[1,2,3,4,5].map((n) => <button key={n} type="button" onClick={() => setStages(n)} className={`rounded-2xl px-3 py-3 text-sm font-black transition ${stages === n ? 'bg-[#092846] text-white shadow-lg shadow-sky-950/15' : 'bg-[#eef5fb] text-[#425a72]'}`}>{n}</button>)}</div>
       </RegulationField>
...
type MeteorologyDisplayMode = 'translated' | 'raw' | 'both';

type MeteorologyBrief = { ok?: boolean; origin?: string; destination?: string; metars?: Record<string, { raw?: string }>; tafs?: Record<string, { raw?: string }>; translated?: Record<string, { metar?: string[]; taf?: string[] }>; atis?: string; updatedAt?: string; message?: string };

function WeatherCodePill({ label, value }: { label: string; value: string }) {
 return <div className="rounded-2xl border border-sky-100 bg-sky-50 px-4 py-3"><p className="text-[10px] font-black uppercase tracking-[0.18em] text-sky-700">{label}</p><p className="mt-1 break-words text-sm font-black text-[#092846]">{value || '—'}</p></div>;
}

function MeteorologyReportBlock({ title, station, rawMetar, rawTaf, translatedMetar, translatedTaf, mode }: { title: string; station: string; rawMetar?: string; rawTaf?: string; translatedMetar?: string[]; translatedTaf?: string[]; mode: MeteorologyDisplayMode }) {
 return (
  <section className="rounded-[2rem] border border-white bg-white p-5 shadow-[0_18px_60px_rgba(20,54,84,0.08)]">
   <div className="flex flex-wrap items-start justify-between gap-3"><div><p className="text-[11px] font-black uppercase tracking-[0.22em] text-sky-600">{title}</p><h3 className="mt-1 text-2xl font-black text-[#092846]">{station || '—'}</h3></div><span className="rounded-full bg-[#092846] px-3 py-1 text-[10px] font-black uppercase tracking-[0.14em] text-white">METAR / TAF</span></div>
   {(mode === 'raw' || mode === 'both') && <div className="mt-4 grid gap-3"><WeatherCodePill label="METAR / SPECI" value={rawMetar || 'Sem METAR recente no provedor.'} /><WeatherCodePill label="TAF" value={rawTaf || 'TAF não disponível para a validade consultada.'} /></div>}
   {(mode === 'translated' || mode === 'both') && <div className="mt-4 grid gap-3"><div className="rounded-2xl border border-emerald-100 bg-emerald-50 p-4"><p className="text-xs font-black uppercase tracking-[0.14em] text-emerald-800">Tradução METAR</p><ul className="mt-2 space-y-1 text-sm font-semibold leading-6 text-emerald-950">{(translatedMetar?.length ? translatedMetar : ['Sem tradução disponível.']).map((line, i) => <li key={i}>• {line}</li>)}</ul></div><div className="rounded-2xl border border-indigo-100 bg-indigo-50 p-4"><p className="text-xs font-black uppercase tracking-[0.14em] text-indigo-800">Tradução TAF</p><ul className="mt-2 space-y-1 text-sm font-semibold leading-6 text-indigo-950">{(translatedTaf?.length ? translatedTaf : ['Sem TAF traduzido disponível.']).map((line, i) => <li key={i}>• {line}</li>)}</ul></div></div>}
  </section>
 );
}

function MeteorologyPanel({ onBack, bundle, isPilot }: { onBack: () => void; bundle: SessionRosterBundle | null; isPilot: boolean }) {
 const [origin, setOrigin] = useState(() => localStorage.getItem('crewcheck_met_origin') || 'SBJP');
 const [destination, setDestination] = useState(() => localStorage.getItem('crewcheck_met_destination') || 'SBBR');
 const [mode, setMode] = useState<MeteorologyDisplayMode>(() => (localStorage.getItem('crewcheck_met_display_mode') as MeteorologyDisplayMode) || 'both');
 const [favorites, setFavorites] = useState<string[]>(() => { try { return JSON.parse(localStorage.getItem('crewcheck_met_favorites') || '[]'); } catch { return []; } });
 const [brief, setBrief] = useState<MeteorologyBrief | null>(null);
 ...
 const nextFlight = useMemo(() => {
  ...
  for (const day of bundle?.roster?.days || []) {
   ...
   const m = dateText.match(/(\d{1,2})\/(\d{1,2})\/(20\d{2})/);
   if (!m) continue;
   const [hh, mm] = String(leg.departureTime || day.dutyReport || '00:00').split(':').map(Number);
   const dt = new Date(Number(m[3]), Number(m[2]) - 1, Number(m[1]), Number.isFinite(hh) ? hh : 0, Number.isFinite(mm) ? mm : 0);
   if (dt.getTime() >= now.getTime() - 6 * 60 * 60 * 1000) return { day, leg, date: dt };
  }
  return null;
 }, [bundle?.roster?.days?.length]);
 const persistFavorites = (items: string[]) => { const unique = Array.from(new Set(items.map(crewcheckIataToIcaoClient).filter(Boolean))).slice(0, 18); setFavorites(unique); localStorage.setItem('crewcheck_met_favorites', JSON.stringify(unique)); };
 const load = useCallback(async (o = origin, d = destination) => {
  const stationOrigin = crewcheckIataToIcaoClient(o);
  const stationDestination = crewcheckIataToIcaoClient(d);
  localStorage.setItem('crewcheck_met_origin', stationOrigin);
  localStorage.setItem('crewcheck_met_destination', stationDestination);
  setLoading(true); setError('');
  try {
   const response = await fetch(`/api/meteorology/brief?origin=${encodeURIComponent(stationOrigin)}&destination=${encodeURIComponent(stationDestination)}`, { headers: { ...crewcheckAuthHeader() } });
   const data = await response.json().catch(() => null);
   if (!response.ok || !data?.ok) throw new Error(data?.message || 'Meteorologia indisponível.');
   setBrief(data);
  } catch (e) { setError(e instanceof Error ? e.message : 'Falha ao consultar meteorologia.'); }
  finally { setLoading(false); }
 }, [origin, destination]);
 useEffect(() => { void load(); const timer = window.setInterval(() => void load(), 10 * 60_000); return () => window.clearInterval(timer); }, []);
 const useNext = () => {
  if (!nextFlight?.leg) { toast.info('Nenhum voo futuro localizado na escala ativa.'); return; }
  const o = crewcheckIataToIcaoClient(nextFlight.leg.origin);
  const d = crewcheckIataToIcaoClient(nextFlight.leg.destination);
  setOrigin(o); setDestination(d); void load(o, d);
 };
 const saveFav = (icao: string) => { const v = crewcheckIataToIcaoClient(icao); if (v) persistFavorites([v, ...favorites]); };
 const originId = brief?.origin || crewcheckIataToIcaoClient(origin);
 const destId = brief?.destination || crewcheckIataToIcaoClient(destination);
 return (
  <div className="min-h-screen bg-[#08111f] px-3 py-5 pb-28 text-white sm:px-6">
   <div className="mx-auto max-w-6xl space-y-4">
    <section className="rounded-[2rem] border border-cyan-200/15 bg-[linear-gradient(135deg,rgba(14,165,233,.22),rgba(15,23,42,.92))] p-5 shadow-2xl shadow-cyan-950/30">
     <button onClick={onBack} className="mb-4 inline-flex items-center gap-2 rounded-2xl border border-white/10 bg-white/10 px-4 py-2 text-sm font-black"><ArrowLeft className="h-4 w-4" /> Voltar</button>
     <div className="grid gap-5 lg:grid-cols-[minmax(0,1fr)_22rem]"><div><p className="text-xs font-black uppercase tracking-[0.24em] text-cyan-100/70">Meteorologia CrewCheck</p><h1 className="mt-2 text-4xl font-black tracking-tight">METAR, SPECI e TAF</h1><p className="mt-2 max-w-3xl text-sm font-semibold leading-6 text-slate-300">Visual estilo AEROMET, favoritos, tradução operacional e modo ATIS para o Telegram. {isPilot ? 'Perfil piloto: cards avançados habilitados.' : 'Perfil cabine: foco em planejamento de mala, chuva, vento e temperatura do pernoite.'}</p></div><div className="rounded-3xl border border-white/10 bg-white/10 p-4"><p className="text-xs font-black uppercase tracking-[0.18em] text-cyan-100/70">Próxima programação</p><p className="mt-2 text-lg font-black">{nextFlight?.leg ? `${nextFlight.leg.origin} → ${nextFlight.leg.destination}` : 'Nenhum voo futuro'}</p><button onClick={useNext} className="mt-3 rounded-2xl bg-cyan-300 px-4 py-3 text-sm font-black text-[#06101d]">Usar origem/destino da escala</button></div></div>
    </section>
    <section className="rounded-[2rem] border border-white/10 bg-white/[0.055] p-4 shadow-2xl shadow-black/20">
     <div className="grid gap-3 md:grid-cols-[1fr_1fr_auto]"><input value={origin} onChange={(e) => setOrigin(e.target.value.toUpperCase())} placeholder="Origem: SBJP ou JPA" className="h-13 rounded-2xl border border-white/10 bg-[#030914] px-4 text-sm font-black text-white outline-none focus:border-cyan-300" /><input value={destination} onChange={(e) => setDestination(e.target.value.toUpperCase())} placeholder="Destino: SBBR ou BSB" className="h-13 rounded-2xl border border-white/10 bg-[#030914] px-4 text-sm font-black text-white outline-none focus:border-cyan-300" /><button onClick={() => void load()} className="rounded-2xl bg-cyan-300 px-5 py-3 text-sm font-black text-[#06101d]"><RefreshCw className={`mr-2 inline h-4 w-4 ${loading ? 'animate-spin' : ''}`} />Consultar</button></div>
     <div className="mt-3 flex flex-wrap gap-2"><select value={mode} onChange={(e) => { const v = e.target.value as MeteorologyDisplayMode; setMode(v); localStorage.setItem('crewcheck_met_display_mode', v); }} className="rounded-2xl border border-white/10 bg-[#030914] px-3 py-2 text-sm font-black text-white"><option value="both">Raw + traduzido</option><option value="translated">Somente traduzido</option><option value="raw">Somente código original</option></select><button onClick={() => saveFav(origin)} className="rounded-2xl border border-white/10 bg-white/10 px-3 py-2 text-sm font-black">Salvar origem</button><button onClick={() => saveFav(destination)} className="rounded-2xl border border-white/10 bg-white/10 px-3 py-2 text-sm font-black">Salvar destino</button>{favorites.map((fav) => <button key={fav} onClick={() => { setOrigin(fav); void load(fav, destination); }} className="rounded-2xl border border-cyan-300/20 bg-cyan-300/10 px-3 py-2 text-sm font-black text-cyan-50">{fav}</button>)}</div>
     {error && <p className="mt-3 rounded-2xl border border-amber-300/20 bg-amber-300/10 p-3 text-sm font-bold text-amber-50">{error}</p>}
    </section>
    {brief && <div className="grid gap-4 lg:grid-cols-2"><MeteorologyReportBlock title="Origem" station={originId} rawMetar={brief.metars?.[originId]?.raw} rawTaf={brief.tafs?.[originId]?.raw} translatedMetar={brief.translated?.[originId]?.metar} translatedTaf={brief.translated?.[originId]?.taf} mode={mode} /><MeteorologyReportBlock title="Destino" station={destId} rawMetar={brief.metars?.[destId]?.raw} rawTaf={brief.tafs?.[destId]?.raw} translatedMetar={brief.translated?.[destId]?.metar} translatedTaf={brief.translated?.[destId]?.taf} mode={mode} /></div>}
    {brief?.atis && <section className="rounded-[2rem] border border-emerald-200/15 bg-emerald-300/10 p-5"><p className="text-xs font-black uppercase tracking-[0.22em] text-emerald-100/70">ATIS CrewCheck · áudio Telegram</p><p className="mt-2 text-sm font-semibold leading-7 text-emerald-50">{brief.atis}</p></section>}
    <section className="rounded-[2rem] border border-white/10 bg-white/[0.045] p-5"><p className="text-xs font-black uppercase tracking-[0.22em] text-cyan-100/65">Glossário rápido</p><div className="mt-3 grid gap-2 sm:grid-cols-2 lg:grid-cols-3">{[['METAR','observação atual'],['SPECI','observação especial'],['TAF','previsão do aeródromo'],['CAVOK','condição favorável'],['BKN','nublado parcial'],['OVC','encoberto'],['RA','chuva'],['TS','trovoada'],['TEMPO','mudança temporária'],['BECMG','mudança gradual'],['QNH','pressão altimétrica'],['G','rajada']].map(([a,b]) => <div key={a} className="rounded-2xl border border-white/10 bg-white/10 p-3"><b>{a}</b><p className="text-xs text-slate-300">{b}</p></div>)}</div></section>
   </div>
  </div>
 );
}
...
function Home() {
 ...
 const inferredRole = useMemo(() => {
  if (roleSelection === 'pilot' || roleSelection === 'cabin') return roleSelection;
  if (currentRosterBundle?.roster) return inferCrewRole(currentRosterBundle.roster, roleSelection).role;
  return crewcheckIsPilotRoleFromText(profileSettings.rank) ? 'pilot' : 'cabin';
 }, [roleSelection, currentRosterBundle?.roster?.rank, currentRosterBundle?.roster?.rawText, profileSettings.rank]);
 const isPilotProfile = inferredRole === 'pilot';
 ...
 useEffect(() => {
  if (currentView === 'history') void refreshSavedRosters();
 }, [currentView, refreshSavedRosters]);
 ...
 useEffect(() => {
  try {
   ...
   if (target && ['home','import','history','calendar','billing','iflight','flightboard','departure','meteorology','routine','perdiem','salary','currency','parking','c32f','settings','support','notes','more'].includes(target)) {
 ...
 const handleDashboardNavigate = useCallback((view: string) => {
  const premiumViews = new Set(['flightboard','departure','smartDeparture','departureExtra','meteorology','perdiem','salary','currency','parking','chief','medical','bids','reports','alerts']);
  ...
  const map: Record<string, () => void> = {
   ...
   meteorology: () => setCurrentView('meteorology'),
   metar: () => setCurrentView('meteorology'),
   taf: () => setCurrentView('meteorology'),
 ...
 const renderWithMobileMenu = (content: ReactNode, active: CrewCheckBottomActive = 'home') => (
  <div className={`crewcheck-side-layout ${premiumDrawerOpen ? 'is-menu-open' : ''}`}>
   ...
   <CrewCheckMobileMenu active={active} onNavigate={(view) => { setPremiumDrawerOpen(false); handleDashboardNavigate(view); }} canAccessAdmin={canAccessC32FAcademy} isPilot={isPilotProfile} drawerOpen={premiumDrawerOpen} onClose={() => setPremiumDrawerOpen(false)} />
 ...
 if (currentView === 'meteorology') return renderWithMobileMenu(<MeteorologyPanel onBack={() => setCurrentView('home')} bundle={currentRosterBundle} isPilot={isPilotProfile} />, 'flightboard');
...
if (currentView === 'regulation') return renderWithMobileMenu(<RegulationCalculatorPanel onBack={() => setCurrentView('home')} onOpenScaleAlerts={() => goToResults('irregularities')} />, 'roster');
 ...
 if (currentView === 'more') {
  return renderWithMobileMenu(<MorePanel onBack={() => setCurrentView('home')} onNavigate={handleDashboardNavigate} onLogout={handleLogout} pendingOffline={pendingOffline} hasRoster={hasCurrentRosterSession()} canAccessC32FAcademy={canAccessC32FAcademy} isPilot={isPilotProfile} />, 'more');
 }
...
function CrewCheckMobileMenu({ active, onNavigate, canAccessAdmin = false, isPilot = false, drawerOpen = false, onClose }: { active: CrewCheckBottomActive; onNavigate: (view: string) => void; canAccessAdmin?: boolean; isPilot?: boolean; drawerOpen?: boolean; onClose?: () => void }) {
 ...
 const groups: Array<{ key: string; label: string; icon: LucideIcon; items: Array<{ label: string; icon: LucideIcon; view: string; activeId?: CrewCheckBottomActive; admin?: boolean; hint?: string; cabinOnly?: boolean; pilotOnly?: boolean }> }> = [
  { key: 'operacional', label: 'Operacional', icon: Plane, items: [
   ...
   { label: 'Meteorologia', icon: Sun, view: 'meteorology', activeId: 'flightboard', hint: 'METAR, SPECI e TAF' },
  ]},
  { key: 'financeiro', label: 'Financeiro', icon: TrendingUp, items: [
   { label: 'Diárias', icon: FileText, view: 'perdiem', activeId: 'more', hint: 'almoço/jantar/ceia', cabinOnly: true },
   { label: 'Salário', icon: DollarSign, view: 'salary', activeId: 'more', hint: 'prévia variável', cabinOnly: true },
  ...
  <aside className={`crewcheck-premium-sidebar ${showExpanded ? 'is-expanded' : 'is-collapsed'} ${drawerOpen ? 'is-mobile-open' : ''}`} aria-label="Menu principal CrewCheck">
   ...
   <nav className="crewcheck-sidebar-nav">
    {groups.map((group) => {
     ...
     const visibleItems = group.items.filter((item) => (!item.admin || canAccessAdmin) && (!item.cabinOnly || !isPilot) && (!item.pilotOnly || isPilot));
   ...
   {open && (
    <div className="crewcheck-speed-dial-panel mb-3 w-[min(21rem,calc(100vw-2rem))] rounded-[1.55rem] border p-3 shadow-2xl backdrop-blur-2xl">
     <div className="mb-2 flex items-center justify-between px-2">
      ...
      <span className="rounded-full px-2 py-1 text-[9px] font-black uppercase tracking-[0.12em]">v11.0.94</span>
     </div>
...
function MorePanel({ onBack, onNavigate, onLogout, pendingOffline, hasRoster, canAccessC32FAcademy, isPilot = false }: { onBack: () => void; onNavigate: (view: string) => void; onLogout: () => void; pendingOffline: number; hasRoster: boolean; canAccessC32FAcademy: boolean; isPilot?: boolean }) {
 return (
  <div className="min-h-screen bg-[#08111f] px-5 py-8 text-white">
   <div className="mx-auto max-w-4xl">
    <HeaderBack title={isPilot ? "Módulos do piloto" : "Módulos de cabine"} subtitle={isPilot ? "Meteorologia, radar e operação" : "Relatórios, cabine, diárias e apoio operacional"} onBack={onBack} />
    <div className="mt-6 grid gap-3 sm:grid-cols-2">
     ...
     <MenuAction icon={Sun} title="Meteorologia" text="METAR, SPECI, TAF, favoritos e briefing estilo ATIS" onClick={() => onNavigate('meteorology')} />
     ...
     {!isPilot && <MenuAction icon={ClipboardList} title="Chefe de Cabine IA" text="Caderno de bordo, água, horários, check-in e relatório" onClick={() => onNavigate('chief')} />}
     {!isPilot && <MenuAction icon={FileText} title="Checklist Médico" text="Formulário de ocorrência, PDF e compartilhamento" onClick={() => onNavigate('chief')} />}
     ...
     {!isPilot && <MenuAction icon={FileText} title="Previsão de Diárias" text="Café, almoço, jantar, ceia e total estimado pela escala" onClick={() => onNavigate('perdiem')} />}
     ...
     {!isPilot && <MenuAction icon={TrendingUp} title="Previsão de Salário" text="Bruto, líquido, descontos e holerite opcional" onClick={() => onNavigate('salary')} />}
