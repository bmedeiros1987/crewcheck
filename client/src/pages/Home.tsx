...
import { crewRoleSelectionFromFunction, resolveCrewFunction, type CrewRoleSelection } from '@/lib/actRules';
...
import { getDatabaseStatus, listSavedRosters, openSavedRoster, openActiveRoster, saveRosterAnalysis, activateRosterAnalysis, deleteRosterAnalysis, type DatabaseStatus, type SavedRosterSummary } from '@/lib/databaseClient';
...
import {
 ...
 googleCalendarIntegrationDiagnostics,
...
type HomeView = 'home' | 'import' | 'history' | 'calendar' | 'billing' | 'iflight' | 'flightboard' | 'departure' | 'wakeup' | 'routine' | 'perdiem' | 'salary' | 'currency' | 'parking' | 'weather' | 'regulation' | 'c32f' | 'settings' | 'support' | 'notes' | 'chief' | 'medical' | 'bids' | 'admin' | 'systemstatus' | 'more';
...
type TelegramStatus = { ok?: boolean; configured?: boolean; connected?: boolean; botUsername?: string | null; code?: string | null; connectUrl?: string | null; expectedUsername?: string | null; message?: string; chat?: { username?: string | null; maskedId?: string | null; firstName?: string | null } | null; preferences?: Record<string, boolean | string>; callmebot?: { configured?: boolean; enabled?: boolean; mode?: string; effectiveMode?: string; targetMasked?: string; phoneMasked?: string; telegramMasked?: string; apiKeyConfigured?: boolean; apiKeyMasked?: string; globalFallback?: boolean; brand?: string; authorizationRequired?: boolean; authorizationAcknowledged?: boolean; authorizeUrl?: string; disclosure?: string; premiumRequired?: boolean; message?: string }; infobip?: { configured?: boolean; enabled?: boolean; targetMasked?: string; fromMasked?: string; missing?: string[]; baseConfigured?: boolean; apiKeyConfigured?: boolean; fromConfigured?: boolean; targetConfigured?: boolean; language?: string; voice?: string; message?: string; premiumRequired?: boolean }; twilio?: { configured?: boolean; enabled?: boolean; targetMasked?: string; fromMasked?: string; missing?: string[]; accountSidConfigured?: boolean; authTokenConfigured?: boolean; fromConfigured?: boolean; targetConfigured?: boolean; elevenLabsAudio?: boolean; ttsFallback?: boolean; message?: string; premiumRequired?: boolean }; wakeupPolicy?: { plan?: string; telegramForFree?: boolean; callPremiumOnly?: boolean; callAllowed?: boolean; premiumRequired?: boolean; message?: string }; syncedRoster?: { available?: boolean; updatedAt?: string | null; periodYear?: number | null; periodMonth?: number | null } };
type WhatsAppConciergeStatus = { ok?: boolean; configured?: boolean; apiConfigured?: boolean; premiumOnly?: boolean; allowed?: boolean; paidPremium?: boolean; trialing?: boolean; premiumRequired?: boolean; numberMasked?: string; directLink?: string; canSendRosterByApi?: boolean; canShareRosterByLink?: boolean; canImportRosterByWhatsApp?: boolean; importEnabled?: boolean; webhookUrl?: string; message?: string; trial?: { trialing?: boolean; dailyUsed?: number; totalUsed?: number; dailyLimit?: number; totalLimit?: number; remainingDaily?: number | null; remainingTotal?: number | null; exceeded?: boolean } };
...
const CREWCHECK_ADMIN_EMAILS = [
 C32F_ADMIN_EMAIL,
 normalizeEmail(import.meta.env.VITE_CREWCHECK_OWNER_EMAIL || ''),
 ...String(import.meta.env.VITE_CREWCHECK_ADMIN_EMAILS || '').split(/[;,\s]+/).map(normalizeEmail),
...
function isC32FAcademyAdmin(user: ReturnType<typeof getStoredUser>): boolean {
 const role = String((user as any)?.role || localStorage.getItem('crewcheck_role') || '').toLowerCase();
 const email = normalizeEmail(user?.email);
 const adminByEmail = Boolean(email && CREWCHECK_ADMIN_EMAILS.includes(email));
 const admin = role.includes('admin') || adminByEmail;
 if (admin) {
  try {
   localStorage.setItem('crewcheck_role', 'admin');
   localStorage.setItem('crewcheck_admin_mode', '1');
  } catch {}
 }
 return admin;
}
...
const CREWCHECK_COUNTRY_PHONE_OPTIONS = [
...
type CrewCountryPhoneOption = typeof CREWCHECK_COUNTRY_PHONE_OPTIONS[number];

function crewCountryByIso(iso?: string | null): CrewCountryPhoneOption {
 const clean = String(iso || '').toUpperCase();
 return CREWCHECK_COUNTRY_PHONE_OPTIONS.find((item) => item.iso === clean) || CREWCHECK_COUNTRY_PHONE_OPTIONS[0];
}

function normalizeCrewPhoneDigits(value?: string | null): string {
 return String(value || '').replace(/\D+/g, '').slice(0, 18);
}

function normalizeCrewCountryCode(value?: string | null): string {
 const digits = normalizeCrewPhoneDigits(value);
 return digits ? `+${digits.slice(0, 4)}` : '+55';
}

function composeCrewPhoneE164(countryCode?: string | null, local?: string | null): string {
 const code = normalizeCrewCountryCode(countryCode || '+55').replace(/\D/g, '');
 let digits = normalizeCrewPhoneDigits(local);
 if (digits.startsWith(code)) digits = digits.slice(code.length);
 return digits ? `+${code}${digits}` : '';
}

function splitCrewPhoneForProfile(e164?: string | null, iso?: string | null, code?: string | null): { phoneCountryIso: string; phoneCountryCode: string; phoneLocal: string; phoneE164: string } {
 const country = crewCountryByIso(iso || 'BR');
 const finalCode = normalizeCrewCountryCode(code || country.code);
 const codeDigits = finalCode.replace(/\D/g, '');
 const digits = normalizeCrewPhoneDigits(e164 || '');
 const local = digits.startsWith(codeDigits) ? digits.slice(codeDigits.length) : digits;
 return { phoneCountryIso: country.iso, phoneCountryCode: finalCode, phoneLocal: local, phoneE164: composeCrewPhoneE164(finalCode, local) };
}
...
function loadLocalProfileSettings(): LocalProfileSettings {
 try {
  return {
   ...
   phoneCountryIso: localStorage.getItem('crewcheck_profile_phone_country_iso') || 'BR',
   phoneCountryCode: localStorage.getItem('crewcheck_profile_phone_country_code') || '+55',
   phoneLocal: localStorage.getItem('crewcheck_profile_phone_local') || '',
   phoneE164: localStorage.getItem('crewcheck_profile_phone_e164') || '',
   timeMode: readCrewTimeMode(),
   customTimeZone: readCrewCustomTimeZone(),
  };
 } catch {
  return { displayName: '', company: '', base: '', rank: '', isInstructor: false, hasVirtualBase: false, virtualBase: '', phoneCountryIso: 'BR', phoneCountryCode: '+55', phoneLocal: '', phoneE164: '', timeMode: 'BSB', customTimeZone: CREWCHECK_DEFAULT_TIMEZONE };
 }
}
...
function saveLocalProfileSettings(settings: LocalProfileSettings): LocalProfileSettings {
 const cleaned = {
  ...
  phoneCountryIso: crewCountryByIso(settings.phoneCountryIso || 'BR').iso,
  phoneCountryCode: normalizeCrewCountryCode(settings.phoneCountryCode || crewCountryByIso(settings.phoneCountryIso || 'BR').code),
  phoneLocal: normalizeCrewPhoneDigits(settings.phoneLocal || settings.phoneE164 || ''),
  phoneE164: composeCrewPhoneE164(settings.phoneCountryCode || crewCountryByIso(settings.phoneCountryIso || 'BR').code, settings.phoneLocal || settings.phoneE164 || ''),
 ...
 try {
  ...
  const resolvedRole = crewRoleSelectionFromFunction(cleaned.rank);
  localStorage.setItem('crewcheck_profile_role_selection', resolvedRole);
  if (resolvedRole !== 'auto') {
   localStorage.setItem('crewcheck_role_selection', resolvedRole);
   sessionStorage.setItem('crewcheck_role_selection', resolvedRole);
  }
  ...
  localStorage.setItem('crewcheck_profile_phone_country_iso', cleaned.phoneCountryIso);
  localStorage.setItem('crewcheck_profile_phone_country_code', cleaned.phoneCountryCode);
  localStorage.setItem('crewcheck_profile_phone_local', cleaned.phoneLocal);
  localStorage.setItem('crewcheck_profile_phone_e164', cleaned.phoneE164);
...
function normalizeCrewIdentityForProfile(value?: string | null): string {
 return String(value || '')
  .normalize('NFD')
  .replace(/[\u0300-\u036f]/g, '')
  .toUpperCase()
...
function isMeaningfulRosterCrewName(value?: string | null): boolean {
 const normalized = normalizeCrewIdentityForProfile(value);
 if (!normalized || normalized.length < 5) return false;
 if (/^(TRIPULANTE|CREW|CREWMEMBER|CREW MEMBER|PASSAGEIRO|USUARIO|USUARIO CREWCHECK|TRIPULANTE EXEMPLO)$/.test(normalized)) return false;
 return normalized.split(/\s+/).filter((token) => token.length > 1).length >= 1;
}

function rosterNameLooksDifferentForProfile(expected?: string | null, received?: string | null): boolean {
 const a = normalizeCrewIdentityForProfile(expected);
 const b = normalizeCrewIdentityForProfile(received);
...
function readRosterOwnerNameFromLocalProfile(profile?: LocalProfileSettings | null): string {
 const candidates = [
  profile?.displayName,
  (() => { try { return localStorage.getItem('crewcheck_profile_display_name') || ''; } catch { return ''; } })(),
  (() => { try { return localStorage.getItem('crewcheck_roster_owner_name') || ''; } catch { return ''; } })(),
  (() => { try { return localStorage.getItem('crewcheck_last_roster_crew_name') || ''; } catch { return ''; } })(),
 ];
 return String(candidates.find((item) => isMeaningfulRosterCrewName(item)) || '').trim();
}

function lockRosterOwnerName(name: string): void {
 try {
  localStorage.setItem('crewcheck_roster_owner_name', name);
  localStorage.setItem('crewcheck_last_roster_crew_name', name);
  localStorage.setItem('crewcheck_identity_lock_enabled', '1');
  localStorage.setItem('crewcheck_identity_lock_updated_at', new Date().toISOString());
 } catch {
  // iOS modo privado pode bloquear storage; o backend ainda valida quando online.
 }
}

function secureRosterIdentityForImport(inputRoster: CrewRoster, profile: LocalProfileSettings, sourceFileName: string, options?: { demo?: boolean }): { roster: CrewRoster; profile?: LocalProfileSettings; message?: string } {
 const roster = { ...inputRoster } as CrewRoster;
 if (options?.demo) return { roster };
 const rosterCrewName = String((roster as any).crewName || '').trim();
 const storedName = readRosterOwnerNameFromLocalProfile(profile);
 if (isMeaningfulRosterCrewName(rosterCrewName) && storedName && rosterNameLooksDifferentForProfile(storedName, rosterCrewName)) {
  try {
   localStorage.setItem('crewcheck_identity_last_blocked_at', new Date().toISOString());
   localStorage.setItem('crewcheck_identity_last_blocked_source', sourceFileName || 'Escala');
   localStorage.setItem('crewcheck_identity_last_blocked_expected', storedName);
   localStorage.setItem('crewcheck_identity_last_blocked_received', rosterCrewName);
  } catch {}
  throw new Error(`Esta escala parece ser de outro tripulante (${rosterCrewName}). A conta está vinculada a ${storedName}. Por segurança antifraude, o CrewCheck não vai subir esta escala. Se for falso positivo, ajuste o perfil ou fale com o suporte.`);
 }
 if (isMeaningfulRosterCrewName(rosterCrewName) && !storedName) {
  const nextProfile = saveLocalProfileSettings({
   ...profile,
   displayName: rosterCrewName,
   base: profile.base || String((roster as any).base || '').toUpperCase(),
   rank: profile.rank || String((roster as any).rank || ''),
  });
  lockRosterOwnerName(rosterCrewName);
  return { roster, profile: nextProfile, message: 'Nome do tripulante salvo como padrão do perfil.' };
 }
 if (!isMeaningfulRosterCrewName(rosterCrewName) && storedName) {
  (roster as any).crewName = storedName;
  lockRosterOwnerName(storedName);
  return { roster, message: 'Escala sem nome identificado: mantive o nome salvo no perfil.' };
 }
 if (isMeaningfulRosterCrewName(rosterCrewName)) lockRosterOwnerName(rosterCrewName);
 return { roster };
}
...
function readCurrentRosterBundle(): SessionRosterBundle | null {
 try {
  ...
  // segue para o fallback persistente
 }
 try {
  const payload = readPersistedRosterPayload();
  if (!payload?.roster?.days?.length) return null;
  const role = (payload.roleSelection || sessionStorage.getItem('crewcheck_role_selection') || 'auto') as CrewRoleSelection;
  const roster = normalizeRosterSchedule(detectAndMarkLayovers(payload.roster as CrewRoster));
  const compliance = (payload.compliance || analyzeCompliance(roster, role)) as ComplianceResult;
  const gym = Array.isArray(payload.gym) ? payload.gym as GymRecommendation[] : getGymRecommendations(roster, role);
  ...
  sessionStorage.setItem('crewcheck_role_selection', role);
  sessionStorage.setItem('crewcheck_source_file', payload.sourceFileName || 'Última escala salva neste dispositivo');
  return { roster, compliance, gym };
 } catch {
  return null;
 }
}

function buildFreshRosterBundle(rosterInput: CrewRoster, roleSelection: CrewRoleSelection) {
 const roster = normalizeRosterSchedule(detectAndMarkLayovers(rosterInput));
 const profileRole = crewRoleSelectionFromFunction(loadLocalProfileSettings().rank);
 const effectiveRole = profileRole !== 'auto' ? profileRole : roleSelection;
 const compliance = analyzeCompliance(roster, effectiveRole);
 const gym = getGymRecommendations(roster, effectiveRole);
 return { roster, compliance, gym, roleSelection: effectiveRole };
}

function persistFreshRosterBundle(rosterInput: CrewRoster, roleSelection: CrewRoleSelection, sourceFileName: string) {
 const bundle = buildFreshRosterBundle(rosterInput, roleSelection);
 sessionStorage.setItem('crewcheck_roster', JSON.stringify(bundle.roster));
 sessionStorage.setItem('crewcheck_compliance', JSON.stringify(bundle.compliance));
 sessionStorage.setItem('crewcheck_gym', JSON.stringify(bundle.gym || []));
 sessionStorage.setItem('crewcheck_role_selection', bundle.roleSelection || roleSelection);
 sessionStorage.setItem('crewcheck_source_file', sourceFileName);
 persistRosterPayloadLocally({ ...bundle, sourceFileName, roleSelection: bundle.roleSelection || roleSelection, updatedAt: new Date().toISOString(), source: 'crewcheck-ios-persistent-fallback-v11108', userScope: crewcheckCurrentUserScope() });
 return bundle;
}
...
function rosterChangeLabelV11113(day: CrewRoster['days'][number]): string {
 const code = String(day.pairingCode || day.type || '').toUpperCase();
 const legs = (day.legs || []).map((leg) => `${leg.flightNumber} ${leg.origin}-${leg.destination} ${leg.departureTime}-${leg.arrivalTime}`).join(' · ');
 const window = [day.dutyReport, day.dutyDebrief].filter(Boolean).join('–');
 return `${day.date} · ${code || day.type}${window ? ` ${window}` : ''}${legs ? ` · ${legs}` : ''}`;
}
...
function buildRosterChangeSummaryV10867(previous: CrewRoster | null | undefined, next: CrewRoster): RosterChangeSummaryV10867 | null {
 ...
 const oldMap = new Map<string, { value: string; day: CrewRoster['days'][number] }>();
 previous.days.forEach((day) => oldMap.set(rosterComparableKeyV10867(day), { value: rosterComparableValueV10867(day), day }));
 const newMap = new Map<string, { value: string; day: CrewRoster['days'][number] }>();
 let duplicatesIgnored = 0;
 next.days.forEach((day) => {
  const key = rosterComparableKeyV10867(day);
  const value = rosterComparableValueV10867(day);
  if (newMap.get(key)?.value === value) duplicatesIgnored += 1;
  newMap.set(key, { value, day });
 });
 ...
 for (const [key, entry] of newMap) {
  ...
  if (!oldEntry) {
   added += 1;
   details.push(`Novo: ${rosterChangeLabelV11113(entry.day)}`);
  } else if (oldEntry.value !== entry.value) {
   changed += 1;
   details.push(`Alterado: ${rosterChangeLabelV11113(entry.day)}`);
  }
 }
 for (const [key, entry] of oldMap) {
  if (!newMap.has(key)) {
   removed += 1;
   details.push(`Removido: ${rosterChangeLabelV11113(entry.day)}`);
  }
 }
 ...
 const message = added || removed || changed
  ? `Comparativo: ${added} nova(s), ${changed} alterada(s), ${removed} removida(s); duplicidades ignoradas: ${duplicatesIgnored}.`
  : `Comparativo: nenhuma alteração operacional detectada; duplicidades ignoradas: ${duplicatesIgnored}.`;
 return { samePeriod, added, removed, changed, duplicatesIgnored, message, details };
}
...
function readPersistedRosterPayload(): any | null {
 try {
  const scope = crewcheckCurrentUserScope();
  const keys = [
   crewcheckRosterSyncKey(),
  ...
  // storage pode estar bloqueado no iOS privado
 }
 return null;
}

function readLastRosterCrewName(): string {
 try {
  return String(localStorage.getItem('crewcheck_last_roster_crew_name') || '').trim();
 } catch {
  return '';
 }
}

function persistRosterPayloadLocally(payload: any): void {
 try {
  const scope = crewcheckCurrentUserScope();
  const serialized = JSON.stringify(payload);
  localStorage.setItem(crewcheckRosterSyncKey(), serialized);
  localStorage.setItem(`crewcheck_latest_roster_bundle_${scope}`, serialized);
  localStorage.setItem(`crewcheck_active_roster_snapshot_${scope}`, serialized);
  localStorage.setItem('crewcheck_last_roster_bundle_v11108', serialized);
  localStorage.setItem('crewcheck_last_roster_bundle_v11101', serialized);
  localStorage.setItem('crewcheck_last_roster_bundle_v11100', serialized);
  if (payload?.roster?.crewName) {
   const crewName = String(payload.roster.crewName).trim();
   localStorage.setItem('crewcheck_last_roster_crew_name', crewName);
   if (isMeaningfulRosterCrewName(crewName)) {
    const currentProfileName = localStorage.getItem('crewcheck_profile_display_name') || '';
    if (!currentProfileName || !rosterNameLooksDifferentForProfile(currentProfileName, crewName)) localStorage.setItem('crewcheck_profile_display_name', currentProfileName || crewName);
    localStorage.setItem('crewcheck_roster_owner_name', currentProfileName || crewName);
   }
  }
  if (payload?.roster?.base) localStorage.setItem('crewcheck_last_roster_base', String(payload.roster.base));
 } catch {
  // sessão segue viva mesmo se o iOS bloquear persistência local
 }
}

function resolveDashboardUserName(profile: LocalProfileSettings, bundle: SessionRosterBundle | null, saved: SavedRosterSummary[], user: ReturnType<typeof getStoredUser>): string {
 const candidates = [
  profile.displayName,
  bundle?.roster?.crewName,
  readPersistedRosterPayload()?.roster?.crewName,
  saved.find((item) => item.isActive)?.crewName,
  saved.find(isSavedRosterCurrentOrFuture)?.crewName,
  saved[0]?.crewName,
  readLastRosterCrewName(),
  user?.name,
  user?.email,
  'Tripulante',
 ];
 return String(candidates.find((item) => String(item || '').trim()) || 'Tripulante').trim();
}
...
type RadarPrivateStatus = {
 ...
 landingCountdown?: string | null;
 confidence?: string | null;
 updatedAt?: string | null;
 message?: string | null;
};

type CrewCheckOperationalAlert = {
 category?: string;
 level?: string;
 title?: string;
 body?: string;
...
function crewcheckOperationalAlertText(alert?: CrewCheckOperationalAlert | null): string {
 const body = String(alert?.body || '').trim();
 if (body) return body;
 const parts = [
  alert?.flightNumber ? `Voo ${alert.flightNumber}` : '',
  alert?.airport ? `Aeroporto ${alert.airport}` : '',
  alert?.origin && alert?.destination ? `${alert.origin} → ${alert.destination}` : '',
  alert?.level ? `Nível ${alert.level}` : '',
 ].filter(Boolean);
 return parts.length ? parts.join(' · ') : 'Há uma atualização operacional para conferir no CrewCheck.';
}
...
function radarStatusLine(status?: RadarPrivateStatus | null): string {
 const parts = [
  ...
  status?.registration ? `Matrícula: ${status.registration}` : '',
  status?.estimatedArrival ? `Pouso previsto: ${status.estimatedArrival}` : '',
  status?.landingCountdown ? `Pousa em: ${status.landingCountdown}` : '',
...
function hasUsefulRadarStatus(status?: RadarPrivateStatus | null): boolean {
 return Boolean(status?.status || status?.gate || status?.terminal || status?.registration || status?.estimatedArrival || status?.landingCountdown || status?.estimatedDeparture || status?.scheduledDeparture);
}
...
// v11.1.78: proteção global do Cockpit. A função estava sendo chamada pelo radar
// da tela inicial, mas não existia no bundle anterior, causando ReferenceError e
// travando a abertura do app. O radar volta a ser auxiliar: só consulta voos
// próximos e nunca bloqueia o dashboard.
function shouldFetchRadarForDashboardEvent(event?: DashboardNextEvent | null): boolean {
 try {
  if (!event?.isFlight || !event.flightNumber) return false;
  const flight = String(event.flightNumber || '').replace(/\s+/g, '').toUpperCase();
  if (!flight || !/^[A-Z]{1,3}\d{1,5}[A-Z]?$/.test(flight)) return false;

  const dateIso = radarDateFromNextEvent(event);
  if (shouldSkipDashboardFlightByRadar(flight, dateIso)) return false;
  if (event.isInProgress || event.isToday || event.isTomorrow) return true;

  const targetRaw = event.targetIso || (dateIso ? `${dateIso}T12:00:00` : '');
  const target = new Date(targetRaw);
  if (!Number.isFinite(target.getTime())) return true;
  const daysFromNow = (target.getTime() - Date.now()) / 86400000;
  return daysFromNow >= -1 && daysFromNow <= 4;
 } catch {
  return false;
 }
}
...
type RegulationMode = 'duty' | 'standby' | 'reserve';
type RegulationCrewType = 'simples' | 'composta' | 'revezamento';
type RegulationFlightKind = 'domestico' | 'internacional';

function regPad(value: number): string {
 return String(value).padStart(2, '0');
}

function regMinutesOfDay(value: string): number {
 const [hoursRaw, minutesRaw] = String(value || '00:00').split(':').map(Number);
 const hours = Number.isFinite(hoursRaw) ? hoursRaw : 0;
 const minutes = Number.isFinite(minutesRaw) ? minutesRaw : 0;
 return Math.max(0, Math.min(23, hours)) * 60 + Math.max(0, Math.min(59, minutes));
}

function regDurationMinutes(start: string, end: string): number {
 const startMin = regMinutesOfDay(start);
 const endMin = regMinutesOfDay(end);
 let diff = endMin - startMin;
 if (diff < 0) diff += 24 * 60;
 return diff;
}

function regAddMinutes(start: string, minutes: number): { label: string; dayOffset: number; totalMinutes: number } {
 const startMin = regMinutesOfDay(start);
 const total = startMin + Math.round(minutes);
 const dayOffset = Math.floor(total / (24 * 60));
 const normalized = ((total % (24 * 60)) + 24 * 60) % (24 * 60);
 return { label: `${regPad(Math.floor(normalized / 60))}:${regPad(normalized % 60)}${dayOffset > 0 ? ` +${dayOffset}` : ''}`, dayOffset, totalMinutes: total };
}

function regFormatDuration(minutes: number): string {
 const safe = Math.max(0, Math.round(minutes));
 const hours = Math.floor(safe / 60);
 const mins = safe % 60;
 if (!hours) return `${mins}min`;
 return mins ? `${hours}h${regPad(mins)}` : `${hours}h`;
}

const REG_RBAC117_B1_ROWS: Array<{ start: number; end: number; label: string; duty: [number, number, number, number, number]; flight: [number, number, number, number, number] }> = [
 ...
 { start: 18 * 60, end: 29 * 60 + 59, label: '18h00–05h59', duty: [9, 9, 9, 9, 9], flight: [8, 8, 7, 7, 7] },
];

function regSectorIndex(stages: number): 0 | 1 | 2 | 3 | 4 {
 if (stages <= 2) return 0;
 if (stages <= 4) return 1;
 if (stages === 5) return 2;
 if (stages === 6) return 3;
 return 4;
}

function regRbac117B1Limit(startTime: string, stages: number) {
 const start = regMinutesOfDay(startTime);
 const lookup = start < 6 * 60 ? start + 24 * 60 : start;
 const row = REG_RBAC117_B1_ROWS.find((item) => lookup >= item.start && lookup <= item.end) || REG_RBAC117_B1_ROWS[0];
 const idx = regSectorIndex(Math.max(1, stages || 1));
 return { startBucket: row.label, dutyHours: row.duty[idx], flightHours: row.flight[idx], source: 'RBAC 117 · Apêndice B · Tabela B.1' };
}

function regDefaultDutyLimitHours(crewType: RegulationCrewType, stages: number, flightKind: RegulationFlightKind, acclimated: boolean, extension: boolean, startTime = '08:00'): number {
 const b1 = regRbac117B1Limit(startTime, stages).dutyHours;
 let base = b1;
 if (crewType === 'composta') {
  base = flightKind === 'domestico' && stages >= 4 ? 12 : 14;
 } else if (crewType === 'revezamento') {
  base = 18;
 }
 if (!acclimated) base = Math.max(9, base - 1);
 if (extension) base += 1;
 return base;
}

function regDefaultFlightLimitHours(crewType: RegulationCrewType, stages: number, flightKind: RegulationFlightKind, startTime = '08:00'): number {
 if (crewType === 'simples') return regRbac117B1Limit(startTime, stages).flightHours;
 if (crewType === 'composta') return flightKind === 'domestico' && stages >= 4 ? 9.5 : 11;
 return 14;
}

function RegulationResultLine({ label, value, highlight }: { label: string; value: string; highlight?: boolean }) {
 return <div className={`flex items-center justify-between gap-3 rounded-2xl px-4 py-3 ${highlight ? 'bg-[#092846] text-white shadow-lg shadow-sky-950/15' : 'bg-[#eef5fb] text-[#092846]'}`}><span className="text-xs font-black uppercase tracking-[0.12em] opacity-70">{label}</span><b className="text-base">{value}</b></div>;
}

function RegulationSegmentButton<T extends string>({ value, current, onClick, children }: { value: T; current: T; onClick: (value: T) => void; children: ReactNode }) {
 return <button type="button" onClick={() => onClick(value)} className={`rounded-2xl px-4 py-3 text-sm font-black transition active:scale-[0.99] ${current === value ? 'bg-[#092846] text-white shadow-lg shadow-sky-950/15' : 'bg-[#eef5fb] text-[#425a72]'}`}>{children}</button>;
}

function RegulationField({ label, children }: { label: string; children: ReactNode }) {
 return <label className="space-y-1.5 text-xs font-black uppercase tracking-[0.12em] text-[#61778e]"><span>{label}</span>{children}</label>;
}

function RegulationCalculatorPanel({ onBack, onOpenScaleAlerts }: { onBack: () => void; onOpenScaleAlerts: () => void }) {
 const [mode, setMode] = useState<RegulationMode>('duty');
 const [crewType, setCrewType] = useState<RegulationCrewType>('simples');
 const [flightKind, setFlightKind] = useState<RegulationFlightKind>('domestico');
 const [acclimated, setAcclimated] = useState(true);
 const [stages, setStages] = useState(2);
 const [presentation, setPresentation] = useState('09:00');
 const [engineCutBuffer, setEngineCutBuffer] = useState(30);
 const [extension, setExtension] = useState(false);
 const [manualLimit, setManualLimit] = useState('');
 const [standbyStart, setStandbyStart] = useState('00:00');
 const [standbyPresentation, setStandbyPresentation] = useState('01:30');
 const [standbyCalled, setStandbyCalled] = useState(true);
 const [reserveStart, setReserveStart] = useState('06:00');
 const [reservePresentation, setReservePresentation] = useState('06:00');
 const [reserveCalled, setReserveCalled] = useState(false);
 const [combinedLimit, setCombinedLimit] = useState(12);
 const activeProfile = resolveCrewFunction(loadLocalProfileSettings().rank);
 const actName = activeProfile.role === 'pilot' ? 'ACT Aeronautas Pilotos 2025/2027' : activeProfile.role === 'cabin' ? 'ACT Aeronautas Comissários 2025/2027' : 'ACT detectada automaticamente pela escala/perfil';
 const rbacLimit = regRbac117B1Limit(presentation, stages);

 const baseDutyLimit = regDefaultDutyLimitHours(crewType, stages, flightKind, acclimated, extension, presentation);
 const dutyLimitHours = manualLimit.trim() ? Math.max(1, Number(manualLimit) || baseDutyLimit) : baseDutyLimit;
 const flightLimitHours = regDefaultFlightLimitHours(crewType, stages, flightKind, presentation);
 const dutyEnd = regAddMinutes(presentation, dutyLimitHours * 60);
 const dutyMotorCut = regAddMinutes(presentation, Math.max(0, dutyLimitHours * 60 - engineCutBuffer));
 const dutyEndNoExtension = regAddMinutes(presentation, regDefaultDutyLimitHours(crewType, stages, flightKind, acclimated, false, presentation) * 60);
 const dutyEndWithExtension = regAddMinutes(presentation, regDefaultDutyLimitHours(crewType, stages, flightKind, acclimated, true, presentation) * 60);

 const standbyElapsed = regDurationMinutes(standbyStart, standbyPresentation);
 const standbyMaxEndWithoutCall = regAddMinutes(standbyStart, 12 * 60);
 const standbyMinEndWithoutCall = regAddMinutes(standbyStart, 3 * 60);
 const standbyStartLimit = regDefaultDutyLimitHours(crewType, stages, flightKind, acclimated, extension, standbyStart);
 const standbyCombinedDeadline = regAddMinutes(standbyStart, standbyStartLimit * 60);
 const standbyNormalDutyDeadline = regAddMinutes(standbyPresentation, dutyLimitHours * 60);
 const standbyEndMinutes = Math.min(standbyCombinedDeadline.totalMinutes, standbyNormalDutyDeadline.totalMinutes);
 const standbyEnd = regAddMinutes('00:00', standbyEndMinutes);
 const standbyMotorCut = regAddMinutes('00:00', Math.max(0, standbyEndMinutes - engineCutBuffer));
 const standbyAvailable = Math.max(0, standbyEndMinutes - regMinutesOfDay(standbyPresentation) - (standbyPresentation < standbyStart ? 24 * 60 : 0));

 const reserveElapsed = regDurationMinutes(reserveStart, reservePresentation);
 const reserveMaxEndWithoutCall = regAddMinutes(reserveStart, 6 * 60);
 const reserveMinEndWithoutCall = regAddMinutes(reserveStart, 3 * 60);
 const reserveStartLimit = regDefaultDutyLimitHours(crewType, stages, flightKind, acclimated, extension, reserveStart);
 const reserveCombinedDeadline = regAddMinutes(reserveStart, reserveStartLimit * 60);
 const reserveNormalDutyDeadline = regAddMinutes(reservePresentation, dutyLimitHours * 60);
 const reserveEndMinutes = Math.min(reserveCombinedDeadline.totalMinutes, reserveNormalDutyDeadline.totalMinutes);
 const reserveEnd = regAddMinutes('00:00', reserveEndMinutes);
 const reserveMotorCut = regAddMinutes('00:00', Math.max(0, reserveEndMinutes - engineCutBuffer));
 const reserveAvailable = Math.max(0, reserveEndMinutes - regMinutesOfDay(reservePresentation) - (reservePresentation < reserveStart ? 24 * 60 : 0));

 const selectClass = 'w-full rounded-2xl border border-[#d7e4ef] bg-white px-3 py-3 text-sm font-black text-[#092846] outline-none focus:border-sky-400';
 const inputClass = 'w-full rounded-2xl border border-[#d7e4ef] bg-white px-3 py-3 text-sm font-black text-[#092846] outline-none focus:border-sky-400';

 return (
  <div className="min-h-screen bg-[#eef6fb] px-3 py-5 pb-28 text-[#092846] sm:px-6">
   <div className="mx-auto max-w-6xl space-y-4">
    <div className="rounded-[2rem] border border-white bg-white/90 p-5 shadow-[0_18px_60px_rgba(20,54,84,0.10)] backdrop-blur-xl">
     <button onClick={onBack} className="mb-4 inline-flex items-center gap-2 rounded-2xl border border-[#d8e4ee] px-4 py-2 text-sm font-black text-[#092846]"><ArrowLeft className="h-4 w-4" /> Voltar</button>
     <div className="grid gap-4 lg:grid-cols-[minmax(0,1fr)_22rem] lg:items-end">
      <div>
       <p className="text-xs font-black uppercase tracking-[0.22em] text-sky-600">Regulamentação automática · {actName}</p>
       <h1 className="mt-2 text-3xl font-black leading-tight sm:text-4xl">Calculadora de jornada, reserva e sobreaviso.</h1>
       <p className="mt-2 max-w-3xl text-sm font-semibold leading-6 text-[#60758a]">O CrewCheck seleciona a ACT pela função do Perfil e aplica o limite mais restritivo entre RBAC 117 e ACT parametrizada. Para tripulação simples, a régua padrão é a Tabela B.1 por horário de apresentação e número de etapas.</p>
      </div>
      <div className="rounded-3xl border border-sky-100 bg-sky-50 p-4 text-sm font-semibold leading-6 text-sky-900">
       <b className="block text-base text-[#092846]">Regra principal aplicada</b>
       Reserva acionada conta desde o início da reserva. O campo legado fica visível só para auditoria, mas o cálculo padrão usa RBAC 117 B.1 e ACT por função.
      </div>
     </div>
    </div>

    <div className="grid gap-2 sm:grid-cols-3">
     <RegulationSegmentButton value="duty" current={mode} onClick={setMode}>Limite de jornada</RegulationSegmentButton>
     <RegulationSegmentButton value="standby" current={mode} onClick={setMode}>Sobreaviso</RegulationSegmentButton>
     <RegulationSegmentButton value="reserve" current={mode} onClick={setMode}>Reserva</RegulationSegmentButton>
    </div>

    <div className="grid gap-4 xl:grid-cols-[minmax(0,1fr)_24rem]">
     <section className="rounded-[2rem] border border-white bg-white p-5 shadow-[0_18px_60px_rgba(20,54,84,0.08)]">
      <div className="mb-5 flex flex-wrap items-center justify-between gap-3">
       <div>
        <p className="text-xs font-black uppercase tracking-[0.18em] text-sky-600">Parâmetros</p>
        <h2 className="text-2xl font-black">{mode === 'duty' ? 'Limite de Jornada' : mode === 'standby' ? 'Sobreaviso com acionamento' : 'Reserva com acionamento'}</h2>
       </div>
       <button type="button" onClick={() => { setManualLimit(''); setExtension(false); }} className="rounded-2xl border border-[#d8e4ee] px-4 py-2 text-xs font-black text-[#425a72]"><RefreshCw className="mr-1 inline h-4 w-4" /> Limpar ajuste</button>
      </div>

      <div className="grid gap-3 md:grid-cols-2">
       <RegulationField label="Tipo de tripulação">
        <div className="grid grid-cols-3 gap-2">
         <RegulationSegmentButton value="simples" current={crewType} onClick={setCrewType}>Simples</RegulationSegmentButton>
         <RegulationSegmentButton value="composta" current={crewType} onClick={setCrewType}>Composta</RegulationSegmentButton>
         <RegulationSegmentButton value="revezamento" current={crewType} onClick={setCrewType}>Revez.</RegulationSegmentButton>
        </div>
       </RegulationField>
       <RegulationField label="Tipo de voo">
        <div className="grid grid-cols-2 gap-2">
         <RegulationSegmentButton value="domestico" current={flightKind} onClick={setFlightKind}>Doméstico</RegulationSegmentButton>
         <RegulationSegmentButton value="internacional" current={flightKind} onClick={setFlightKind}>Internacional</RegulationSegmentButton>
        </div>
       </RegulationField>
       <RegulationField label="Número de pernas">
        <select value={stages} onChange={(event) => setStages(Number(event.target.value) || 1)} className={selectClass}>
         {[1, 2, 3, 4, 5].map((value) => <option key={value} value={value}>{value} {value === 1 ? 'perna' : 'pernas'}</option>)}
        </select>
       </RegulationField>
       <RegulationField label="Corte após pouso / margem">
        <select value={engineCutBuffer} onChange={(event) => setEngineCutBuffer(Number(event.target.value))} className={selectClass}>
         <option value={30}>30 min</option>
         <option value={45}>45 min</option>
         <option value={60}>60 min</option>
        </select>
       </RegulationField>
       <RegulationField label="Limite manual opcional">
        <input type="number" step="0.5" min={1} max={24} value={manualLimit} placeholder={`${baseDutyLimit}h automático`} onChange={(event) => setManualLimit(event.target.value)} className={inputClass} />
       </RegulationField>
       <RegulationField label="Teto SAV/Reserva legado">
        <input type="number" step="0.5" min={8} max={24} value={combinedLimit} onChange={(event) => setCombinedLimit(Math.max(8, Math.min(24, Number(event.target.value) || 12)))} className={inputClass} />
        <p className="mt-1 text-[11px] font-semibold normal-case leading-5 text-[#60758a]">Não substitui a Tabela B.1. Reserva acionada usa o início da reserva como início da jornada.</p>
       </RegulationField>
      </div>

      <div className="mt-4 flex flex-wrap gap-2">
       <button type="button" onClick={() => setAcclimated((value) => !value)} className={`rounded-full px-4 py-2 text-xs font-black ${acclimated ? 'bg-emerald-100 text-emerald-700' : 'bg-amber-100 text-amber-800'}`}>{acclimated ? 'Tripulante aclimatado' : 'Não aclimatado / desconhecido'}</button>
       <button type="button" onClick={() => setExtension((value) => !value)} className={`rounded-full px-4 py-2 text-xs font-black ${extension ? 'bg-violet-100 text-violet-700' : 'bg-[#eef5fb] text-[#425a72]'}`}>{extension ? 'Com extensão de 1h' : 'Sem extensão'}</button>
      </div>

      {mode === 'duty' && <div className="mt-5 grid gap-3 md:grid-cols-2">
       <RegulationField label="Apresentação / início da jornada"><input type="time" value={presentation} onChange={(event) => setPresentation(event.target.value)} className={inputClass} /></RegulationField>
       <div className="grid gap-2 rounded-3xl bg-[#f7fbff] p-4">
        <RegulationResultLine label="Jornada máxima" value={`${dutyLimitHours}h`} highlight />
        <RegulationResultLine label="Fonte automática" value={`${rbacLimit.source} · ${rbacLimit.startBucket}`} />
        <RegulationResultLine label="Tempo de voo ref." value={regFormatDuration(flightLimitHours * 60)} />
        <RegulationResultLine label="Corte dos motores até" value={dutyMotorCut.label} />
        <RegulationResultLine label="Encerramento até" value={dutyEnd.label} />
        <RegulationResultLine label="Sem extensão" value={dutyEndNoExtension.label} />
        <RegulationResultLine label="Com extensão +1h" value={dutyEndWithExtension.label} />
       </div>
      </div>}

      {mode === 'standby' && <div className="mt-5 grid gap-3 md:grid-cols-2">
       <RegulationField label="Início do sobreaviso"><input type="time" value={standbyStart} onChange={(event) => setStandbyStart(event.target.value)} className={inputClass} /></RegulationField>
       <RegulationField label="Acionamento/apresentação"><input type="time" value={standbyPresentation} onChange={(event) => setStandbyPresentation(event.target.value)} className={inputClass} /></RegulationField>
       <button type="button" onClick={() => setStandbyCalled((value) => !value)} className={`md:col-span-2 rounded-2xl px-4 py-3 text-sm font-black ${standbyCalled ? 'bg-[#092846] text-white' : 'bg-[#eef5fb] text-[#425a72]'}`}>{standbyCalled ? 'Com acionamento' : 'Sem acionamento'}</button>
       {standbyCalled ? <div className="md:col-span-2 grid gap-2 rounded-3xl bg-[#f7fbff] p-4 md:grid-cols-2">
        <RegulationResultLine label="Tempo em sobreaviso até apresentação" value={regFormatDuration(standbyElapsed)} />
        <RegulationResultLine label="Jornada disponível" value={regFormatDuration(standbyAvailable)} highlight />
        <RegulationResultLine label="Corte dos motores até" value={standbyMotorCut.label} />
        <RegulationResultLine label="Encerramento até" value={standbyEnd.label} />
        <RegulationResultLine label="Teto combinado" value={standbyCombinedDeadline.label} />
        <RegulationResultLine label="Limite normal da jornada" value={standbyNormalDutyDeadline.label} />
       </div> : <div className="md:col-span-2 grid gap-2 rounded-3xl bg-[#f7fbff] p-4 md:grid-cols-2">
        <RegulationResultLine label="Duração mínima" value={`até ${standbyMinEndWithoutCall.label}`} />
        <RegulationResultLine label="Duração máxima" value={`até ${standbyMaxEndWithoutCall.label}`} highlight />
       </div>}
      </div>}

      {mode === 'reserve' && <div className="mt-5 grid gap-3 md:grid-cols-2">
       <RegulationField label="Início da reserva"><input type="time" value={reserveStart} onChange={(event) => setReserveStart(event.target.value)} className={inputClass} /></RegulationField>
       <RegulationField label="Acionamento/apresentação"><input type="time" value={reservePresentation} onChange={(event) => setReservePresentation(event.target.value)} className={inputClass} /></RegulationField>
       <button type="button" onClick={() => setReserveCalled((value) => !value)} className={`md:col-span-2 rounded-2xl px-4 py-3 text-sm font-black ${reserveCalled ? 'bg-[#092846] text-white' : 'bg-[#eef5fb] text-[#425a72]'}`}>{reserveCalled ? 'Reserva com acionamento' : 'Reserva sem acionamento'}</button>
       {reserveCalled ? <div className="md:col-span-2 grid gap-2 rounded-3xl bg-[#f7fbff] p-4 md:grid-cols-2">
        <RegulationResultLine label="Tempo em reserva até apresentação" value={regFormatDuration(reserveElapsed)} />
        <RegulationResultLine label="Jornada disponível" value={regFormatDuration(reserveAvailable)} highlight />
        <RegulationResultLine label="Corte dos motores até" value={reserveMotorCut.label} />
        <RegulationResultLine label="Encerramento até" value={reserveEnd.label} />
        <RegulationResultLine label="Teto combinado" value={reserveCombinedDeadline.label} />
        <RegulationResultLine label="Limite normal da jornada" value={reserveNormalDutyDeadline.label} />
       </div> : <div className="md:col-span-2 grid gap-2 rounded-3xl bg-[#f7fbff] p-4 md:grid-cols-2">
        <RegulationResultLine label="Duração mínima" value={`até ${reserveMinEndWithoutCall.label}`} />
        <RegulationResultLine label="Duração máxima" value={`até ${reserveMaxEndWithoutCall.label}`} highlight />
       </div>}
      </div>}
     </section>

     <aside className="space-y-4">
      <div className="rounded-[2rem] border border-white bg-white p-5 shadow-[0_18px_60px_rgba(20,54,84,0.08)]">
       <div className="flex items-center gap-3"><div className="flex h-12 w-12 items-center justify-center rounded-2xl bg-sky-100 text-sky-700"><ShieldAlert className="h-6 w-6" /></div><div><p className="text-xs font-black uppercase tracking-[0.16em] text-sky-600">Modo confiável</p><h3 className="text-lg font-black">ACT por função</h3></div></div>
       <p className="mt-3 text-sm font-semibold leading-6 text-[#60758a]">Função lida do Perfil: <b>{activeProfile.functionLabel}</b>. Menus e cálculos seguem <b>{actName}</b>; se a função não estiver clara, ajuste em Configurações &gt; Conta.</p>
       <button onClick={onOpenScaleAlerts} className="mt-4 w-full rounded-2xl border border-[#d8e4ee] px-4 py-3 text-sm font-black text-[#092846]">Abrir alertas da escala</button>
      </div>
      <div className="rounded-[2rem] border border-amber-100 bg-amber-50 p-5 text-sm font-semibold leading-6 text-amber-900">
       <b className="block text-base text-amber-950">Como usar na prática</b>
       1. Escolha o tipo de cálculo. 2. Informe a apresentação/início. 3. Use “corte dos motores até” para o último pouso/corte e “encerramento” para a jornada com corte administrativo.
      </div>
      <div className="rounded-[2rem] border border-white bg-white p-5 text-sm font-semibold leading-6 text-[#60758a] shadow-[0_18px_60px_rgba(20,54,84,0.08)]">
       <b className="block text-base text-[#092846]">Observação</b>
       O cálculo é de apoio pessoal e depende de parâmetros configurados, tipo de tripulação, extensão, GRF/SGRF e interpretação operacional aplicável.
      </div>
     </aside>
    </div>
   </div>
...
type CockpitSeatForecast = { symbol?: string; title?: string; route?: string; flight?: string; score?: string; riskLabel?: string; source?: string; updatedAt?: string; empty?: boolean };
function cockpitSeatForecastFromSavedExtraOption(): CockpitSeatForecast {
 try {
  const raw = localStorage.getItem('crewcheck_extra_selected_option') || '';
  if (!raw) return { empty: true, symbol: '⚪', title: 'Sem voo selecionado', riskLabel: 'a escolher', source: 'Vivo de Extra' };
  const option = JSON.parse(raw) as ExtraFlightOptionEstimate;
  const load = option?.staffLoad || null;
  const symbol = extraStaffLoadSymbol(load);
  const riskLabel = extraStaffLoadSemanticLabel(load);
  const flights = (option.legs || []).map((leg) => String(leg.flightNumber || '').replace(/\s+/g, '').toUpperCase()).filter(Boolean).join(' + ');
  return {
   empty: false,
   symbol,
   title: load?.summary || load?.statusLabel || load?.chanceLabel || riskLabel || 'ocupação estimada',
   route: option.route || `${option.legs?.[0]?.origin || '—'} → ${option.legs?.[option.legs.length - 1]?.destination || '—'}`,
   flight: flights || option.title,
   score: typeof load?.chanceScore === 'number' ? `${load.chanceScore}%` : '',
   riskLabel,
   source: load?.exactLoad ? 'Load oficial' : (load?.source || option.source || 'estimativa CrewCheck'),
   updatedAt: load?.updatedAt || '',
  };
 } catch { return { empty: true, symbol: '⚪', title: 'Selecione um voo', riskLabel: 'a escolher', source: 'Vivo de Extra' }; }
}
...
function Home() {
 ...
 const activeRosterBootstrapRef = useRef(false);
 const [showOnboarding, setShowOnboarding] = useState(false);
 useEffect(() => { try { localStorage.setItem('crewcheck_onboarding_premium_v1', 'done'); } catch {} }, []);

 const user = getStoredUser();
 useEffect(() => {
  const nextRole = crewRoleSelectionFromFunction(profileSettings.rank);
  if (nextRole !== 'auto') setRoleSelection(nextRole);
 }, [profileSettings.rank]);
 const resolveSavedRosterOpenId = useCallback((id: string): string => {
  const summary = savedRosters.find((item) => item.id === id);
  const hasToken = Boolean(getToken());
  // Quando a sessão expira, o gerenciador pode estar exibindo resumo do banco,
  // mas o pacote completo existe no histórico local pelo checksum.
  return !hasToken && summary?.checksum ? summary.checksum : (summary?.checksum || id);
 }, [savedRosters]);
 ...
 const dashboardNextEventLive: DashboardNextEvent = dashboardNextEvent?.isFlight && hasUsefulRadarStatus(nextFlightStatus)
  ? {
    ...
    registration: nextFlightStatus?.registration || dashboardNextEvent.registration,
    aircraftIcao: nextFlightStatus?.aircraftIcao || dashboardNextEvent.aircraftIcao,
    estimatedArrival: nextFlightStatus?.estimatedArrival || dashboardNextEvent.estimatedArrival,
    landingCountdown: nextFlightStatus?.landingCountdown || dashboardNextEvent.landingCountdown,
 ...
 const dashboardUserName = resolveDashboardUserName(profileSettings, currentRosterBundle, savedRosters, user);
 const dashboardUserBase = profileSettings.base || currentRosterBundle?.roster?.base || user?.base || undefined;
 const dashboardSeatForecast = cockpitSeatForecastFromSavedExtraOption();
 ...
 useEffect(() => {
  if (!dashboardNextEvent?.isFlight || !dashboardNextEvent.flightNumber || !shouldFetchRadarForDashboardEvent(dashboardNextEvent)) {
  ...
  const loadRadarStatus = async () => {
   try {
    ...
    // v11.1.37: consulta passiva do Cockpit não dispara Telegram ao abrir o app.
    ...
    if (hasUsefulRadarStatus(payload)) {
     ...
     const compact = JSON.stringify({ status: payload.status || '', gate: payload.gate || '', terminal: payload.terminal || '', registration: payload.registration || '', estimatedArrival: payload.estimatedArrival || '', landingCountdown: payload.landingCountdown || '', estimatedDeparture: payload.estimatedDeparture || '' });
 ...
 useEffect(() => {
  if (!currentRosterBundle?.roster?.days?.length) return;
  let active = true;
  let timer: number | undefined;
  const runOperationalMonitor = async () => {
   try {
    if (localStorage.getItem('crewcheck_operational_monitor_enabled') === 'false') return;
    const params = new URLSearchParams({ notifyTelegram: '1', source: 'app' });
    const response = await fetch(`/api/operational-alerts/check?${params.toString()}`, { cache: 'no-store', headers: crewcheckAuthHeader() });
    const payload = await response.json().catch(() => null) as { ok?: boolean; alerts?: CrewCheckOperationalAlert[]; message?: string } | null;
    if (!active || !payload?.ok || !Array.isArray(payload.alerts)) return;
    for (const alert of payload.alerts.slice(0, 6)) {
     const key = `ops_${alert.category || 'alert'}_${alert.flightNumber || alert.airport || 'geral'}_${alert.fingerprint || alert.title || Date.now()}`.replace(/\s+/g, '_').slice(0, 180);
     notifyCrewCheck(alert.title || 'CrewCheck · alerta operacional', crewcheckOperationalAlertText(alert), key);
    }
   } catch {
    // Monitor operacional é auxiliar e nunca bloqueia a tela inicial.
   }
  };
  void runOperationalMonitor();
  timer = window.setInterval(() => void runOperationalMonitor(), 3 * 60_000);
 ...
 useEffect(() => {
  if (!canAccessC32FAcademy || savedLoading) return;
  try {
   const key = `crewcheck_admin_history_backfill_v11108_${crewcheckCurrentUserScope()}`;
   ...
   syncPendingRosters()
    .then((result) => {
     ...
     if (result.synced > 0) {
      void refreshSavedRosters();
      toast.success(`${result.synced} escala(s) antigas sincronizada(s) no gerenciador.`);
     }
    })
    .catch(() => undefined);
  } catch {
   // Sem storage ou offline: o botão sincronizar continua disponível.
  }
 }, [canAccessC32FAcademy, savedLoading, refreshSavedRosters]);

 useEffect(() => {
  const refreshCloud = () => void refreshSavedRosters();
  const syncOfflineQueue = () => {
   void syncPendingRosters()
    .then((result) => {
     setPendingOffline(result.remaining);
     if (result.synced > 0) {
      toast.success(`${result.synced} escala(s) sincronizada(s) ao voltar a conexão.`);
      void refreshSavedRosters();
     } else void refreshSavedRosters();
    })
    .catch(() => void refreshSavedRosters());
  };
 ...
 useEffect(() => {
  try {
   ...
   if (target && ['home','import','history','calendar','billing','iflight','flightboard','departure','routine','perdiem','salary','currency','parking','weather','regulation','c32f','settings','support','notes','chief','medical','bids','admin','systemstatus','more'].includes(target)) {
 ...
 useEffect(() => {
  if (hasCurrentRosterSession() || activeRosterBootstrapRef.current) return;
  activeRosterBootstrapRef.current = true;
  let cancelled = false;
  openActiveRoster()
   .then((data) => {
    if (cancelled || hasCurrentRosterSession() || !data?.roster?.days?.length) return;
    persistFreshRosterBundle(data.roster, roleSelection, data.summary?.sourceFileName || 'Escala ativa sincronizada');
    setDashboardClockTick(Date.now());
    window.setTimeout(() => window.dispatchEvent(new CustomEvent('crewcheck:refresh-smart-departure')), 450);
    toast.success('Escala ativa recuperada para este dispositivo.');
   })
   .catch(() => {
    activeRosterBootstrapRef.current = false;
   });
 ...
 useEffect(() => {
  ...
  openSavedRoster(latest.id)
   .then((data) => {
    persistFreshRosterBundle(data.roster, roleSelection, latest.sourceFileName || 'Última escala sincronizada');
    toast.success('Última escala do histórico sincronizada neste dispositivo.');
    setDashboardClockTick(Date.now());
    window.setTimeout(() => window.dispatchEvent(new CustomEvent('crewcheck:refresh-smart-departure')), 450);
   })
   .catch(() => {
 ...
 const handleOpenSavedRoster = useCallback(async (id: string, targetView: ResultsView = 'summary') => {
  ...
  try {
   const openId = resolveSavedRosterOpenId(id);
   const data = await openSavedRoster(openId);
   const summary = savedRosters.find((item) => item.id === id);
   persistFreshRosterBundle(data.roster, roleSelection, summary?.sourceFileName || 'Escala salva');
 ...
 }, [roleSelection, savedRosters, setLocation, resolveSavedRosterOpenId]);
 ...
 }, [roleSelection, savedRosters, resolveSavedRosterOpenId]);
 ...
 async function handleActivateSavedRoster(id: string) {
  ...
  try {
   const summary = savedRosters.find((item) => item.id === id);
   if (!getToken()) {
    const data = await openSavedRoster(summary?.checksum || id);
    persistFreshRosterBundle(data.roster, roleSelection, summary?.sourceFileName || 'Escala ativa local');
    setDashboardClockTick(Date.now());
    window.setTimeout(() => window.dispatchEvent(new CustomEvent('crewcheck:refresh-smart-departure')), 350);
    toast.success('Escala ativa neste dispositivo. Faça login para sincronizar em nuvem.');
    return;
   }
   await activateRosterAnalysis(id);
   const data = await openSavedRoster(summary?.checksum || id);
   persistFreshRosterBundle(data.roster, roleSelection, summary?.sourceFileName || 'Escala ativa');
 ...
 function handleProfileSettingsChange(next: LocalProfileSettings) {
  const saved = saveLocalProfileSettings(next);
  setProfileSettings(saved);
  const nextRole = crewRoleSelectionFromFunction(saved.rank);
  if (nextRole !== 'auto') setRoleSelection(nextRole);
  fetch('/api/profile/contact', {
   method: 'POST',
   headers: crewcheckAuthHeader({ 'content-type': 'application/json' }),
   body: JSON.stringify({
    phoneCountryIso: saved.phoneCountryIso,
  ...
  toast.success(nextRole !== 'auto' ? `Perfil atualizado. Regulamentação automática: ${nextRole === 'pilot' ? 'ACT Pilotos' : 'ACT Comissários'}.` : 'Perfil atualizado neste dispositivo.');
 }
 ...
 const publishRosterSyncBundle = useCallback((roster: CrewRoster, compliance: ComplianceResult, gym: GymRecommendation[], sourceFileName: string) => {
  const payload = { roster, compliance, gym, sourceFileName, updatedAt: new Date().toISOString(), source: 'crewcheck-unified-sync-v11108', userScope: crewcheckCurrentUserScope() };
  try {
   persistRosterPayloadLocally(payload);
 ...
 useEffect(() => {
  function applyIncomingBundle(payload: any) {
   ...
   try {
    persistFreshRosterBundle(payload.roster, roleSelection, payload.sourceFileName || 'Escala sincronizada');
    setDashboardClockTick(Date.now());
 ...
 const handleDashboardNavigate = useCallback((view: string) => {
  const premiumViews = new Set(['flightboard','departure','smartDeparture','departureExtra','wakeup','despertador','perdiem','salary','currency','parking','weather','chief','medical','bids','reports','alerts']);
  ...
  const map: Record<string, () => void> = {
   ...
   regulamentacao: () => setCurrentView('regulation'),
   conformidade: () => setCurrentView('regulation'),
   manager: () => setCurrentView('history'),
   gerenciador: () => setCurrentView('history'),
   ...
   wakeup: () => setCurrentView('wakeup'),
   despertador: () => setCurrentView('wakeup'),
   perdiem: () => setCurrentView('perdiem'),
   salary: () => setCurrentView('salary'),
   currency: () => setCurrentView('currency'),
   parking: () => setCurrentView('parking'),
   weather: () => setCurrentView('weather'),
   meteorologia: () => setCurrentView('weather'),
   defesacivil: () => setCurrentView('weather'),
 ...
 const commitRoster = useCallback(async (inputRoster: CrewRoster, sourceFileName: string, targetView: ResultsView = 'summary', options?: { demo?: boolean; sourceFileDataBase64?: string | null; sourceMimeType?: string | null; sourceFileSize?: number | null }) => {
  ...
  let roster = normalizeRosterSchedule(detectAndMarkLayovers(inputRoster));
  const identityResult = secureRosterIdentityForImport(roster, profileSettings, sourceFileName, { demo: isDemo });
  roster = identityResult.roster;
  if (identityResult.profile) setProfileSettings(identityResult.profile);
  if (identityResult.message && !isDemo) toast.info(identityResult.message);
  ...
  sessionStorage.setItem('crewcheck_source_file', sourceFileName);
  persistRosterPayloadLocally({ roster, compliance, gym, sourceFileName, roleSelection, updatedAt: new Date().toISOString(), source: 'crewcheck-active-roster-v11108', userScope: crewcheckCurrentUserScope() });
  if (isDemo) {
  ...
  if (changeSummary && !isDemo) toast.info(changeSummary.message, { description: changeSummary.details?.slice(0, 3).join(' • ') || 'Sem diferenças detalhadas.' } as any);
  ...
  setLocation('/results');
 }, [forceSyncRosterEverywhere, profileSettings, roleSelection, setLocation]);
 ...
 const renderWithMobileMenu = (content: ReactNode, active: CrewCheckBottomActive = 'home') => (
  <div className={`crewcheck-side-layout ${premiumDrawerOpen ? 'is-menu-open' : ''}`}>
   ...
   <CrewCheckMobileMenu active={active} onNavigate={(view) => { setPremiumDrawerOpen(false); handleDashboardNavigate(view); }} canAccessAdmin={canAccessC32FAcademy} crewRole={crewRoleSelectionFromFunction(profileSettings.rank)} drawerOpen={premiumDrawerOpen} onClose={() => setPremiumDrawerOpen(false)} />
 ...
 if (currentView === 'weather') return renderWithMobileMenu(<MeteorologiaDefesaCivilScreen bundle={currentRosterBundle} onBack={() => setCurrentView('home')} />, 'more');

if (currentView === 'regulation') return renderWithMobileMenu(<RegulationCalculatorPanel onBack={() => setCurrentView('home')} onOpenScaleAlerts={() => goToResults('irregularities')} />, 'roster');
 ...
 if (currentView === 'wakeup') {
  return renderWithMobileMenu(<WakeupPremiumPanel profileSettings={profileSettings} onProfileSettingsChange={handleProfileSettingsChange} billingStatus={homeBillingStatus} canAccessAdmin={canAccessC32FAcademy} hasRoster={hasCurrentRosterSession()} onBack={() => setCurrentView('home')} onOpenImport={() => setCurrentView('import')} />, 'settings');
 }

 if (currentView === 'settings') {
  return renderWithMobileMenu(<SettingsHomePanel
   userLabel={dashboardUserName}
 ...
 return renderWithMobileMenu(
  <div className="relative min-h-screen bg-[#08111f] crewcheck-has-global-menu">
   ...
   />
   <Dashboard
    user={{ name: dashboardUserName, company: profileSettings.company || 'CrewCheck Premium', base: dashboardUserBase, avatar: profileAvatar || undefined }}
    ...
    seatForecast={dashboardSeatForecast}
...
function PremiumOnboardingModal({ onClose, onImport, onTelegram, onDemo }: { onClose: () => void; onImport: () => void; onTelegram: () => void; onDemo: () => void }) {
 const steps = [
  ...
  { icon: MessageCircle, title: 'Telegram CrewCheck', text: 'Conecte uma vez e depois envie PDF direto pelo bot. O CrewCheck lê, salva e responde com o status da importação.' },
 ];
 return (
...
function CrewCheckMobileMenu({ active, onNavigate, canAccessAdmin = false, crewRole = 'auto', drawerOpen = false, onClose }: { active: CrewCheckBottomActive; onNavigate: (view: string) => void; canAccessAdmin?: boolean; crewRole?: CrewRoleSelection; drawerOpen?: boolean; onClose?: () => void }) {
 ...
 const groups: Array<{ key: string; label: string; icon: LucideIcon; items: Array<{ label: string; icon: LucideIcon; view: string; activeId?: CrewCheckBottomActive; admin?: boolean; roleAllowed?: Array<'cabin' | 'pilot' | 'auto'>; hint?: string }> }> = [
  { key: 'operacional', label: 'Operacional', icon: Plane, items: [
   ...
   { label: 'Meteorologia', icon: ShieldAlert, view: 'weather', activeId: 'more', hint: 'METAR/TAF por escala' },
  ]},
  { key: 'relatorios', label: 'Relatórios', icon: BarChart3, items: [
   { label: 'Relatórios', icon: BarChart3, view: 'reports', activeId: 'more', hint: 'métricas e exportações' },
   { label: 'Alertas da escala', icon: Bell, view: 'alerts', activeId: 'roster', hint: 'avisos auditáveis' },
   { label: 'Rotina', icon: ClipboardList, view: 'routine', activeId: 'more', hint: 'sugestões na escala' },
   { label: 'Calendário', icon: CalendarDays, view: 'calendar', activeId: 'more', hint: 'visual, ICS e Google' },
   { label: 'Chefe de Cabine', icon: ClipboardList, view: 'chief', activeId: 'more', roleAllowed: ['cabin','auto'], hint: 'caderno e bordo' },
  ]},
  ...
  <aside className={`crewcheck-premium-sidebar ${showExpanded ? 'is-expanded' : 'is-collapsed'} ${drawerOpen ? 'is-mobile-open' : ''}`} aria-label="Menu principal CrewCheck">
   ...
   <nav className="crewcheck-sidebar-nav">
    {groups.map((group) => {
     ...
     const visibleItems = group.items.filter((item) => (!item.admin || canAccessAdmin) && (!item.roleAllowed || item.roleAllowed.includes(crewRole))); 
...
function CrewCheckFloatingSpeedMenu({ active, onNavigate, canAccessC32FAcademy, hasRoster }: { active: 'home' | 'roster' | 'iflight' | 'departure' | 'settings' | 'more'; onNavigate: (view: string) => void; canAccessC32FAcademy: boolean; hasRoster: boolean }) {
 ...
 const quickItems: Array<{ label: string; hint: string; icon: LucideIcon; view: string; premium?: boolean }> = [
  ...
  { label: 'Checklist Médico', hint: 'grátis · ocorrência e PDF', icon: FileText, view: 'medical' },
  ...
  { label: 'Despertador Premium', hint: 'hotel, quarto e ligação', icon: Bell, view: 'wakeup', premium: true },
  ...
  { label: 'Gerenciador de Escalas', hint: 'ativa, abrir ou excluir', icon: History, view: 'history' },
  { label: 'Regulamentação', hint: 'jornada e acionamento', icon: ShieldAlert, view: 'regulation' },
  { label: 'Relatórios', hint: 'métricas e PDF', icon: BarChart3, view: 'reports' },
  { label: 'Alertas da escala', hint: 'avisos e conformidade', icon: Bell, view: 'alerts' },
  { label: 'Calendário', hint: 'visual, ICS e Google', icon: CalendarDays, view: 'calendar' },
  { label: 'Meteorologia', hint: 'METAR/TAF e tempestade', icon: ShieldAlert, view: 'weather' },
...
type ExtraFlightOptionEstimate = {
 ...
 staffLoad?: ExtraFlightStaffLoad | null;
 baggage?: string | null;
...
type CrewCheckLayoverItem = { airport: string; startIso: string; endIso?: string; nights: number; fromFlight?: string; returnFlight?: string; route?: string; returnRoute?: string };
type CrewCheckAwayHomePlan = { homeAirport: string; layovers: CrewCheckLayoverItem[]; daysAway: number; returnIso?: string; returnFlight?: string; returnRoute?: string; nextAwayAirport?: string };
type AirportWeatherSnapshot = { ok?: boolean; airport?: string; airportName?: string; current?: { label?: string; severity?: string; tone?: string; temperature?: number; precipitationMm?: number; windKmh?: number; windGustKmh?: number; advisory?: string }; daily?: Array<{ date?: string; label?: string; severity?: string; tone?: string; temperatureMax?: number; temperatureMin?: number; precipitationProbability?: number; precipitationMm?: number; windGustKmh?: number; advisory?: string }>; source?: string; updatedAt?: string; message?: string };
function crewcheckDateKeyAtNoon(value?: string | null): Date | null {
...
function shouldFetchAirportWeatherNearDate(value?: string | null): boolean {
 const target = crewcheckDateKeyAtNoon(value);
...
type ReturnHomePlan = { ok?: boolean; title?: string; airport?: string; airportName?: string; home?: { name?: string; source?: string; address?: string | null }; landingIso?: string; bufferMinutes?: number; readyToLeaveLabel?: string; primary?: { mode?: string; label?: string; localizedDuration?: string; localizedDistance?: string; durationMinutes?: number; leaveAtLabel?: string; arriveAtLabel?: string; source?: string }; routeOptions?: Array<{ mode?: string; label?: string; localizedDuration?: string; localizedDistance?: string; durationMinutes?: number; leaveAtLabel?: string; arriveAtLabel?: string; source?: string }>; message?: string; diagnostics?: Record<string, unknown> };

function crewCheckWeatherToneClass(severity?: string | null): string { const tone = String(severity || '').toLowerCase(); if (tone === 'red') return 'border-red-200/40 bg-red-300/15 text-red-50'; if (tone === 'amber') return 'border-amber-200/40 bg-amber-300/15 text-amber-50'; if (tone === 'yellow') return 'border-yellow-200/40 bg-yellow-300/15 text-yellow-50'; return 'border-emerald-200/35 bg-emerald-300/15 text-emerald-50'; }
function crewCheckWeatherSymbol(severity?: string | null): string { const tone = String(severity || '').toLowerCase(); if (tone === 'red') return '🔴'; if (tone === 'amber') return '🟠'; if (tone === 'yellow') return '🟡'; return '🟢'; }
function crewCheckFlightNumberForReturn(leg?: any): string { return String((leg as any)?.flightNumber || (leg as any)?.flight || '').toUpperCase().replace(/\s+/g, ''); }

function buildCrewCheckAwayHomePlan(bundle: SessionRosterBundle | null | undefined, fallbackBase = 'BSB'): CrewCheckAwayHomePlan {
 const roster = bundle?.roster;
 const homeAirport = normalizedAirportCode(roster?.base || fallbackBase || 'BSB') || 'BSB';
 const days = (roster?.days || []).map((day) => ({ day, date: rosterDateFromString(day.date) })).filter((item) => !Number.isNaN(item.date.getTime())).sort((a, b) => a.date.getTime() - b.date.getTime());
 const today = new Date(); today.setHours(0, 0, 0, 0);
 const layovers: CrewCheckLayoverItem[] = [];
 let current: CrewCheckLayoverItem | null = null; let firstAwayDate: Date | null = null; let returnHomeDate: Date | null = null; let returnFlight = ''; let returnRoute = '';
 for (const { day, date } of days) {
  if (date.getTime() < today.getTime() - 86400000) continue;
  const legs = Array.isArray(day.legs) ? day.legs : []; if (!legs.length) continue;
  const first = legs[0]; const last = legs[legs.length - 1];
  const origin = normalizedAirportCode((first as any)?.origin || (first as any)?.from || '');
  const destination = normalizedAirportCode((last as any)?.destination || (last as any)?.to || '');
  const iso = date.toISOString().slice(0, 10); if (!destination) continue;
  if (!current && destination !== homeAirport) { current = { airport: destination, startIso: iso, nights: 1, fromFlight: crewCheckFlightNumberForReturn(last), route: `${origin || '—'} → ${destination}` }; firstAwayDate = firstAwayDate || date; }
  else if (current && destination === homeAirport) { current.endIso = iso; current.returnFlight = crewCheckFlightNumberForReturn(last); current.returnRoute = `${origin || '—'} → ${destination}`; current.nights = Math.max(1, Math.round((date.getTime() - new Date(`${current.startIso}T00:00:00`).getTime()) / 86400000)); layovers.push(current); returnHomeDate = date; returnFlight = current.returnFlight || ''; returnRoute = current.returnRoute || ''; current = null; break; }
  else if (current && origin === current.airport && destination !== current.airport) { current.endIso = iso; current.returnFlight = crewCheckFlightNumberForReturn(last); current.returnRoute = `${origin || '—'} → ${destination}`; current.nights = Math.max(1, Math.round((date.getTime() - new Date(`${current.startIso}T00:00:00`).getTime()) / 86400000)); layovers.push(current); if (destination !== homeAirport) current = { airport: destination, startIso: iso, nights: 1, fromFlight: crewCheckFlightNumberForReturn(last), route: `${origin || '—'} → ${destination}` }; else { returnHomeDate = date; returnFlight = crewCheckFlightNumberForReturn(last); returnRoute = `${origin || '—'} → ${destination}`; current = null; break; } }
 }
 if (current) layovers.push(current);
 const daysAway = firstAwayDate && returnHomeDate ? Math.max(1, Math.round((returnHomeDate.getTime() - firstAwayDate.getTime()) / 86400000) + 1) : layovers.reduce((sum, item) => sum + Math.max(1, item.nights || 1), 0);
 return { homeAirport, layovers, daysAway, returnIso: returnHomeDate?.toISOString().slice(0, 10), returnFlight, returnRoute, nextAwayAirport: layovers[0]?.airport };
}
function optionArrivalAirportForReturn(item: ExtraFlightOptionEstimate | null | undefined, fallback = ''): string { const legs = Array.isArray(item?.legs) ? item?.legs || [] : []; const last = legs[legs.length - 1]; return normalizedAirportCode((last as any)?.destination || (last as any)?.to || fallback || ''); }
function optionArrivalIsoForReturn(item: ExtraFlightOptionEstimate | null | undefined, fallbackDate = ''): string { const legs = Array.isArray(item?.legs) ? item?.legs || [] : []; const last = legs[legs.length - 1]; const rawTime = String((last as any)?.arrival || (last as any)?.arrivalTime || item?.arriveAt || '').match(/(\d{1,2}:\d{2})/)?.[1] || '12:00'; const date = String((last as any)?.arrivalDate || item?.selectedDate || fallbackDate || new Date().toISOString().slice(0, 10)).slice(0, 10); return `${date}T${rawTime.padStart(5, '0')}:00-03:00`; }
...
function SmartDepartureScreen({ nextEvent, bundle, userBase, hasRoster, canAccessAdmin = false, onBack, onOpenRoster }: { nextEvent?: DashboardNextEvent; bundle?: SessionRosterBundle | null; userBase?: string; hasRoster: boolean; canAccessAdmin?: boolean; onBack: () => void; onOpenRoster: () => void }) {
 ...
 const [returnHomePlan, setReturnHomePlan] = useState<ReturnHomePlan | null>(null);
 const [returnHomeLoading, setReturnHomeLoading] = useState(false);
 const [returnHomeError, setReturnHomeError] = useState('');
 const [layoverWeather, setLayoverWeather] = useState<Record<string, AirportWeatherSnapshot>>({});
 ...
 const awayHomePlan = useMemo(() => buildCrewCheckAwayHomePlan(bundle, userBase || defaultExtraTargetAirport || 'BSB'), [bundle, userBase, defaultExtraTargetAirport]);
 ...
 useEffect(() => {
  const items = awayHomePlan.layovers.slice(0, 6).filter((item) => shouldFetchAirportWeatherNearDate(item.startIso));
  if (!items.length) { setLayoverWeather({}); return; }
  let active = true;
  async function loadLayoverWeather() {
   const entries: Record<string, AirportWeatherSnapshot> = {};
   for (const item of items) {
    try {
     const params = new URLSearchParams({ airport: item.airport, date: item.startIso, days: '5' });
     const response = await fetch(`/api/weather/airport?${params.toString()}`, { headers: crewcheckAuthHeader(), cache: 'force-cache' });
     const payload = await response.json().catch(() => null) as AirportWeatherSnapshot | null;
     if (payload) entries[`${item.airport}-${item.startIso}`] = payload;
    } catch {}
   }
   if (active) setLayoverWeather(entries);
  }
  void loadLayoverWeather();
  return () => { active = false; };
 }, [awayHomePlan.layovers.map((item) => `${item.airport}-${item.startIso}`).join('|')]);

 const calculateReturnHome = useCallback(async () => {
  const arrivalAirport = optionArrivalAirportForReturn(selectedExtraOption, selectedExtraDestination || airport || awayHomePlan.homeAirport);
  const landingIso = optionArrivalIsoForReturn(selectedExtraOption, extraFlightDate);
  const homeLat = localStorage.getItem('crewcheck_home_lat') || '';
  const homeLng = localStorage.getItem('crewcheck_home_lng') || '';
  setReturnHomeLoading(true);
  setReturnHomeError('');
  try {
   const params = new URLSearchParams({ airport: arrivalAirport || awayHomePlan.homeAirport || 'BSB', landingIso, bufferMinutes: '25', homeLat, homeLng, homeCep, homeAddress });
   const response = await fetch(`/api/smart-return-home?${params.toString()}`, { headers: crewcheckAuthHeader(), cache: 'no-store' });
   const payload = await response.json().catch(() => null) as ReturnHomePlan | null;
   if (!payload || payload.ok === false) throw new Error(payload?.message || 'Não consegui calcular a volta pra casa.');
   setReturnHomePlan(payload);
   toast.success('Volta pra Casa Inteligente calculada.');
  } catch (error) {
   setReturnHomeError(error instanceof Error ? error.message : 'Falha ao calcular volta pra casa.');
   setReturnHomePlan(null);
  } finally { setReturnHomeLoading(false); }
 }, [selectedExtraOption, selectedExtraDestination, airport, awayHomePlan.homeAirport, extraFlightDate, homeCep, homeAddress]);
       ...
       <div ref={vivoSectionRef} className="mt-5 rounded-[1.7rem] border border-fuchsia-200/20 bg-fuchsia-300/10 p-4 shadow-xl shadow-black/10">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
         <div>
          ...
          <h3 className="mt-1 text-xl font-black text-fuchsia-50">Vivo de Extra / Volta pra Casa</h3>
          <p className="mt-2 text-sm leading-6 text-fuchsia-50/80">Pesquise origem e destino livremente. Use para chegar à base, voltar pra casa depois de pousar fora da base ou montar rota inversa. O CrewCheck calcula opções de voo, previsão de assento e depois pode acionar a Volta pra Casa Inteligente.</p>
        ...
        <div className="mt-4 grid gap-3 md:grid-cols-2 xl:grid-cols-4">
         ...
         <label className="rounded-2xl border border-white/10 bg-black/20 p-3">
          <span className="text-[10px] font-black uppercase tracking-[0.18em] text-fuchsia-100/70">Destino / voltar pra casa</span>
          <input list="crewcheck-extra-destinations" value={extraTargetAirport} onChange={(event) => { const value = event.target.value.toUpperCase().replace(/[^A-Z]/g, '').slice(0, 3); setExtraTargetAirport(value); localStorage.setItem('crewcheck_extra_target_airport', value); }} placeholder="Ex.: BSB" className="mt-2 w-full rounded-xl border border-white/10 bg-white/90 px-3 py-3 text-sm font-black text-slate-950 outline-none placeholder:text-slate-500" />
          <datalist id="crewcheck-extra-destinations">{extraDestinationOptions.map((code) => <option key={code} value={code}>{code}{contractualBaseOptions.includes(code) ? ' · base contratual' : ' · base virtual/perfil'}</option>)}</datalist>
          <button type="button" onClick={() => { const from = normalizeExtraAirport(originAirportForExtra); const to = normalizeExtraAirport(extraTargetAirport) || selectedExtraDestination; setOriginAirportForExtra(to); setExtraTargetAirport(from || defaultExtraTargetAirport); localStorage.setItem('crewcheck_extra_origin_airport', to); localStorage.setItem('crewcheck_extra_target_airport', from || defaultExtraTargetAirport); toast.success('Pesquisa inversa ativada: origem e destino trocados.'); }} className="mt-2 w-full rounded-xl border border-emerald-200/25 bg-emerald-300/15 px-3 py-2 text-[11px] font-black text-emerald-50">Inverter rota / voltar pra casa</button>
         </label>
         <label className="rounded-2xl border border-white/10 bg-black/20 p-3">
        ...
        {selectedExtraOption ? (
         <div className="mt-3 rounded-2xl border border-emerald-200/30 bg-emerald-300/12 px-4 py-3 text-xs font-black text-emerald-50">
          Opção selecionada para Saída/Volta Inteligente: {selectedExtraOption.route} · decolagem {cleanExtraClock(selectedExtraOption.legs?.[0]?.departure || selectedExtraOption.leaveAt) || selectedExtraOption.leaveAt} · apresentação {flightPresentationBuffer} min antes · aeroporto {selectedExtraDepartureAirport || 'a confirmar'}.
         </div>
        ) : null}


        <div className="mt-4 grid gap-3 xl:grid-cols-[minmax(0,1fr)_22rem]">
         <div className="rounded-[1.7rem] border border-emerald-200/20 bg-emerald-300/10 p-4 text-emerald-50 shadow-xl shadow-black/10">
          <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
           <div>
            <p className="text-[10px] font-black uppercase tracking-[0.22em] text-emerald-100/70">Volta pra Casa Inteligente</p>
            <h3 className="mt-1 text-xl font-black">Casa depois do pouso</h3>
            <p className="mt-2 text-sm leading-6 text-emerald-50/85">Calcula aeroporto → casa depois da chegada, somando 25 min de desembarque/saída do terminal. Use depois de escolher um voo no Vivo de Extra.</p>
           </div>
           <button type="button" onClick={() => void calculateReturnHome()} disabled={returnHomeLoading} className="rounded-2xl bg-emerald-200 px-4 py-3 text-xs font-black uppercase tracking-[0.12em] text-[#06241a] disabled:opacity-60">{returnHomeLoading ? 'Calculando' : 'Calcular volta'}</button>
          </div>
          {returnHomeError ? <p className="mt-3 rounded-2xl border border-amber-200/30 bg-amber-300/15 px-3 py-2 text-xs font-bold text-amber-50">{returnHomeError}</p> : null}
          {returnHomePlan?.primary ? <div className="mt-4 grid gap-2 sm:grid-cols-4">
           <SmartDepartureMetric label="Sair do aeroporto" value={returnHomePlan.readyToLeaveLabel || '—'} hint={`${returnHomePlan.bufferMinutes || 25} min pós-pouso`} />
           <SmartDepartureMetric label="Melhor rota" value={returnHomePlan.primary.localizedDuration || `${returnHomePlan.primary.durationMinutes || '—'} min`} hint={returnHomePlan.primary.label || returnHomePlan.primary.mode || 'rota'} />
           <SmartDepartureMetric label="Chegada em casa" value={returnHomePlan.primary.arriveAtLabel || '—'} hint={returnHomePlan.primary.localizedDistance || 'distância'} />
           <SmartDepartureMetric label="Fonte" value={String(returnHomePlan.primary.source || '').includes('TomTom') ? 'TomTom' : String(returnHomePlan.primary.source || '').includes('Google') ? 'Google' : 'CrewCheck'} hint="trânsito auditável" />
          </div> : <div className="mt-4 grid gap-2 sm:grid-cols-3">
           <SmartDepartureMetric label="Casa" value={homeAddress || homeCep ? 'cadastrada' : 'validar'} hint="Configurações → Perfil" />
           <SmartDepartureMetric label="Aeroporto" value={optionArrivalAirportForReturn(selectedExtraOption, selectedExtraDestination || awayHomePlan.homeAirport) || '—'} hint="chegada selecionada" />
           <SmartDepartureMetric label="Pouso" value={selectedExtraOption ? cleanExtraClock(selectedExtraOption.arriveAt) || 'selecionado' : 'selecione voo'} hint="Vivo de Extra" />
          </div>}
          {returnHomePlan?.message ? <p className="mt-3 text-xs font-semibold leading-5 text-emerald-50/80">{returnHomePlan.message}</p> : null}
         </div>

         <div className="rounded-[1.7rem] border border-cyan-200/20 bg-cyan-300/10 p-4 text-cyan-50 shadow-xl shadow-black/10">
          <p className="text-[10px] font-black uppercase tracking-[0.22em] text-cyan-100/70">fora de casa</p>
          <h3 className="mt-1 text-xl font-black">{awayHomePlan.daysAway ? `${awayHomePlan.daysAway} dia(s)` : 'sem pernoite futuro'}</h3>
          <p className="mt-2 text-sm leading-6 text-cyan-50/80">{awayHomePlan.returnIso ? `Volta pra casa em ${awayHomePlan.returnIso.split('-').reverse().join('/')} ${awayHomePlan.returnFlight ? `· ${awayHomePlan.returnFlight}` : ''}` : 'Ainda não achei voo futuro terminando em casa na escala salva.'}</p>
          {awayHomePlan.layovers.length ? <div className="mt-3 grid gap-2">
           {awayHomePlan.layovers.slice(0, 3).map((layover) => {
            const weather = layoverWeather[`${layover.airport}-${layover.startIso}`];
            const daily = weather?.daily?.find((item) => item.date === layover.startIso) || weather?.daily?.[0];
            const sev = daily?.severity || weather?.current?.severity || 'green';
            return <div key={`${layover.airport}-${layover.startIso}`} className="rounded-2xl border border-cyan-200/18 bg-white/[0.055] px-3 py-2 text-xs font-bold leading-5 text-cyan-50">
             <p className="font-black uppercase tracking-[0.12em]">🌦️ {layover.airport} · {layover.nights || 1} noite(s)</p>
             <p className="mt-1 text-cyan-50/75">{daily ? `${daily.label || 'Tempo'} · ${Math.round(Number(daily.temperatureMin || 0))}–${Math.round(Number(daily.temperatureMax || 0))}°C · chuva ${Math.round(Number(daily.precipitationProbability || 0))}%` : 'Previsão carregando...'}</p>
             <p className="mt-1 text-[10px] font-black uppercase tracking-[0.12em] text-cyan-100/45">meteorologia sem semáforo · cores reservadas para ocupação</p>
            </div>;
           })}
          </div> : null}
         </div>
...
type AirportBoardRow = {
 ...
 codeshareFlights?: string[] | null;
 codeshareHiddenCount?: number | null;
 codeshareFiltered?: boolean | null;
};
...
type AirLabsLiveTrackSnapshot = {
 ...
 source?: string | null;
 updatedAt?: string | null;
 rawAgeSeconds?: number | null;
 message?: string | null;
 providerStatus?: Record<string, boolean>;
 diagnostics?: RadarDiagnostic[];
};
...
type CrewCheckAirlineCatalogEntry = { iata?: string; icao?: string; name?: string; logo?: string; aliases?: string[] };
type CrewCheckAirportCatalogEntry = { iata?: string; icao?: string; airport?: string; country_code?: string; region_name?: string; latitude?: number; longitude?: number };

const crewcheckAirlineCatalogMap = new Map<string, CrewCheckAirlineCatalogEntry>();
const crewcheckAirportCatalogMap = new Map<string, CrewCheckAirportCatalogEntry>();
let crewcheckCatalogLoadPromise: Promise<void> | null = null;

function catalogCode(value?: string | null, max = 8): string {
 return String(value || '').toUpperCase().replace(/[^A-Z0-9]/g, '').slice(0, max);
}

function normalizeRemoteLogoUrl(value?: string | null): string {
 const raw = String(value || '').trim();
 if (!raw) return '';
...
function registerAirlineCatalogItem(item: CrewCheckAirlineCatalogEntry) {
 const logo = normalizeRemoteLogoUrl(item.logo);
 const normalized = { ...item, logo };
 const keys = [item.iata, item.icao, ...(item.aliases || [])].map((key) => catalogCode(key)).filter(Boolean);
 if (item.name) keys.push(catalogCode(item.name, 24));
 for (const key of keys) crewcheckAirlineCatalogMap.set(key, normalized);
}

function registerAirportCatalogItem(item: CrewCheckAirportCatalogEntry) {
 const keys = [item.iata, item.icao].map((key) => catalogCode(key, 4)).filter(Boolean);
 for (const key of keys) crewcheckAirportCatalogMap.set(key, item);
}

function findAirlineCatalogEntry(...values: Array<string | null | undefined>): CrewCheckAirlineCatalogEntry | undefined {
 const compactFlight = values.map((value) => String(value || '').toUpperCase().replace(/\s+/g, '')).find((value) => /^[A-Z0-9]{2,3}\d{1,5}$/.test(value));
 const keys = [
  ...values.map((value) => catalogCode(value)),
  compactFlight ? catalogCode(compactFlight.match(/^([A-Z0-9]{2,3})/)?.[1] || '') : '',
 ].filter(Boolean);
 for (const key of keys) {
  const direct = crewcheckAirlineCatalogMap.get(key);
  if (direct) return direct;
 }
 const source = values.filter(Boolean).join(' ').toLowerCase();
 if (!source) return undefined;
 return Array.from(crewcheckAirlineCatalogMap.values()).find((item) => item.name && source.includes(String(item.name).toLowerCase()));
}

async function loadCrewCheckAviationCatalogs(): Promise<void> {
 if (crewcheckCatalogLoadPromise) return crewcheckCatalogLoadPromise;
 crewcheckCatalogLoadPromise = (async () => {
  try {
   const [airlinesResponse, airportsResponse] = await Promise.all([
    fetch('/api/catalog/airlines?limit=5000', { cache: 'force-cache', headers: crewcheckAuthHeader() }),
    fetch('/api/catalog/airports?country=BR&limit=2000', { cache: 'force-cache', headers: crewcheckAuthHeader() }),
   ]);
   const airlinesPayload = await airlinesResponse.json().catch(() => null);
   const airportsPayload = await airportsResponse.json().catch(() => null);
   if (Array.isArray(airlinesPayload?.data)) airlinesPayload.data.forEach(registerAirlineCatalogItem);
   if (Array.isArray(airportsPayload?.data)) airportsPayload.data.forEach(registerAirportCatalogItem);
  } catch {
   // Catálogo é enriquecimento visual; nunca bloqueia o radar.
  }
 })();
 return crewcheckCatalogLoadPromise;
}


function normalizeAirlineIdentity(...values: Array<string | null | undefined>) {
 const catalog = findAirlineCatalogEntry(...values);
 if (catalog) {
  const code = catalog.iata || catalog.icao || String(values[0] || '').toUpperCase();
  if (/^(LA|JJ|TAM)$/i.test(String(code)) || /LATAM|TAM/i.test(String(catalog.name || ''))) return { code: 'LA', label: 'LATAM', logo: '/assets/airlines/latam.png', badge: '/assets/airlines/latam-badge.png', missingLogo: false };
  return { code, label: catalog.name || code || 'Cia aérea', logo: catalog.logo || '/assets/airlines/crewcheck-airline.svg', badge: catalog.logo || '/assets/airlines/crewcheck-airline.svg', missingLogo: !catalog.logo };
 }
 const source = values.filter(Boolean).join(' ').toUpperCase();
 if (/\b(LA|JJ|TAM)\b/.test(source) || source.includes('LATAM') || source.includes('TAM')) return { code: 'LA', label: 'LATAM', logo: '/assets/airlines/latam.png', badge: '/assets/airlines/latam-badge.png', missingLogo: false };
 ...
 if (/\b(TTL|TOTAL)\b/.test(source) || source.includes('TOTAL')) return { code: 'TTL', label: 'Total', logo: '/assets/airlines/total.png', badge: '/assets/airlines/total-badge.png', missingLogo: false };
...
function AirlineLogoBadge({ code, name, compact = false }: { code?: string | null; name?: string | null; compact?: boolean }) {
 ...
 return (
  <span
   className={`crewcheck-airline-logo-badge crewcheck-radar-airline-logo inline-flex items-center justify-center rounded-2xl border border-slate-200 bg-white shadow-sm ring-1 ring-white/70 dark:border-white dark:bg-white ${compact ? 'is-compact h-12 min-w-24 max-w-[9.5rem] px-3' : 'h-14 min-w-32 max-w-[11rem] px-4'}`}
   title={identity.label}
   style={{ backgroundColor: '#ffffff', color: '#0f172a' }}
  >
   <img src={identity.logo} alt={identity.label} className={`crewcheck-airline-logo-img ${compact ? 'max-h-8 max-w-[7.2rem]' : 'max-h-10 max-w-[9rem]'} object-contain`} loading="lazy" style={{ filter: 'none', mixBlendMode: 'normal' }} />
  </span>
 );
}
...
function normalizeCodeshareCarrierClient(value?: string | null): string {
 const clean = String(value || '').toUpperCase().replace(/[^A-Z0-9]/g, '');
 if (!clean) return '';
 if (['JJ','TAM'].includes(clean)) return 'LA';
 if (clean === 'GLO') return 'G3';
 if (clean === 'AZU') return 'AD';
 if (clean === 'TAP') return 'TP';
 if (clean === 'CMP') return 'CM';
 if (clean === 'AAL') return 'AA';
 return clean.slice(0, 3);
}

function carrierFromFlightNumberClient(value?: string | null): string {
 return normalizeCodeshareCarrierClient(String(value || '').match(/^([A-Z]{1,3})\s*\d{2,5}/i)?.[1] || '');
}

function rowCarrierClient(row: AirportBoardRow): string {
 return normalizeCodeshareCarrierClient(row.airlineCode || carrierFromFlightNumberClient(row.flightNumber));
}

function isOriginalBrazilianCarrierClient(code: string): boolean {
 return ['LA','G3','AD','2Z','M3','TTL','SID'].includes(normalizeCodeshareCarrierClient(code));
}

function boardCodeshareKeyClient(row: AirportBoardRow): string {
 const type = String(row.type || '').toLowerCase();
 const origin = normalizeExtraAirport(row.origin);
 const destination = normalizeExtraAirport(row.destination || row.relatedAirport);
 const airport = normalizeExtraAirport((row as any).airport);
...
function boardExactFlightKeyClient(row: AirportBoardRow): string {
 const flight = String(row.flightNumber || '').replace(/\s+/g, '').toUpperCase();
 if (!flight) return '';
 return [
  String(row.type || '').toLowerCase(),
  normalizeExtraAirport((row as any).airport),
  normalizeExtraAirport(row.origin),
  normalizeExtraAirport(row.destination || row.relatedAirport),
  flight,
  String(row.confirmedTime || row.scheduledTime || '').slice(0, 5),
 ].join('|');
}

function boardRowQualityClient(row: AirportBoardRow): number {
 const carrier = rowCarrierClient(row);
 let score = 0;
 if (isOriginalBrazilianCarrierClient(carrier)) score += 70;
 if (/^(LA|G3|AD)\d{2,5}$/i.test(String(row.flightNumber || '').replace(/\s+/g, '').toUpperCase())) score += 30;
...
function mergeCodeshareRowClient(primary: AirportBoardRow, duplicate: AirportBoardRow): AirportBoardRow {
 const aliases = new Set([...(primary.codeshareFlights || []), ...(duplicate.codeshareFlights || [])].filter(Boolean));
 const primaryFlight = String(primary.flightNumber || '').replace(/\s+/g, '').toUpperCase();
 const duplicateFlight = String(duplicate.flightNumber || '').replace(/\s+/g, '').toUpperCase();
...
function dedupeAirportBoardCodeshareRowsClient(rows: AirportBoardRow[]): AirportBoardRow[] {
 const exact = new Map<string, AirportBoardRow>();
 for (const row of rows) {
  const key = boardExactFlightKeyClient(row);
  if (!key) {
   exact.set(`loose-${exact.size}`, row);
   continue;
  }
  const current = exact.get(key);
  if (!current) exact.set(key, row);
  else exact.set(key, boardRowQualityClient(row) > boardRowQualityClient(current) ? mergeCodeshareRowClient(row, current) : mergeCodeshareRowClient(current, row));
 }
 const grouped = new Map<string, AirportBoardRow>();
 for (const row of exact.values()) {
  const key = boardCodeshareKeyClient(row);
  if (!key) {
   grouped.set(`unique-${grouped.size}`, row);
   continue;
  }
  ...
  const chosen = boardRowQualityClient(row) > boardRowQualityClient(current) ? row : current;
  grouped.set(key, mergeCodeshareRowClient(chosen, chosen === row ? current : row));
 }
...
function cleanAirportMonitorRows(rows: AirportBoardRow[]): AirportBoardRow[] {
 ...
 return dedupeAirportBoardCodeshareRowsClient(cleaned)
...
function extraStaffLoadTone(load?: ExtraFlightStaffLoad | null): string {
 const risk = String(load?.riskLevel || '').toLowerCase();
 if (risk === 'low') return 'border-emerald-200 bg-emerald-50 text-emerald-950 dark:border-emerald-200/20 dark:bg-emerald-300/10 dark:text-emerald-50';
 if (risk === 'medium') return 'border-yellow-200 bg-yellow-50 text-yellow-950 dark:border-yellow-200/20 dark:bg-yellow-300/10 dark:text-yellow-50';
 if (risk === 'high') return 'border-orange-200 bg-orange-50 text-orange-950 dark:border-orange-200/20 dark:bg-orange-300/10 dark:text-orange-50';
 if (risk === 'critical') return 'border-rose-200 bg-rose-50 text-rose-950 dark:border-rose-200/20 dark:bg-rose-300/10 dark:text-rose-50';
 return 'border-slate-200 bg-white text-slate-950 dark:border-white/10 dark:bg-white/[0.055] dark:text-white';
}

function extraStaffLoadSymbol(load?: ExtraFlightStaffLoad | null): string {
 const risk = String(load?.riskLevel || '').toLowerCase();
 if (risk === 'critical') return '🔴';
 if (risk === 'high') return '🟠';
 if (risk === 'medium') return '🟡';
 if (risk === 'low') return '🟢';
 const score = Number(load?.chanceScore);
...
function extraStaffLoadSemanticLabel(load?: ExtraFlightStaffLoad | null): string {
 const risk = String(load?.riskLevel || '').toLowerCase();
 if (risk === 'low') return 'boa chance';
 if (risk === 'medium') return 'atenção';
 if (risk === 'high') return 'baixa chance';
 if (risk === 'critical') return 'muito cheio';
 return 'sem sinal';
}

function extraStaffLoadDisplay(load?: ExtraFlightStaffLoad | null) {
 if (!load) return { title: 'Previsão de assento', value: '—', sub: 'sem sinal de ocupação', score: '', symbol: '⚪', semantic: 'sem sinal' };
 const symbol = extraStaffLoadSymbol(load);
 const semantic = extraStaffLoadSemanticLabel(load);
 return {
  title: load.exactLoad ? 'Load oficial' : 'Previsão de assento',
  value: `${symbol} ${load.summary || load.statusLabel || load.chanceLabel || semantic}`,
  sub: load.seatsSignal || load.confidence || 'sinal auxiliar de ocupação',
  score: typeof load.chanceScore === 'number' ? `${load.chanceScore}%` : '',
  symbol,
  semantic,
 };
...
function ExtraFlightOptionCard({ item, index, expanded, onToggle, selected, onSelect, canSeeAdmin = false }: { item: ExtraFlightOptionEstimate; index: number; expanded: boolean; onToggle: () => void; selected?: boolean; onSelect?: () => void; canSeeAdmin?: boolean }) {
 ...
 const loadView = extraStaffLoadDisplay(item.staffLoad);
 return (
  <div role="button" tabIndex={0} onClick={onToggle} onKeyDown={(event) => { if (event.key === 'Enter' || event.key === ' ') { event.preventDefault(); onToggle(); } }} className={`group cursor-pointer rounded-[2rem] border bg-white p-4 text-slate-950 shadow-xl shadow-slate-950/5 transition hover:-translate-y-0.5 hover:shadow-2xl dark:border-white/10 dark:bg-white/[0.07] dark:text-white ${selected ? 'ring-2 ring-cyan-300/70 border-cyan-300/40' : 'border-slate-200/80'}`}>
   ...
   <div className="mt-5 grid grid-cols-2 gap-3">
    ...
    <div className={`min-h-[7rem] rounded-[1.6rem] border p-4 shadow-sm ${extraStaffLoadTone(item.staffLoad)}`}>
     <p className="text-[10px] font-black uppercase tracking-[0.28em] opacity-70">{loadView.title}</p>
     <p className="mt-4 text-2xl font-black leading-tight">{loadView.value}</p>
     <p className="mt-2 text-sm font-bold opacity-80">{loadView.sub}{loadView.score ? ` · ${loadView.score}` : ''}</p>
    </div>
    {canSeeAdmin ? <div className="min-h-[7rem] rounded-[1.6rem] border border-amber-200 bg-amber-50 p-4 shadow-sm dark:border-amber-200/20 dark:bg-amber-300/10">
     <p className="text-[10px] font-black uppercase tracking-[0.28em] text-amber-700 dark:text-amber-100">Sinal admin</p>
     <p className="mt-4 text-3xl font-black leading-tight text-amber-950 dark:text-amber-50">{occupation}</p>
     <p className="mt-2 text-sm font-bold text-amber-800/70 dark:text-amber-100/80">disponibilidade tarifária</p>
    </div> : null}
   </div>
   ...
   {expanded ? (
    <div className="mt-4 rounded-3xl border border-slate-200 bg-slate-50/80 p-4 dark:border-white/10 dark:bg-black/25">
     ...
     <div className="mt-3 grid gap-2 sm:grid-cols-3">
      ...
      <div className={`rounded-2xl border p-3 ${extraStaffLoadTone(item.staffLoad)}`}><p className="text-[10px] font-black uppercase tracking-[0.14em] opacity-70">previsão de assento</p><p className="mt-1 text-sm font-black">{loadView.value}</p><p className="mt-1 text-[11px] font-semibold leading-5 opacity-80">{item.staffLoad?.confidence || 'sem load oficial'}</p></div>
     </div>
     ...
     {item.staffLoad ? <div className={`mt-3 rounded-2xl border px-3 py-3 text-xs font-bold leading-5 ${extraStaffLoadTone(item.staffLoad)}`}>
      <p className="font-black uppercase tracking-[0.14em]">Vivo de Extra · previsão de assento</p>
      <p className="mt-1">{item.staffLoad.summary || item.staffLoad.statusLabel} · {item.staffLoad.confidence}</p>
      {item.staffLoad.details?.length ? <ul className="mt-2 list-disc space-y-1 pl-5">{item.staffLoad.details.slice(0, 4).map((line) => <li key={line}>{line}</li>)}</ul> : null}
      <p className="mt-2 opacity-80">{item.staffLoad.advisory}</p>
     </div> : null}
     {legs.length ? (
...
function FlightBoardScreen({ onBack, isAdmin = false }: { onBack: () => void; isAdmin?: boolean }) {
 ...
 const [boardDate, setBoardDate] = useState(() => localStorage.getItem('crewcheck_radar_board_date') || todayIsoForAirportClient('BSB'));
 ...
 const [trackFlight, setTrackFlight] = useState(() => localStorage.getItem('crewcheck_radar_live_flight') || '');
 const [liveTrack, setLiveTrack] = useState<AirLabsLiveTrackSnapshot | null>(null);
 const [liveTrackLoading, setLiveTrackLoading] = useState(false);
 const [liveTrackError, setLiveTrackError] = useState('');
 ...
 useEffect(() => {
  let alive = true;
  loadCrewCheckAviationCatalogs().then(() => { if (alive) setCatalogTick((value) => value + 1); }).catch(() => undefined);
  return () => { alive = false; };
 }, []);
 void catalogTick;
 ...
 const loadBoard = useCallback(async (forceRefresh = false) => {
  ...
  setError('');
  try {
   const selectedDate = boardDate || todayIsoForAirportClient(airport);
   localStorage.setItem('crewcheck_radar_board_date', selectedDate);
   const params = new URLSearchParams({ airport, type, limit: '100', diagnostics: '1', date: selectedDate, apiFirst: '1', timeBasis: 'airport-local', displayTimeZone: preferredSystemTimeZone(timeSettings) });
 ...
 const loadLiveTrack = useCallback(async (flightValue = trackFlight, silent = false) => {
  const flight = String(flightValue || '').replace(/\s+/g, '').toUpperCase();
  if (!flight) {
   setLiveTrackError('Informe o voo. Exemplo: LA3730.');
   return;
  }
  if (!silent) setLiveTrackLoading(true);
  setLiveTrackError('');
  try {
   localStorage.setItem('crewcheck_radar_live_flight', flight);
   const params = new URLSearchParams({ flightNumber: flight, displayTimeZone: preferredSystemTimeZone(timeSettings) });
   const response = await fetch(`/api/radar/live-track?${params.toString()}`, { cache: 'no-store', headers: crewcheckAuthHeader() });
   const payload = await response.json().catch(() => null) as AirLabsLiveTrackSnapshot | null;
   if (!response.ok || !payload) throw new Error('Radar ao vivo indisponível.');
   setLiveTrack(payload);
   if (!payload.ok) setLiveTrackError(payload.message || 'Voo não localizado ao vivo no momento.');
  } catch (error) {
   setLiveTrackError(error instanceof Error ? error.message : 'Não foi possível rastrear o voo ao vivo.');
  } finally {
   if (!silent) setLiveTrackLoading(false);
  }
 }, [trackFlight, timeSettings]);

 useEffect(() => {
  if (!liveTrack?.flightNumber || !liveTrack.ok) return;
  const timer = window.setInterval(() => void loadLiveTrack(liveTrack.flightNumber || '', true), 30_000);
  return () => window.clearInterval(timer);
 }, [liveTrack?.flightNumber, liveTrack?.ok, loadLiveTrack]);
  ...
  <div className="crewcheck-flightboard-screen min-h-screen overflow-x-hidden bg-[#061425] px-4 py-6 pb-32 text-white sm:px-6 lg:px-8">
   <div className="mx-auto max-w-7xl">
    <HeaderBack
     ...
     subtitle="monitores de aeroporto · portão · status · busca por data"
     onBack={onBack}
     ...
     <div className="border-b border-cyan-200/10 px-5 py-5 sm:px-7">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
       <div>
        ...
        <div className="mt-3 flex flex-wrap gap-2">
         <span className="rounded-full border border-cyan-200/20 bg-cyan-300/10 px-3 py-1 text-[0.68rem] font-black uppercase tracking-[0.16em] text-cyan-50">Voos por data · portão quando disponível</span>
       ...
       <div className="grid grid-cols-2 gap-2 sm:flex sm:flex-wrap">
        ...
        <input type="date" value={boardDate} onChange={(event) => { setBoardDate(event.target.value); localStorage.setItem('crewcheck_radar_board_date', event.target.value); }} title="Pesquisar voos em outros dias" className="h-12 rounded-2xl border border-white/10 bg-white/10 px-4 text-sm font-black text-white outline-none backdrop-blur-xl" />
...
function LiveFlightPrecisionPanel({ track, loading }: { track: AirLabsLiveTrackSnapshot | null; loading?: boolean }) {
 const hasAircraft = Boolean(track?.registration || track?.aircraftIcao);
 const hasEta = Boolean(track?.estimatedArrival || track?.confirmedTime || track?.scheduledTime);
 const hasLive = Boolean(track?.ok && (track?.altitudeFt || track?.speedKt || track?.headingDeg));
 return (
  <div className="rounded-3xl border border-cyan-200/15 bg-[linear-gradient(135deg,rgba(8,47,73,.75),rgba(2,6,23,.95))] p-4 shadow-inner shadow-black/30">
   <div className="flex flex-wrap items-start justify-between gap-3">
    <div>
     <p className="text-[0.68rem] font-black uppercase tracking-[0.18em] text-cyan-100/55">Precisão operacional</p>
     <h4 className="text-2xl font-black text-white">{track?.flightNumber || 'Informe o voo'}</h4>
     <p className="mt-1 text-xs font-semibold leading-5 text-cyan-50/65">Sem mapa: foco em matrícula, status e pouso estimado para melhorar a próxima programação.</p>
    </div>
    <span className={`rounded-full border px-3 py-1 text-[10px] font-black uppercase tracking-[0.14em] ${hasAircraft || hasEta ? 'border-emerald-200/30 bg-emerald-300/15 text-emerald-50' : 'border-amber-200/30 bg-amber-300/15 text-amber-50'}`}>{loading ? 'Atualizando' : hasAircraft || hasEta ? 'Dados úteis' : 'Aguardando'}</span>
   </div>
   <div className="mt-4 grid grid-cols-2 gap-2 text-xs sm:grid-cols-4">
    <BoardPill label="Matrícula" value={track?.registration || '—'} />
    <BoardPill label="Aeronave" value={track?.aircraftIcao || '—'} />
    <BoardPill label="Pouso estimado" value={track?.landingCountdown || track?.estimatedArrival || track?.confirmedTime || '—'} />
    <BoardPill label="Portão" value={track?.gate || '—'} />
    <BoardPill label="Terminal" value={track?.terminal || '—'} />
    <BoardPill label="Altitude" value={track?.altitudeFt != null ? `${Number(track.altitudeFt).toLocaleString('pt-BR')} ft` : '—'} />
    <BoardPill label="Velocidade" value={track?.speedKt != null ? `${Math.round(Number(track.speedKt))} kt` : '—'} />
    <BoardPill label="Fonte" value={track?.source || 'AirLabs'} />
   </div>
   <p className="mt-4 rounded-2xl border border-white/10 bg-white/[0.045] px-3 py-2 text-xs font-semibold leading-5 text-cyan-50/70">
    {hasLive ? 'ADS-B ao vivo disponível para validar se a aeronave já está em voo.' : 'Quando a aeronave ainda está no solo ou fora de cobertura, o CrewCheck usa schedule/status AirLabs como estimativa conservadora.'}
   </p>
  </div>
 );
}

function LiveRadarMap({ track, loading }: { track: AirLabsLiveTrackSnapshot | null; loading?: boolean }) {
 const hasPosition = Boolean(track?.ok && typeof track.lat === 'number' && typeof track.lng === 'number');
 const left = hasPosition ? Math.max(3, Math.min(97, (((Number(track?.lng) + 180) / 360) * 100))) : 50;
 const top = hasPosition ? Math.max(8, Math.min(92, (((90 - Number(track?.lat)) / 180) * 100))) : 50;
 const heading = Number(track?.headingDeg || 0);
 return (
  <div className="relative min-h-[18rem] overflow-hidden rounded-3xl border border-cyan-200/15 bg-[radial-gradient(circle_at_20%_20%,rgba(34,211,238,.18),transparent_28%),linear-gradient(135deg,rgba(8,47,73,.65),rgba(2,6,23,.95))] p-4 shadow-inner shadow-black/30">
   <div className="absolute inset-0 opacity-35" style={{ backgroundImage: 'linear-gradient(rgba(103,232,249,.14) 1px, transparent 1px), linear-gradient(90deg, rgba(103,232,249,.14) 1px, transparent 1px)', backgroundSize: '36px 36px' }} />
   <div className="relative z-10 flex items-start justify-between gap-3">
    <div>
     <p className="text-[0.68rem] font-black uppercase tracking-[0.18em] text-cyan-100/55">Mapa tático</p>
     <h4 className="text-xl font-black text-white">{hasPosition ? `${Number(track?.lat).toFixed(3)}, ${Number(track?.lng).toFixed(3)}` : 'Aguardando coordenadas'}</h4>
    </div>
    <span className={`rounded-full border px-3 py-1 text-[10px] font-black uppercase tracking-[0.14em] ${hasPosition ? 'border-emerald-200/30 bg-emerald-300/15 text-emerald-50' : 'border-amber-200/30 bg-amber-300/15 text-amber-50'}`}>{loading ? 'Atualizando' : hasPosition ? 'Ao vivo' : 'Sem posição'}</span>
   </div>
   {hasPosition ? <div className="absolute z-20 flex -translate-x-1/2 -translate-y-1/2 flex-col items-center gap-2" style={{ left: `${left}%`, top: `${top}%` }}>
    <div className="flex h-14 w-14 items-center justify-center rounded-full border border-cyan-100/40 bg-cyan-300 text-[#061425] shadow-[0_0_35px_rgba(34,211,238,.65)]" style={{ transform: `rotate(${heading}deg)` }}><Plane className="h-7 w-7" /></div>
    <span className="rounded-full bg-black/60 px-3 py-1 text-xs font-black text-white backdrop-blur">{track?.flightNumber}</span>
   </div> : <div className="absolute inset-0 z-10 flex items-center justify-center text-center text-sm font-bold leading-6 text-cyan-50/65">O voo pode ainda estar no solo, fora da cobertura ADS-B ou indisponível no plano atual.</div>}
   <div className="relative z-10 mt-24 grid grid-cols-2 gap-2 text-xs sm:grid-cols-4">
    <BoardPill label="Altitude" value={track?.altitudeFt != null ? `${Number(track.altitudeFt).toLocaleString('pt-BR')} ft` : '—'} />
    <BoardPill label="Velocidade" value={track?.speedKt != null ? `${Math.round(Number(track.speedKt))} kt` : '—'} />
    <BoardPill label="Rumo" value={track?.headingDeg != null ? `${Math.round(Number(track.headingDeg))}°` : '—'} />
    <BoardPill label="Matrícula" value={track?.registration || '—'} />
   </div>
  </div>
 );
...
function AirportBoardDesktopRow({ row, type }: { row: AirportBoardRow; type: FlightBoardType }) {
 ...
 return (
  <div className="grid grid-cols-[7rem_11rem_minmax(12rem,1fr)_7rem_6rem_7rem_7rem_minmax(10rem,1fr)] items-center border-b border-cyan-200/5 bg-cyan-950/20 px-5 py-3 text-xl font-black text-white odd:bg-white/[0.035]">
   <span>
    <span className="block">{displayFlightNumberForLogo(row.flightNumber, row.airlineCode, row.airlineName)}</span>
    {row.registration ? <span className="mt-0.5 block text-[10px] font-black uppercase tracking-[0.12em] text-cyan-100/65">{row.registration}</span> : null}
    {row.codeshareHiddenCount ? <span className="mt-0.5 block text-[9px] font-black uppercase tracking-[0.12em] text-cyan-100/45">+{row.codeshareHiddenCount} codeshare oculto</span> : null}
   </span>
...
function AirportBoardMobileCard({ row, type }: { row: AirportBoardRow; type: FlightBoardType }) {
 ...
 return (
  <div className="rounded-3xl border border-cyan-200/10 bg-white/[0.07] p-4 shadow-xl shadow-black/20">
   ...
   {(row.registration || row.codeshareHiddenCount) ? <div className="mt-2 grid grid-cols-2 gap-2"><BoardPill label="Matrícula" value={row.registration || '—'} /><BoardPill label="Codeshare" value={row.codeshareHiddenCount ? `+${row.codeshareHiddenCount} oculto(s)` : '—'} /></div> : null}
...
function HistoryScreen({ rosters, loading, dbStatus, openingId, onBack, onOpen, onActivate, onDelete, onRefresh }: {
 ...
 return (
  <div className="min-h-screen bg-[#08111f] px-5 py-8 text-white">
   <div className="mx-auto max-w-5xl">
    <HeaderBack title="Gerenciador de Escalas" subtitle={dbOnline ? 'Escala ativa, histórico no banco atual e backup opcional' : 'Histórico local/offline'} onBack={onBack} action={<button onClick={onRefresh} className="rounded-2xl border border-white/10 bg-white/10 px-4 py-2 text-xs font-black text-cyan-100"><RefreshCw className="mr-2 inline h-4 w-4" />Atualizar</button>} />
    <div className="mt-5 grid gap-3 md:grid-cols-3">
     ...
     <div className="rounded-[1.4rem] border border-emerald-200/15 bg-emerald-300/10 p-4">
      <p className="text-[10px] font-black uppercase tracking-[0.22em] text-emerald-100/70">Backup Storage</p>
      <h3 className="mt-2 text-lg font-black text-white">{storage?.configured ? 'Supabase Storage' : 'opcional'}</h3>
      <p className="mt-1 text-xs font-semibold text-emerald-50/70">{storage?.configured ? 'Banco atual segue como principal; backup privado prioriza Premium e limita o grátis.' : 'Banco atual ativo. Supabase entra apenas como backup/redundância se configurado.'}</p>
     </div>
         ...
         <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
          <div className="min-w-0">
           <div className="flex flex-wrap items-center gap-2">
            ...
            {item.storageReady && <span className="rounded-full bg-emerald-300/15 px-2.5 py-1 text-[10px] font-black uppercase tracking-[0.14em] text-emerald-100">Backup</span>}
            {String(item.importStatus || '') === 'processed_backup_premium_only' && <span className="rounded-full bg-violet-300/15 px-2.5 py-1 text-[10px] font-black uppercase tracking-[0.14em] text-violet-100">Backup Premium</span>}
            {['processed_storage_pending','processed_backup_pending'].includes(String(item.importStatus || '')) && <span className="rounded-full bg-amber-300/15 px-2.5 py-1 text-[10px] font-black uppercase tracking-[0.14em] text-amber-100">Backup pendente</span>}
           </div>
...
function WakeupPremiumPanel({ profileSettings, onProfileSettingsChange, billingStatus, canAccessAdmin, hasRoster, onBack, onOpenImport }: WakeupPremiumPanelProps) {
 const isPremium = Boolean(billingStatus?.premiumAccess || canAccessAdmin);
 const isTrialPremium = Boolean(billingStatus?.premiumAccess && String(billingStatus?.status || '').toLowerCase() === 'trialing' && !canAccessAdmin);
 const initialCountry = crewCountryByIso(profileSettings.phoneCountryIso || 'BR');
 const [countryIso, setCountryIso] = useState(initialCountry.iso);
 const [phoneLocal, setPhoneLocal] = useState(profileSettings.phoneLocal || splitCrewPhoneForProfile(profileSettings.phoneE164, profileSettings.phoneCountryIso, profileSettings.phoneCountryCode).phoneLocal || '');
 const [notificationMode, setNotificationMode] = useState('call_telegram');
 const [autoWakeup, setAutoWakeup] = useState(true);
 const [leadMinutes, setLeadMinutes] = useState('120');
 const [snoozeMinutes, setSnoozeMinutes] = useState('10');
 const [maxCalls, setMaxCalls] = useState('2');
 const [callAfterSnoozes, setCallAfterSnoozes] = useState('2');
 const [airport, setAirport] = useState('');
 ...
 const [shareWithEmergency, setShareWithEmergency] = useState(false);
 ...
 const [shareHotelRoom, setShareHotelRoom] = useState(false);
 const [visitorEnabled, setVisitorEnabled] = useState(false);
 const [busy, setBusy] = useState(false);
 const [statusText, setStatusText] = useState('');
 const [whatsappStatus, setWhatsappStatus] = useState<WhatsAppConciergeStatus | null>(null);
 const whatsappAllowed = Boolean(isPremium && (whatsappStatus?.allowed !== false));
 const whatsappTrial = whatsappStatus?.trial;
 const whatsappTrialLabel = isTrialPremium && whatsappTrial ? `${whatsappTrial.remainingDaily ?? '—'}/${whatsappTrial.dailyLimit ?? '—'} hoje · ${whatsappTrial.remainingTotal ?? '—'}/${whatsappTrial.totalLimit ?? '—'} no teste` : '';
 const country = crewCountryByIso(countryIso);
 const phoneE164 = composeCrewPhoneE164(country.code, phoneLocal);
 const learnedLead = useMemo(() => {
  const a = normalizeCrewClockClient(departureTime);
  const b = normalizeCrewClockClient(presentationTime);
  if (!a || !b) return Number(leadMinutes) || 120;
  const [ah, am] = a.split(':').map(Number);
  const [bh, bm] = b.split(':').map(Number);
  let diff = (ah * 60 + am) - (bh * 60 + bm);
  if (diff < 0) diff += 1440;
  return diff || Number(leadMinutes) || 120;
 }, [departureTime, presentationTime, leadMinutes]);

 useEffect(() => {
  fetch('/api/telegram/status', { headers: crewcheckAuthHeader(), cache: 'no-store' }).then((r) => r.json()).then((payload) => {
   const prefs = payload?.preferences || {};
   if (prefs.wakeupNotificationMode) setNotificationMode(String(prefs.wakeupNotificationMode));
   if (prefs.wakeupAutoEnabled !== undefined) setAutoWakeup(prefs.wakeupAutoEnabled !== false && prefs.wakeupAutoEnabled !== 'false');
   if (prefs.wakeupAutoLeadMinutes) setLeadMinutes(String(prefs.wakeupAutoLeadMinutes));
   if (prefs.overnightWakeupSnoozeMinutes) setSnoozeMinutes(String(prefs.overnightWakeupSnoozeMinutes));
   if (prefs.wakeupMaxCalls) setMaxCalls(String(prefs.wakeupMaxCalls));
   if (prefs.wakeupCallAfterSnoozes) setCallAfterSnoozes(String(prefs.wakeupCallAfterSnoozes));
   const savedPhone = prefs.userPhone || prefs.infobipPhone || profileSettings.phoneE164;
   if (savedPhone && !phoneLocal) setPhoneLocal(splitCrewPhoneForProfile(savedPhone, countryIso, country.code).phoneLocal);
  }).catch(() => null);
  fetch('/api/emergency-contact', { headers: crewcheckAuthHeader(), cache: 'no-store' }).then((r) => r.json()).then((payload) => {
   const c = payload?.contact;
   if (!c) return;
   setEmergencyName(c.display_name || c.displayName || '');
   setEmergencyRelation(c.relation || 'Cônjuge');
   setEmergencyEmail(c.email || '');
   setEmergencyPhone(c.phone_e164 || c.phoneE164 || '');
   setEmergencyTelegram(c.telegram_username || c.telegramUsername || '');
   setEmergencyWhatsapp(c.whatsapp_phone || c.whatsappPhone || '');
   setVisitorEnabled(Boolean(c.visitor_enabled || c.visitorEnabled));
   setShareRoster(Boolean(c.share_roster || c.shareRoster));
   setShareHotelRoom(Boolean(c.share_hotel_room || c.shareHotelRoom));
  }).catch(() => null);
  fetch('/api/whatsapp/concierge/status', { headers: crewcheckAuthHeader(), cache: 'no-store' }).then((r) => r.json()).then(setWhatsappStatus).catch(() => null);
 // eslint-disable-next-line react-hooks/exhaustive-deps
 }, []);

 async function savePhoneAndPreferences(testCall = false) {
  if (!phoneE164 || normalizeCrewPhoneDigits(phoneE164).length < 10) {
   toast.error('Informe um telefone válido com DDD/local. O país adiciona o código internacional.');
   return;
  }
  setBusy(true);
  try {
   await fetch('/api/profile/contact', { method: 'POST', headers: crewcheckAuthHeader({ 'content-type': 'application/json' }), body: JSON.stringify({ phoneCountryIso: country.iso, phoneCountryCode: country.code, phoneLocal, phoneE164 }) }).then(async (r) => { const p = await r.json().catch(() => null); if (!r.ok || p?.ok === false) throw new Error(p?.message || 'Falha ao salvar telefone.'); });
   onProfileSettingsChange({ ...profileSettings, phoneCountryIso: country.iso, phoneCountryCode: country.code, phoneLocal: normalizeCrewPhoneDigits(phoneLocal), phoneE164 });
   await fetch('/api/telegram/preferences', { method: 'POST', headers: crewcheckAuthHeader({ 'content-type': 'application/json' }), body: JSON.stringify({ overnightWakeup: true, wakeupCallEnabled: true, wakeupCallProvider: 'infobip', wakeupNotificationMode: notificationMode, overnightWakeupSnoozeMinutes: snoozeMinutes, wakeupMaxCalls: maxCalls, wakeupCallAfterSnoozes: callAfterSnoozes, wakeupMaxSnoozes: '6', wakeupAutoEnabled: autoWakeup, wakeupAutoLeadMinutes: leadMinutes, infobipPhone: phoneE164, twilioPhone: phoneE164, userPhone: phoneE164, phoneE164, userPhoneCountryIso: country.iso, userPhoneCountryCode: country.code, whatsappConcierge: true, whatsappAudioReplies: true }) });
   if (testCall) {
    const response = await fetch('/api/telegram/wakeup/test', { method: 'POST', headers: crewcheckAuthHeader({ 'content-type': 'application/json' }), body: JSON.stringify({ forceProvider: 'infobip', phone: phoneE164, presentationTime: presentationTime || '06:00', destination: airport || 'próxima programação', timeZone: preferredSystemTimeZone(profileSettings) }) });
    const payload = await response.json().catch(() => null);
    setStatusText(payload?.message || payload?.detail || 'Teste enviado para o canal de ligação premium.');
    if (!response.ok || payload?.ok === false) throw new Error(payload?.message || payload?.detail || 'Não foi possível iniciar o teste de ligação.');
   }
   toast.success(testCall ? 'Teste de ligação iniciado.' : 'Despertador Premium salvo.');
  } catch (error) {
   toast.error(error instanceof Error ? error.message : 'Não foi possível salvar o Despertador Premium.');
  } finally {
   setBusy(false);
  }
 }

 async function saveLayover() {
  if (!airport || !presentationTime) { toast.error('Informe aeroporto e apresentação para o CrewCheck aprender.'); return; }
  setBusy(true);
  try {
   const response = await fetch('/api/layover/details', { method: 'POST', headers: crewcheckAuthHeader({ 'content-type': 'application/json' }), body: JSON.stringify({ rosterDate, airport, city, hotelName, roomNumber, presentationTime, departureTime, leadMinutes: learnedLead, shareWithEmergency, timeZone: preferredSystemTimeZone(profileSettings) }) });
   const payload = await response.json().catch(() => null);
   if (!response.ok || payload?.ok === false) throw new Error(payload?.message || 'Não foi possível salvar o pernoite.');
   setStatusText(`Aprendizado salvo: ${airport} · ${hotelName || 'hotel não informado'} · ${learnedLead} min antes da decolagem.`);
   toast.success('Pernoite, quarto e apresentação salvos.');
  } catch (error) { toast.error(error instanceof Error ? error.message : 'Falha ao salvar pernoite.'); }
 ...
 async function saveEmergencyContact() {
  setBusy(true);
  try {
   const response = await fetch('/api/emergency-contact', { method: 'POST', headers: crewcheckAuthHeader({ 'content-type': 'application/json' }), body: JSON.stringify({ displayName: emergencyName, relation: emergencyRelation, email: emergencyEmail, phone: emergencyPhone, telegram: emergencyTelegram, whatsappPhone: emergencyWhatsapp, visitorEnabled, shareRoster, shareHotelRoom, shareWakeupStatus: true }) });
   const payload = await response.json().catch(() => null);
   if (!response.ok || payload?.ok === false) throw new Error(payload?.message || 'Não foi possível salvar o contato.');
   toast.success('Contato de emergência/visitante salvo.');
  } catch (error) { toast.error(error instanceof Error ? error.message : 'Falha ao salvar contato.'); }
  finally { setBusy(false); }
 }

 async function openSystemWhatsApp() {
  if (!isPremium) { toast.error('WhatsApp Concierge é exclusivo do Premium.'); return; }
  try {
   const response = await fetch('/api/whatsapp/concierge/link', { headers: crewcheckAuthHeader(), cache: 'no-store' });
   const payload = await response.json().catch(() => null);
   if (!response.ok || payload?.ok === false || !payload?.url) throw new Error(payload?.message || 'WhatsApp ainda não configurado.');
   window.open(payload.url, '_blank', 'noopener,noreferrer');
  } catch (error) { toast.error(error instanceof Error ? error.message : 'Não foi possível abrir o WhatsApp.'); }
 }

 async function openRosterWhatsAppLink() {
  if (!isPremium) { toast.error('Envio de escala pelo WhatsApp é exclusivo do Premium.'); return; }
  setBusy(true);
  try {
   const response = await fetch('/api/whatsapp/roster/link', { headers: crewcheckAuthHeader(), cache: 'no-store' });
   const payload = await response.json().catch(() => null);
   if (!response.ok || payload?.ok === false || !payload?.url) throw new Error(payload?.message || 'Não foi possível preparar a escala para WhatsApp.');
   window.open(payload.url, '_blank', 'noopener,noreferrer');
   toast.success('WhatsApp aberto com resumo da escala.');
  } catch (error) { toast.error(error instanceof Error ? error.message : 'Falha ao abrir escala no WhatsApp.'); }
  finally { setBusy(false); }
 }


 async function openWhatsAppImportLink() {
  if (!isPremium) { toast.error('Importação de escala pelo WhatsApp é exclusiva do Premium.'); return; }
  setBusy(true);
  try {
   const response = await fetch('/api/whatsapp/import/link', { headers: crewcheckAuthHeader(), cache: 'no-store' });
   const payload = await response.json().catch(() => null);
   if (!response.ok || payload?.ok === false || !payload?.url) throw new Error(payload?.message || 'Não foi possível abrir importação por WhatsApp.');
   window.open(payload.url, '_blank', 'noopener,noreferrer');
   toast.success('WhatsApp aberto. Anexe o PDF original da escala nessa conversa.');
  } catch (error) { toast.error(error instanceof Error ? error.message : 'Falha ao abrir importação pelo WhatsApp.'); }
  finally { setBusy(false); }
 }

 async function sendRosterWhatsAppByApi() {
  if (!isPremium) { toast.error('Envio automático por WhatsApp é exclusivo do Premium.'); return; }
  if (!phoneE164) { toast.error('Salve seu telefone no Perfil/Despertador Premium antes de enviar por API.'); return; }
  setBusy(true);
  try {
   const response = await fetch('/api/whatsapp/roster/send', { method: 'POST', headers: crewcheckAuthHeader({ 'content-type': 'application/json' }), body: JSON.stringify({ to: phoneE164 }) });
   const payload = await response.json().catch(() => null);
   if (!response.ok || payload?.ok === false) throw new Error(payload?.message || 'Não foi possível enviar por WhatsApp.');
   setWhatsappStatus((current) => current ? { ...current, trial: payload?.trial || current.trial } : current);
   toast.success(payload?.message || 'Escala enviada por WhatsApp.');
  } catch (error) { toast.error(error instanceof Error ? error.message : 'Falha no envio automático por WhatsApp.'); }
  finally { setBusy(false); }
 }

 return (
  <div className="min-h-screen bg-[color:var(--crew-bg)] px-4 pb-24 pt-4 text-[color:var(--crew-text)] md:px-6 lg:px-8">
   <HeaderBack title="Despertador Premium" subtitle="Ligação, Telegram/WhatsApp, hotel, quarto e aprendizado por apresentação." onBack={onBack} action={<Button onClick={() => savePhoneAndPreferences(false)} disabled={busy} className="rounded-2xl font-black">Salvar</Button>} />
   {!isPremium && <div className="mb-4 rounded-[1.5rem] border border-amber-300/30 bg-amber-300/10 p-4 text-sm font-semibold text-amber-100">Este módulo é Premium. Você pode configurar o telefone agora; a automação completa entra quando o Premium estiver ativo.</div>}
   <div className="grid gap-4 xl:grid-cols-[1fr_.9fr]">
    <div className="space-y-4">
     <SettingsCategoryCard eyebrow="Contato seguro" title="Telefone do despertar inteligente" description="O número fica no perfil do usuário e é usado somente para alertas autorizados.">
      <div className="grid gap-3 md:grid-cols-[.85fr_1.15fr]">
       <label className="text-xs font-black uppercase tracking-[0.16em] text-[color:var(--crew-muted)]">País<select value={countryIso} onChange={(e) => { const next = crewCountryByIso(e.target.value); setCountryIso(next.iso); }} className="crewcheck-premium-input mt-2 w-full"><option value="BR">Brasil +55</option>{CREWCHECK_COUNTRY_PHONE_OPTIONS.filter((item) => item.iso !== 'BR').map((item) => <option key={item.iso} value={item.iso}>{item.name} {item.code}</option>)}</select></label>
       <ProfileField label="Telefone com DDD/local" value={phoneLocal} placeholder={country.example} onChange={(value) => setPhoneLocal(normalizeCrewPhoneDigits(value))} />
      </div>
      <p className="mt-3 rounded-2xl bg-black/10 px-3 py-2 text-xs font-bold text-[color:var(--crew-muted)]">Será salvo como <strong>{phoneE164 || `${country.code} ...`}</strong>. Configurações técnicas do canal de ligação ficam ocultas do usuário.</p>
      <div className="mt-3 flex flex-wrap gap-2"><Button type="button" onClick={() => savePhoneAndPreferences(false)} disabled={busy}>Salvar telefone</Button><Button type="button" variant="outline" onClick={() => savePhoneAndPreferences(true)} disabled={busy || !isPremium}><PhoneCall className="h-4 w-4" /> Testar ligação Premium</Button></div>
     </SettingsCategoryCard>

     <SettingsCategoryCard eyebrow="Automação" title="Despertar baseado na apresentação" description="Quando autorizado, o CrewCheck calcula o despertar pela hora de apresentação e aprende o padrão por hotel/aeroporto.">
      <div className="grid gap-3 md:grid-cols-2">
       <TogglePill enabled={autoWakeup} onClick={() => setAutoWakeup((v) => !v)} label="Despertar automático" description={autoWakeup ? 'Ativo quando houver apresentação' : 'Manual'} />
       <label className="text-xs font-black uppercase tracking-[0.16em] text-[color:var(--crew-muted)]">Como notificar<select value={notificationMode} onChange={(e) => setNotificationMode(e.target.value)} className="crewcheck-premium-input mt-2 w-full"><option value="call_telegram">Ligação + Telegram</option><option value="call_only">Somente ligação</option><option value="telegram_only">Somente Telegram</option></select></label>
       <ProfileField label="Antecedência padrão (min)" value={leadMinutes} placeholder="120" onChange={(value) => setLeadMinutes(normalizeCrewPhoneDigits(value).slice(0, 3))} />
       <ProfileField label="Soneca (min)" value={snoozeMinutes} placeholder="10" onChange={(value) => setSnoozeMinutes(normalizeCrewPhoneDigits(value).slice(0, 2))} />
       <ProfileField label="Máximo de ligações" value={maxCalls} placeholder="2" onChange={(value) => setMaxCalls(normalizeCrewPhoneDigits(value).slice(0, 1))} />
       <ProfileField label="Ligar após nº de sonecas" value={callAfterSnoozes} placeholder="2" onChange={(value) => setCallAfterSnoozes(normalizeCrewPhoneDigits(value).slice(0, 1))} />
      </div>
     </SettingsCategoryCard>

     <SettingsCategoryCard eyebrow="Pernoite" title="Hotel, quarto e apresentação" description="Informe no card de pernoite para o CrewCheck aprender a diferença entre apresentação e decolagem por localidade e hotel.">
      <div className="grid gap-3 md:grid-cols-3"><ProfileField label="Data" value={rosterDate} placeholder="2026-07-02" onChange={setRosterDate} /><ProfileField label="Aeroporto" value={airport} placeholder="SLZ" uppercase onChange={(v) => setAirport(v.slice(0,3))} /><ProfileField label="Cidade" value={city} placeholder="São Luís" onChange={setCity} /></div>
      <div className="mt-3 grid gap-3 md:grid-cols-2"><ProfileField label="Hotel" value={hotelName} placeholder="Nome do hotel" onChange={setHotelName} /><ProfileField label="Quarto" value={roomNumber} placeholder="Ex.: 1204" onChange={setRoomNumber} /></div>
      <div className="mt-3 grid gap-3 md:grid-cols-3"><ProfileField label="Apresentação" value={presentationTime} placeholder="04:45" onChange={(v) => setPresentationTime(v.slice(0,5))} /><ProfileField label="Decolagem" value={departureTime} placeholder="06:45" onChange={(v) => setDepartureTime(v.slice(0,5))} /><div className="rounded-2xl border border-white/10 bg-white/[.05] p-4"><p className="text-xs font-black uppercase tracking-[0.16em] text-[color:var(--crew-muted)]">Aprendizado</p><p className="mt-2 text-xl font-black">{learnedLead} min</p><p className="text-xs text-[color:var(--crew-muted)]">apresentação → decolagem</p></div></div>
      <div className="mt-3"><TogglePill enabled={shareWithEmergency} onClick={() => setShareWithEmergency((v) => !v)} label="Compartilhar hotel/quarto com contato autorizado" description="Somente quando o visitante Premium estiver habilitado." /></div>
      <div className="mt-3 flex flex-wrap gap-2"><Button type="button" onClick={saveLayover} disabled={busy}>Salvar pernoite e ensinar sistema</Button>{!hasRoster && <Button type="button" variant="outline" onClick={onOpenImport}>Importar escala</Button>}</div>
     </SettingsCategoryCard>
    </div>

    <div className="space-y-4">
     <SettingsCategoryCard eyebrow="Emergência" title="Contato de emergência / visitante" description="Base para acesso visitante futuro, com autorização explícita do usuário sobre o que poderá ser visto.">
      <div className="grid gap-3 md:grid-cols-2"><ProfileField label="Como chamar" value={emergencyName} placeholder="Nome" onChange={setEmergencyName} /><label className="text-xs font-black uppercase tracking-[0.16em] text-[color:var(--crew-muted)]">Relação<select value={emergencyRelation} onChange={(e) => setEmergencyRelation(e.target.value)} className="crewcheck-premium-input mt-2 w-full">{['Cônjuge','Pai','Mãe','Amigo','Namorado','Namorada','Outro'].map((item) => <option key={item} value={item}>{item}</option>)}</select></label></div>
      <div className="mt-3 grid gap-3"><ProfileField label="E-mail para criar visitante" value={emergencyEmail} placeholder="email@exemplo.com" onChange={setEmergencyEmail} /><ProfileField label="Telefone de emergência" value={emergencyPhone} placeholder="+5561..." onChange={setEmergencyPhone} /><ProfileField label="Telegram" value={emergencyTelegram} placeholder="usuario" onChange={(v) => setEmergencyTelegram(v.replace(/^@+/, ''))} /><ProfileField label="WhatsApp" value={emergencyWhatsapp} placeholder="+5561..." onChange={setEmergencyWhatsapp} /></div>
      <div className="mt-3 grid gap-2"><TogglePill enabled={visitorEnabled} onClick={() => setVisitorEnabled((v) => !v)} label="Acesso visitante simplificado" description="Preparado para liberar uma tela reduzida no futuro." /><TogglePill enabled={shareRoster} onClick={() => setShareRoster((v) => !v)} label="Compartilhar escala" description="Visitante vê somente informações autorizadas." /><TogglePill enabled={shareHotelRoom} onClick={() => setShareHotelRoom((v) => !v)} label="Compartilhar hotel e quarto" description="Útil para emergência e acompanhamento familiar." /></div>
      <Button type="button" onClick={saveEmergencyContact} disabled={busy} className="mt-3 w-full">Salvar contato autorizado</Button>
     </SettingsCategoryCard>

     <SettingsCategoryCard eyebrow="Concierge Premium" title="Telegram e WhatsApp" description="WhatsApp fica restrito ao Premium porque a API é paga. Chaves, provedor e limites técnicos permanecem somente no servidor.">
      <div className="grid gap-3">
       <StatusTile icon={MessageCircle} title="Telegram Concierge" text="Escala, hotel e lembretes por texto/áudio, conforme permissão do usuário. Status: configurável." />
       <StatusTile icon={MessageCircle} title={whatsappAllowed ? "WhatsApp Concierge Premium" : "WhatsApp bloqueado no plano grátis"} text={`${whatsappStatus?.message || 'Canal preparado para responder escala, hotel e áudio quando configurado no Render.'} ${whatsappStatus?.configured ? `Número: ${whatsappStatus.numberMasked || ''}.` : 'Aguardando número do sistema no Render.'} ${whatsappStatus?.canImportRosterByWhatsApp ? 'Importação por PDF habilitada.' : 'Importação por PDF depende do webhook do provedor.'} ${whatsappTrialLabel ? `Limite do teste: ${whatsappTrialLabel}.` : ''}`} />
      </div>
      {!isPremium && <div className="mt-3 rounded-2xl border border-amber-300/25 bg-amber-300/10 p-3 text-xs font-bold leading-5 text-amber-50">WhatsApp, áudio e envio de escala por API ficam liberados apenas no Premium. O Telegram permanece como canal principal do plano grátis quando configurado.</div>}
      {isTrialPremium && <div className="mt-3 rounded-2xl border border-cyan-300/25 bg-cyan-300/10 p-3 text-xs font-bold leading-5 text-cyan-50">Durante os 7 dias grátis do Premium, o WhatsApp tem limite para proteger o custo da API: {whatsappTrialLabel || 'uso controlado pelo servidor'}.</div>}
      <div className="mt-3 grid gap-2 sm:grid-cols-2">
       <Button type="button" variant="outline" onClick={openSystemWhatsApp} disabled={!whatsappAllowed || busy}>Adicionar/Abrir WhatsApp do CrewCheck</Button>
       <Button type="button" variant="outline" onClick={openWhatsAppImportLink} disabled={!whatsappAllowed || busy || !whatsappStatus?.canImportRosterByWhatsApp}>Importar escala pelo WhatsApp</Button>
       <Button type="button" variant="outline" onClick={openRosterWhatsAppLink} disabled={!whatsappAllowed || busy || !hasRoster}>Abrir WhatsApp com resumo da escala</Button>
       <Button type="button" onClick={sendRosterWhatsAppByApi} disabled={!whatsappAllowed || busy || !hasRoster || !whatsappStatus?.apiConfigured}>Enviar escala pela API Premium</Button>
      </div>
      <p className="mt-3 text-xs font-semibold leading-5 text-[color:var(--crew-muted)]">Para importar, abra o WhatsApp do CrewCheck e anexe o PDF original. O app só recebe automaticamente se o provedor de WhatsApp encaminhar mensagens/mídia para o webhook do CrewCheck. O link direto usa wa.me e não mostra chave de API ao usuário.</p>
     </SettingsCategoryCard>

     {statusText && <div className="rounded-[1.5rem] border border-emerald-300/25 bg-emerald-300/10 p-4 text-sm font-bold text-emerald-100">{statusText}</div>}
    </div>
   </div>
  </div>
 );
...
function TogglePill({ enabled, onClick, label, description }: { enabled: boolean; onClick: () => void; label: string; description: string }) {
 return <button type="button" onClick={onClick} className={`flex w-full items-center justify-between gap-3 rounded-2xl border p-4 text-left transition ${enabled ? 'border-emerald-300/35 bg-emerald-300/10' : 'border-white/10 bg-white/[.04]'}`}><span><span className="block text-sm font-black">{label}</span><span className="mt-1 block text-xs leading-5 text-[color:var(--crew-muted)]">{description}</span></span><span className={`h-6 w-11 rounded-full p-1 transition ${enabled ? 'bg-emerald-300' : 'bg-white/20'}`}><span className={`block h-4 w-4 rounded-full bg-white transition ${enabled ? 'translate-x-5' : 'translate-x-0'}`} /></span></button>;
}

function normalizeCrewClockClient(value = '') {
...
function SettingsHomePanel({ userLabel, userEmail, profileSettings, avatar, pendingOffline, hasRoster, canAccessAdmin, themeMode, onThemeModeChange, onProfileSettingsChange, onAvatarChange, onBack, onOpenIFlight, onOpenImport, onOpenResultsSettings, onOpenPerDiem, onOpenSalary, onExportPdf, onExportIcs, onSyncPending, onSyncTelegramRoster, onLogout }: {
 ...
 const googleDiagnostics = googleCalendarIntegrationDiagnostics();
 ...
 const [callmebotEnabled, setCallmebotEnabled] = useState(true);
 const [wakeupCallProvider, setWakeupCallProvider] = useState('auto');
 const [infobipPhone, setInfobipPhone] = useState('');
 const [callmebotMode, setCallmebotMode] = useState('telegram');
 const [callmebotPhone, setCallmebotPhone] = useState('');
 const [callmebotTelegramUser, setCallmebotTelegramUser] = useState('');
 const [callmebotApiKey, setCallmebotApiKey] = useState('');
 const [callmebotLang, setCallmebotLang] = useState('pt-BR-Standard-A');
 const [callmebotRepeat, setCallmebotRepeat] = useState('2');
 const [wakeupNotificationMode, setWakeupNotificationMode] = useState('call_telegram');
 const [wakeupSnoozeMinutes, setWakeupSnoozeMinutes] = useState('10');
 const [wakeupMaxCalls, setWakeupMaxCalls] = useState('2');
 const [wakeupCallAfterSnoozes, setWakeupCallAfterSnoozes] = useState('2');
 const [wakeupMaxSnoozes, setWakeupMaxSnoozes] = useState('6');
 const [showCallmebotManual, setShowCallmebotManual] = useState(false);
...
const supportIsPremium = Boolean(billingStatus?.premiumAccess);
 const wakeupCallAllowed = Boolean(supportIsPremium || telegramStatus?.wakeupPolicy?.callAllowed);
 ...
 function crewcheckMaskProviderNames(value: string) {
  return String(value || '')
   .replace(/Infobip/gi, 'provedor VOIP principal')
   .replace(/Twilio/gi, 'provedor VOIP alternativo')
   .replace(/CallMeBot/gi, 'provedor de voz')
   .replace(/InOut\.bot|InOut/gi, 'canal VOIP direto')
   .replace(/direct-voip/gi, 'canal VOIP direto')
   .replace(/ElevenLabs/gi, 'voz premium')
   .replace(/Google TTS/gi, 'voz MP3')
 ...
 function applyTelegramPayload(payload?: TelegramStatus | null) {
  ...
  const prefs = payload.preferences || {};
  setCallmebotEnabled(prefs.wakeupCallEnabled !== false);
  if (typeof prefs.wakeupCallProvider === 'string') setWakeupCallProvider(['callmebot','infobip','twilio','auto','voip','phone','direct','direct-voip','inout'].includes(prefs.wakeupCallProvider) ? prefs.wakeupCallProvider : 'auto'); 
  if (typeof prefs.infobipPhone === 'string') setInfobipPhone(prefs.infobipPhone);
  if (typeof prefs.callmebotMode === 'string') setCallmebotMode(prefs.callmebotMode || 'telegram');
  if (typeof prefs.callmebotPhone === 'string') setCallmebotPhone(prefs.callmebotPhone);
  if (typeof prefs.callmebotTelegramUser === 'string') setCallmebotTelegramUser(prefs.callmebotTelegramUser);
  if (typeof prefs.callmebotLang === 'string') setCallmebotLang(prefs.callmebotLang || 'pt-BR-Standard-A');
  if (typeof prefs.callmebotRepeat === 'string') setCallmebotRepeat(prefs.callmebotRepeat || '2');
  if (typeof prefs.wakeupNotificationMode === 'string') setWakeupNotificationMode(['call','call_telegram','telegram','telegram_only','call_only'].includes(prefs.wakeupNotificationMode) ? prefs.wakeupNotificationMode : 'call_telegram');
  if (typeof prefs.overnightWakeupSnoozeMinutes === 'string') setWakeupSnoozeMinutes(prefs.overnightWakeupSnoozeMinutes || '10');
  if (typeof prefs.wakeupMaxCalls === 'string') setWakeupMaxCalls(prefs.wakeupMaxCalls || '2');
  if (typeof prefs.wakeupCallAfterSnoozes === 'string') setWakeupCallAfterSnoozes(prefs.wakeupCallAfterSnoozes || '2');
  if (typeof prefs.wakeupMaxSnoozes === 'string') setWakeupMaxSnoozes(prefs.wakeupMaxSnoozes || '6');
 }
 ...
 async function refreshTelegramStatus(silent = false) {
  try {
   ...
   if (!silent && payload?.connected) toast.success('Telegram CrewCheck conectado.');
  } catch (error) {
   if (!silent) toast.error(error instanceof Error ? error.message : 'Não foi possível verificar o Telegram CrewCheck agora.');
  }
 }
 ...
 async function connectTelegram() {
  ...
  try {
   ...
   if (payload.connected) {
    ...
    toast.success('Telegram CrewCheck já está conectado e a escala foi sincronizada quando disponível.');
    return;
   }
   ...
   toast.error(error instanceof Error ? error.message : 'Não foi possível conectar o Telegram CrewCheck.');
  } finally {
 ...
 async function testTelegram() {
  ...
  try {
   ...
   toast.error(error instanceof Error ? error.message : 'Não foi possível enviar o teste do Telegram CrewCheck.');
  } finally {
   setTelegramBusy(false);
  }
 }

 function sanitizeTelegramPreferencePatch(patch: Record<string, boolean | string>) {
  const allowed = new Set(['smartDeparture','roster','radar','gateStatus','traffic','irregularities','dailySummary','weeklySummary','concierge','conciergePremiumText','conciergeFreeDailyLimit','conciergeVoice','conciergeAudioReplies','conciergeName','overnightWakeup','overnightWakeupSnoozeMinutes','wakeupNotificationMode','wakeupMaxCalls','wakeupCallAfterSnoozes','wakeupMaxSnoozes','wakeupCallEnabled','wakeupCallProvider','infobipPhone','twilioPhone','callmebotMode','callmebotPhone','callmebotTelegramUser','callmebotApiKey','callmebotLang','callmebotRepeat','crewcheckVoiceProviderAuthorized','radarImportantOnly','radarQuiet']);
  const clean: Record<string, boolean | string> = {};
  Object.entries(patch || {}).forEach(([key, value]) => {
   if (!allowed.has(key)) return;
   if (typeof value === 'boolean') clean[key] = value;
   else clean[key] = String(value || '').slice(0, key === 'callmebotApiKey' ? 90 : key === 'conciergeName' ? 32 : 120);
  });
  return clean;
 }

 async function saveTelegramPreferences(patch: Record<string, boolean | string>) {
  const cleanPatch = sanitizeTelegramPreferencePatch(patch);
  const next = { ...(telegramStatus?.preferences || {}), ...cleanPatch } as Record<string, boolean | string>;
  setTelegramStatus((current) => ({ ...(current || {}), preferences: next }));
  try {
   const response = await fetch('/api/telegram/preferences', { method: 'POST', headers: crewcheckAuthHeader({ 'content-type': 'application/json' }), body: JSON.stringify({ preferencesPatch: cleanPatch }), cache: 'no-store' });
   const payload = await response.json().catch(() => null) as TelegramStatus | null;
   if (!response.ok || !payload?.ok) throw new Error(payload?.message || 'Não foi possível salvar a preferência.');
   applyTelegramPayload(payload);
   return payload;
  } catch (error) {
   toast.error(error instanceof Error ? error.message : 'Não foi possível salvar a preferência do Telegram CrewCheck.');
   return null;
  }
 }

 async function saveTelegramPreference(key: string, value: boolean) {
  await saveTelegramPreferences({ [key]: value } as Record<string, boolean | string>);
 }
 ...
 async function saveTelegramConciergeName() {
  ...
  const payload = await saveTelegramPreferences({ conciergeName: cleaned });
  if (payload?.ok) toast.success('Nome do Concierge salvo.');
 }

 async function saveCallmebotWakeupSettings(showToast = true) {
  const next: Record<string, boolean | string> = {
   overnightWakeup: true,
   wakeupCallEnabled: wakeupCallAllowed ? callmebotEnabled : false,
   wakeupCallProvider: ['auto', 'twilio', 'infobip', 'callmebot', 'voip', 'phone', 'direct', 'direct-voip', 'inout'].includes(wakeupCallProvider) ? wakeupCallProvider : 'auto',
   infobipPhone: infobipPhone.replace(/[^+\d]/g, '').slice(0, 22),
   twilioPhone: infobipPhone.replace(/[^+\d]/g, '').slice(0, 22),
   callmebotMode: ['phone', 'telegram', 'auto'].includes(callmebotMode) ? callmebotMode : 'telegram',
   callmebotPhone: callmebotPhone.replace(/[^+\d]/g, '').slice(0, 22),
   callmebotTelegramUser: callmebotTelegramUser.replace(/\s+/g, '').replace(/^@+/, '@').slice(0, 64),
   callmebotLang: callmebotLang.replace(/[^A-Za-z0-9_-]/g, '').slice(0, 40) || 'pt-BR-Standard-A',
   callmebotRepeat: String(Math.max(1, Math.min(5, Number(callmebotRepeat) || 2))),
   crewcheckVoiceProviderAuthorized: true,
   wakeupNotificationMode: ['call','call_telegram','telegram','telegram_only','call_only'].includes(wakeupNotificationMode) ? wakeupNotificationMode : 'call_telegram',
   overnightWakeupSnoozeMinutes: String(Math.max(3, Math.min(90, Number(wakeupSnoozeMinutes) || 10))),
   wakeupMaxCalls: String(Math.max(0, Math.min(2, Number(wakeupMaxCalls) || 2))),
   wakeupCallAfterSnoozes: String(Math.max(1, Math.min(8, Number(wakeupCallAfterSnoozes) || 2))),
   wakeupMaxSnoozes: String(Math.max(1, Math.min(12, Number(wakeupMaxSnoozes) || 6))),
  };
  const cleanKey = callmebotApiKey.replace(/\s+/g, '').slice(0, 90);
  if (cleanKey) next.callmebotApiKey = cleanKey;
  const payload = await saveTelegramPreferences(next);
  if (payload?.ok) {
   setCallmebotApiKey('');
   if (showToast) toast.success('Despertador por chamada salvo.');
  }
  return payload;
 }

 async function autoConfigureCallmebotWakeup() {
  if (!telegramStatus?.connected) {
   toast.info('Conecte o Telegram CrewCheck primeiro. Depois o Despertador Inteligente CrewCheck usa o mesmo usuário automaticamente.');
   return;
  }
  setTelegramBusy(true);
  try {
   const response = await fetch('/api/telegram/wakeup/autoconfig', {
    method: 'POST',
    headers: crewcheckAuthHeader({ 'content-type': 'application/json' }),
    body: JSON.stringify({ telegramUsername, wakeupCallProvider: wakeupCallProvider || 'voip', infobipPhone: infobipPhone || callmebotPhone, twilioPhone: infobipPhone || callmebotPhone, callmebotPhone: callmebotPhone || infobipPhone, callmebotRepeat, callmebotLang, crewcheckVoiceProviderAuthorized: true, wakeupNotificationMode, overnightWakeupSnoozeMinutes: wakeupSnoozeMinutes, wakeupMaxCalls, wakeupCallAfterSnoozes, wakeupMaxSnoozes, timeZone: preferredSystemTimeZone(profileSettings) }),
    cache: 'no-store',
   });
   const payload = await response.json().catch(() => null) as TelegramStatus & { message?: string } | null;
   if (!response.ok || !payload?.ok) throw new Error(payload?.message || 'Não foi possível configurar automaticamente.');
   applyTelegramPayload(payload);
   setCallmebotMode(payload.callmebot?.mode || payload.callmebot?.effectiveMode || 'telegram');
   toast.success(payload.message || 'Despertador Inteligente CrewCheck configurado automaticamente pelo Telegram.');
  } catch (error) {
   toast.error(error instanceof Error ? error.message : 'Não foi possível configurar o Despertador Inteligente CrewCheck automaticamente.');
  } finally {
   setTelegramBusy(false);
  }
 }
 ...
 async function authorizeCrewCheckWakeupProvider() {
  const authorizeUrl = telegramStatus?.callmebot?.authorizeUrl || 'https://t.me/provedor de voz_integração';
  await saveTelegramPreferences({ overnightWakeup: true, wakeupCallEnabled: true, crewcheckVoiceProviderAuthorized: true });
  try {
   window.open(authorizeUrl, '_blank', 'noopener,noreferrer');
   toast.success('Abrindo autorização de chamadas do Despertador Inteligente CrewCheck. No Telegram, toque em Iniciar/Start e permita chamadas.');
  } catch {
   window.location.href = authorizeUrl;
  }
 }

 async function testCallmebotWakeup() {
  if (!wakeupCallAllowed) {
   toast.info('No plano gratuito o despertador avisa pelo Telegram. Ligações automáticas ficam no Premium.');
   return;
  }
  const cleanInfobipPhone = infobipPhone.replace(/[^+\d]/g, '').slice(0, 22);
  const renderAlreadyHasTarget = Boolean(telegramStatus?.callmebot?.configured || telegramStatus?.callmebot?.targetMasked || telegramStatus?.twilio?.targetConfigured || telegramStatus?.twilio?.targetMasked || telegramStatus?.infobip?.targetConfigured || telegramStatus?.infobip?.targetMasked);
  if (!cleanInfobipPhone && !renderAlreadyHasTarget) {
   setShowCallmebotManual(true);
   toast.info('Preencha o telefone da ligação VOIP antes de testar. Ex.: 5561999999999.');
   return;
  }
  setTelegramBusy(true);
  try {
   const saved = await saveCallmebotWakeupSettings(false);
   if (!saved?.ok) throw new Error('Salve as configurações do Despertador Inteligente CrewCheck antes do teste.');
   const response = await fetch('/api/telegram/wakeup/test', {
    method: 'POST',
    headers: crewcheckAuthHeader({ 'content-type': 'application/json' }),
    body: JSON.stringify({ forceProvider: ['callmebot','twilio','infobip','voip','phone','direct','direct-voip','inout'].includes(wakeupCallProvider) ? wakeupCallProvider : 'voip', provider: ['callmebot','twilio','infobip','voip','phone','direct','direct-voip','inout'].includes(wakeupCallProvider) ? wakeupCallProvider : 'voip', callOnly: true, allowTelegramVoice: false, infobipPhone: cleanInfobipPhone, twilioPhone: cleanInfobipPhone, callmebotPhone: cleanInfobipPhone, callmebotApiKey: callmebotApiKey.replace(/\s+/g, '').slice(0, 90), wakeupNotificationMode, overnightWakeupSnoozeMinutes: wakeupSnoozeMinutes, wakeupMaxCalls, wakeupCallAfterSnoozes, wakeupMaxSnoozes, timeZone: preferredSystemTimeZone(profileSettings) }),
    cache: 'no-store',
   });
   const payload = await response.json().catch(() => null) as { ok?: boolean; message?: string; status?: TelegramStatus; result?: { message?: string; code?: string; attempts?: Array<{ provider?: string; status?: number; code?: string; message?: string; error?: string; missing?: string[] }> } } | null;
   if (!response.ok || !payload?.ok) {
    const firstAttempt = payload?.result?.attempts?.find((attempt) => attempt?.message || attempt?.error);
    const detail = crewcheckMaskProviderNames(firstAttempt?.message || firstAttempt?.error || payload?.result?.message || payload?.message || 'Não foi possível iniciar a chamada de teste.');
    const missing = firstAttempt?.missing?.length ? ` Falta: ${firstAttempt.missing.join(', ')}.` : '';
    throw new Error(`${detail}${missing}`);
   }
   if (payload.status) applyTelegramPayload(payload.status);
   const providerSuccess = 'Ligação VOIP solicitada pelo Despertador Inteligente CrewCheck. O teste não envia áudio no Telegram.';
   toast.success(crewcheckMaskProviderNames(payload.result?.message || payload.message || providerSuccess));
  } catch (error) {
   toast.error(error instanceof Error ? crewcheckMaskProviderNames(error.message) : 'Não foi possível testar o despertador por chamada.');
  } finally {
   setTelegramBusy(false);
  }
 }
...
Contexto: mensagem enviada pela área de Ajuda do CrewCheck.`,
 ...
 function handleSaveClientId() {
  ...
  toast.success('OAuth Client ID salvo/removido neste dispositivo.');
 }
 ...
 function renderCategoryContent() {
   ...
   return (
    <SettingsCategoryCard eyebrow="Perfil local" title="Quem aparece no CrewCheck" description="Esses dados ficam salvos apenas neste dispositivo e ajudam a identificar chefe de cabine, base e exportações.">
     ...
     <div className="mt-5 grid gap-3 sm:grid-cols-2">
      ...
      <label className="text-xs font-black uppercase tracking-[0.16em] text-slate-400">Função / ACT automática<select value={profileForm.rank} onChange={(event) => setProfileForm({ ...profileForm, rank: event.target.value })} className="crewcheck-premium-input mt-2 w-full">
        <option value="">Detectar pela escala</option>
        <option value="CCM">CCM / Comissário(a)</option>
        <option value="Chefe de Cabine">Chefe de Cabine</option>
        <option value="Comandante">Comandante</option>
        <option value="Copiloto">Copiloto / FO</option>
        <option value="Piloto">Piloto(a)</option>
       </select><span className="mt-2 block text-[11px] font-semibold normal-case leading-5 text-slate-400">{resolveCrewFunction(profileForm.rank).role === 'auto' ? 'O CrewCheck usará a escala e o PDF para aplicar a ACT correta.' : `Aplicação automática: ${resolveCrewFunction(profileForm.rank).role === 'pilot' ? 'ACT Pilotos' : 'ACT Comissários'} · ${resolveCrewFunction(profileForm.rank).functionLabel}`}</span></label>
      <label className="text-xs font-black uppercase tracking-[0.16em] text-slate-400">País do telefone<select value={profileForm.phoneCountryIso} onChange={(e) => { const country = crewCountryByIso(e.target.value); setProfileForm({ ...profileForm, phoneCountryIso: country.iso, phoneCountryCode: country.code }); }} className="crewcheck-premium-input mt-2 w-full">{CREWCHECK_COUNTRY_PHONE_OPTIONS.map((country) => <option key={country.iso} value={country.iso}>{country.name} {country.code}</option>)}</select></label>
      <ProfileField label="Telefone do despertar" value={profileForm.phoneLocal} placeholder={crewCountryByIso(profileForm.phoneCountryIso).example} onChange={(value) => { const country = crewCountryByIso(profileForm.phoneCountryIso); const local = normalizeCrewPhoneDigits(value); setProfileForm({ ...profileForm, phoneLocal: local, phoneCountryCode: country.code, phoneE164: composeCrewPhoneE164(country.code, local) }); }} />
     </div>
   ...
   return (
    <SettingsCategoryCard eyebrow="Agenda automática" title="Google Calendar e exportação ICS" description="Escolha o calendário, evite duplicidade e mantenha voos, rotina e alertas sincronizados quando desejar.">
     ...
     <div className="mt-5 grid gap-3 sm:grid-cols-2">
      ...
      <label className="block"><span className="text-xs font-black uppercase tracking-[0.14em] text-cyan-100/60">Modo de exportação</span><select value={googleSettings.exportMode || 'flights-rest'} onChange={(event) => persistGoogle({ ...googleSettings, exportMode: event.target.value as GoogleCalendarSyncMode })} className="mt-2 h-12 w-full rounded-2xl border border-white/10 bg-[#07111f] px-3 text-sm font-bold text-white outline-none focus:border-cyan-300"><option value="flights-rest">Voo + folgas · recomendado</option><option value="all">Escala completa · sem rotina separada</option><option value="flights">Voos detalhados</option><option value="gym">Somente academia</option><option value="routine">Somente rotina</option></select></label>
     </div>
     <div className="mt-4 rounded-3xl border border-cyan-300/20 bg-cyan-300/10 p-4">
      <div className="flex flex-col gap-1 sm:flex-row sm:items-center sm:justify-between">
       <div><p className="text-xs font-black uppercase tracking-[0.18em] text-cyan-100/65">OAuth CrewCheck</p><h4 className="text-lg font-black text-white">Vínculo Google Calendar pronto para autorização do usuário</h4></div>
       <span className={`rounded-full px-3 py-1 text-[11px] font-black uppercase tracking-[0.14em] ${googleConnected ? 'bg-emerald-300/20 text-emerald-100' : 'bg-amber-300/20 text-amber-100'}`}>{googleConnected ? 'Conectado' : 'Aguardando login'}</span>
      </div>
      <div className="mt-3 grid gap-2 md:grid-cols-2">
       {googleDiagnostics.map((item) => <div key={item.label} className={`rounded-2xl border p-3 text-xs font-bold leading-5 ${item.tone === 'ok' ? 'border-emerald-300/20 bg-emerald-300/10 text-emerald-50' : item.tone === 'warn' ? 'border-amber-300/20 bg-amber-300/10 text-amber-50' : 'border-white/10 bg-white/[0.045] text-slate-200'}`}><span className="block text-[10px] uppercase tracking-[0.16em] opacity-70">{item.label}</span><span className="break-all">{item.value}</span></div>)}
      </div>
      <p className="mt-3 text-xs font-semibold leading-5 text-cyan-50/80">Para agendas pessoais, o CrewCheck usa OAuth: cada usuário clica em Conectar Google e autoriza a própria agenda. A Service Account só funciona se o calendário for compartilhado com ela ou em Workspace com delegação administrativa.</p>
     </div>
     ...
     {!hasEnvClientId && <div className="mt-4 rounded-2xl border border-amber-300/20 bg-amber-300/10 p-4"><p className="text-sm font-bold text-amber-50">OAuth Client ID opcional neste dispositivo</p><input value={clientIdOverride} onChange={(event) => setClientIdOverride(event.target.value)} placeholder="Cole outro OAuth Client ID temporariamente" className="mt-3 h-12 w-full rounded-2xl border border-white/10 bg-[#07111f] px-4 text-sm font-semibold text-white outline-none placeholder:text-slate-600 focus:border-cyan-300" /><button onClick={handleSaveClientId} className="mt-3 rounded-2xl bg-amber-200 px-5 py-3 text-sm font-black text-[#07111f]">Salvar Client ID local</button></div>}
    </SettingsCategoryCard>
   );
  }
  ...
  if (activeCategory === 'alerts') {
   ...
   const alertItems: Array<[string, string, string]> = [['operationalMonitor','Monitor operacional','varredura automática de voos próximos e aeroportos da escala'], ['flightStatusPush','Status do voo','confirmado, cancelado, embarque próximo, última chamada e atraso'], ['aviationWeather','Meteorologia aeronáutica','METAR/TAF com TS/CB, vento, teto e visibilidade'], ['smartDeparture','Saída Inteligente','hora de sair, margem, modal e rota'], ['gateStatus','Portão e terminal','portão, terminal, embarque e última chamada'], ['traffic','Trânsito e rota','tempo estimado, fonte, atraso e alternativa'], ['radar','Radar de voo','somente mudança útil: status, portão, terminal ou previsão'], ['radarImportantOnly','Radar anti-spam','silencia Programado repetido sem portão/alteração'], ['irregularities','Irregularidades','descanso, jornada e alertas críticos'], ['dailySummary','Resumo diário','programação do dia e pontos de atenção'], ['weeklySummary','Resumo semanal','diárias, salário estimado e pendências'], ['overnightWakeup','Despertar de apresentação','alerta insistente do hotel até confirmar que acordou'], ['concierge','Concierge por texto e voz','responder por texto ou áudio sobre escala, portão, saída, diárias e rotina'], ['roster','Escala','importação, alteração e próximo evento']];
   return (
    <SettingsCategoryCard eyebrow="Central de alertas" title="Escolha o que vale notificar" description="O usuário decide o nível de aviso. O CrewCheck evita spam e prioriza alertas úteis: hora de sair, portão, trânsito, status de voo, última chamada e irregularidades. Status Programado sem mudança real fica silencioso.">
     ...
     <div className="mt-5 rounded-[1.6rem] border border-cyan-200/20 bg-cyan-300/10 p-4"><div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><div><p className="text-[10px] font-black uppercase tracking-[0.18em] text-cyan-100/65">Monitor em tempo quase real</p><h4 className="text-lg font-black text-white">Portão, status do voo e meteorologia</h4><p className="mt-1 text-xs font-semibold leading-5 text-slate-300">Com o app aberto ou em segundo plano permitido, o CrewCheck varre voos próximos da escala e avisa mudança de portão/status. Com Telegram conectado, também envia fora do app.</p></div><button type="button" onClick={() => { const next = localStorage.getItem('crewcheck_operational_monitor_enabled') === 'false'; localStorage.setItem('crewcheck_operational_monitor_enabled', next ? 'true' : 'false'); toast.success(next ? 'Monitor operacional ativado.' : 'Monitor operacional pausado neste aparelho.'); }} className="rounded-2xl bg-cyan-300 px-5 py-4 text-sm font-black text-[#07111f]">Ativar/Pausar neste aparelho</button></div></div>
...
if (activeCategory === 'telegram') {
   ...
   return (
    <div className="space-y-5">
     ...
     <SettingsCategoryCard eyebrow="Telegram CrewCheck" title="Alertas importantes fora do app" description="Canal executivo de alertas e perguntas por texto ou áudio: escala, portão, Saída Inteligente, radar, diárias, rotina e irregularidades com limites no gratuito e experiência completa no Premium.">
      ...
      <div className="mt-4 grid gap-3 sm:grid-cols-2">
       {[
        ...
        ['overnightWakeup', 'Despertar de apresentação', 'notificação insistente com Já acordei e Soneca'],
       ].map(([key, title, text]) => (
      ...
      <div className="mt-4 rounded-[1.35rem] border border-indigo-200/20 bg-indigo-300/10 p-4">
       <p className="text-[10px] font-black uppercase tracking-[0.2em] text-indigo-100/70">Apresentação e despertar</p>
       <h4 className="mt-1 text-xl font-black text-white">Soneca inteligente no Telegram</h4>
       <p className="mt-1 text-sm leading-6 text-slate-300">Quando ativado, o CrewCheck envia lembretes até o usuário tocar em “Já acordei”. A soneca também fica no botão da própria notificação.</p>
       <div className="mt-3 grid gap-2 sm:grid-cols-3">
        {[5,10,15,20,30,45].map((minutes) => <button key={minutes} type="button" onClick={() => void saveTelegramPreferences({ ...(telegramPrefs || {}), overnightWakeupSnoozeMinutes: String(minutes), overnightWakeup: true })} className={`rounded-2xl border px-4 py-3 text-xs font-black ${(telegramPrefs as Record<string, string>)?.overnightWakeupSnoozeMinutes === String(minutes) ? 'border-indigo-200 bg-indigo-200 text-slate-950' : 'border-white/10 bg-white/10 text-indigo-50'}`}>{minutes} min</button>)}
       </div>
      </div>
      <div className="mt-4 rounded-[1.35rem] border border-amber-200/25 bg-amber-300/10 p-4">
       <p className="text-[10px] font-black uppercase tracking-[0.2em] text-amber-100/70">Despertador Premium</p>
       <h4 className="mt-1 text-xl font-black text-white">Configuração movida para menu dedicado</h4>
       <p className="mt-1 text-sm leading-6 text-slate-300">Telefone, ligação, soneca, hotel, quarto, apresentação e contato de emergência ficam agora em <b>Operacional &gt; Despertador Premium</b>. As configurações técnicas do provedor de ligação não aparecem mais para o usuário comum.</p>
       {canAccessAdmin && <button type="button" onClick={() => setShowCallmebotManual((value) => !value)} className="mt-3 rounded-2xl border border-white/10 bg-white/10 px-4 py-3 text-xs font-black text-amber-50">{showCallmebotManual ? 'Ocultar diagnóstico admin' : 'Diagnóstico admin do canal de ligação'}</button>}
       {canAccessAdmin && showCallmebotManual && <div className="mt-3 rounded-2xl border border-white/10 bg-black/15 p-4 text-xs leading-5 text-amber-50">Canal de ligação Premium: diagnóstico restrito ao administrador. Para o usuário final, o sistema exibe apenas “ligação premium”, “Telegram” e “WhatsApp”, sem nome de API ou provedor.</div>}
      </div>
...
function SystemStatusScreen({ onBack }: { onBack: () => void }) {
 ...
 const tiles = [
  ...
  { title: 'Google Flights', ok: Boolean(radar?.hasSerpintegração || radar?.hasSearchintegração), detail: (radar?.hasSerpintegração || radar?.hasSearchintegração) ? 'fonte de voos configurada' : 'não configurado' },
...
type DefesaCivilAlertsResponse = {
 ok?: boolean;
 sourceConfigured?: boolean;
 officialOnly?: boolean;
 source?: string;
 message?: string;
 alerts?: DefesaCivilAlert[];
 updatedAt?: string;
 officialChannels?: { sms?: string; whatsapp?: string; service10145?: string; idapInfo?: string };
 detail?: string;
};


type AviationWeatherAlert = {
 id?: string;
 airport?: string;
 icao?: string;
 title?: string;
...
type AviationWeatherAlertsResponse = {
 ok?: boolean;
 sourceConfigured?: boolean;
 source?: string;
 message?: string;
...
function aviationSeverityLabel(value?: string | null): string {
 const normalized = String(value || '').toLowerCase();
 if (normalized === 'extreme') return 'Crítico';
 if (normalized === 'severe') return 'Severo';
 if (normalized === 'moderate') return 'Atenção';
 if (normalized === 'minor') return 'Monitorar';
 return 'A confirmar';
}


function formatDefesaCivilDate(value?: string | null): string {
 if (!value) return '—';
...
function aviationSeverityClass(value?: string | null): string {
 const normalized = String(value || '').toLowerCase();
 if (normalized === 'extreme') return 'border-red-300/35 bg-red-500/15 text-red-50';
 if (normalized === 'severe') return 'border-orange-300/35 bg-orange-400/15 text-orange-50';
 if (normalized === 'moderate') return 'border-amber-300/35 bg-amber-300/15 text-amber-50';
 return 'border-cyan-300/25 bg-cyan-300/10 text-cyan-50';
}


type AviationWeatherTextMode = 'raw' | 'translated';

const CREWCHECK_WEATHER_PHONETIC_WORDS: Record<string, string> = {
...
const CREWCHECK_WEATHER_AIRPORT_NAMES: Record<string, string> = {
 SBBR: 'Aeroporto Internacional Juscelino Kubitschek', SBGR: 'Aeroporto Internacional de Guarulhos', SBSP: 'Aeroporto de Congonhas', SBRJ: 'Aeroporto Santos Dumont', SBGL: 'Aeroporto Internacional do Galeão', SBKP: 'Aeroporto Internacional de Viracopos', SBCF: 'Aeroporto Internacional de Confins', SBGO: 'Aeroporto de Goiânia', SBCT: 'Aeroporto Internacional Afonso Pena', SBFZ: 'Aeroporto Internacional de Fortaleza', SBRF: 'Aeroporto Internacional do Recife', SBSV: 'Aeroporto Internacional de Salvador', SBSG: 'Aeroporto Internacional de Natal', SBFL: 'Aeroporto Internacional de Florianópolis', SBPA: 'Aeroporto Internacional de Porto Alegre', SBBE: 'Aeroporto Internacional de Belém', SBEG: 'Aeroporto Internacional de Manaus', SBPJ: 'Aeroporto de Palmas', SBJP: 'Aeroporto Internacional de João Pessoa', SBMO: 'Aeroporto Internacional de Maceió', SBMA: 'Aeroporto de Marabá', SBKG: 'Aeroporto de Campina Grande', SBCX: 'Aeroporto de Caxias do Sul', KMIA: 'Aeroporto Internacional de Miami', KMCO: 'Aeroporto Internacional de Orlando', LPPT: 'Aeroporto de Lisboa', SCEL: 'Aeroporto de Santiago', SAEZ: 'Aeroporto Internacional de Ezeiza'
};

function crewWeatherIcaoPhonetic(icao?: string | null): string {
 return String(icao || '').toUpperCase().replace(/[^A-Z]/g, '').split('').map((letter) => CREWCHECK_WEATHER_PHONETIC_WORDS[letter] || letter).join(' ');
}

function crewWeatherAirportTitle(icao?: string | null): string {
 const code = String(icao || '').toUpperCase().replace(/[^A-Z]/g, '');
 return CREWCHECK_WEATHER_AIRPORT_NAMES[code] || code || 'Aeroporto consultado';
}

function crewWeatherSignedTemperature(value = ''): string {
 const raw = String(value || '').trim();
 if (!raw) return '';
 return raw.startsWith('M') ? `menos ${Number(raw.slice(1))}` : String(Number(raw));
}

function crewWeatherMetarDayTime(raw = ''): string {
 const match = String(raw || '').match(/\b(\d{2})(\d{2})(\d{2})Z\b/);
 if (!match) return '';
 return `Dia ${Number(match[1])}, horário ${match[2]}:${match[3]} UTC.`;
}

function crewWeatherWindText(raw = ''): string {
 const match = String(raw || '').toUpperCase().match(/(?:^|\s)(VRB|\d{3})(\d{2,3})(?:G(\d{2,3}))?KT\b/);
 if (!match) return '';
 const direction = match[1] === 'VRB' ? 'variável' : `${match[1]} graus`;
 const speed = Number(match[2]);
 const gust = match[3] ? Number(match[3]) : 0;
 return gust ? `Vento de ${direction} com ${speed} nós, rajadas de ${gust} nós.` : `Vento de ${direction} com ${speed} nós.`;
}

function crewWeatherVisibilityText(raw = ''): string {
 const upper = String(raw || '').toUpperCase();
 if (/\bCAVOK\b/.test(upper)) return 'CAVOK: visibilidade de 10 quilômetros ou mais, sem nuvens significativas abaixo de 5.000 pés e sem tempo significativo.';
 const match = upper.match(/(?:^|\s)(\d{4})(?:\s|$)/);
 if (!match) return '';
 const meters = Number(match[1]);
 if (!Number.isFinite(meters)) return '';
 if (meters >= 9999) return 'Visibilidade de 10 quilômetros ou mais.';
 if (meters >= 1000) return `Visibilidade de ${(meters / 1000).toLocaleString('pt-BR', { maximumFractionDigits: 1 })} quilômetros.`;
 return `Visibilidade de ${meters} metros.`;
}

function crewWeatherCloudText(raw = ''): string {
 const labels: Record<string, string> = { FEW: 'poucas nuvens', SCT: 'nuvens esparsas', BKN: 'céu nublado', OVC: 'céu encoberto', VV: 'visibilidade vertical' };
 const clouds = [...String(raw || '').toUpperCase().matchAll(/\b(FEW|SCT|BKN|OVC|VV)(\d{3})(CB|TCU)?\b/g)].map((match) => {
  const base = Number(match[2]) * 100;
  const suffix = match[3] === 'CB' ? ', cumulonimbus' : match[3] === 'TCU' ? ', torre de cúmulus' : '';
  const teto = /^(BKN|OVC|VV)$/.test(match[1]) ? ' com teto' : '';
  return `${labels[match[1]] || match[1]}${teto} a ${base.toLocaleString('pt-BR')} pés${suffix}`;
 });
 if (!clouds.length) return /\bNSC\b/.test(String(raw || '').toUpperCase()) ? 'Sem nuvens significativas.' : '';
 return `Condição de céu: ${clouds.join('; ')}.`;
}

function crewWeatherTemperatureText(raw = ''): string {
 const match = String(raw || '').toUpperCase().match(/\b(M?\d{2})\/(M?\d{2})\b/);
 if (!match) return '';
 return `Temperatura ${crewWeatherSignedTemperature(match[1])} graus. Ponto de orvalho ${crewWeatherSignedTemperature(match[2])} graus.`;
}

function crewWeatherQnhText(raw = ''): string {
 const match = String(raw || '').toUpperCase().match(/\bQ(\d{4})\b/);
 return match ? `QNH ${match[1]} hectopascais.` : '';
}

function crewWeatherPhenomenaText(raw = ''): string {
 const upper = String(raw || '').toUpperCase();
 const items: string[] = [];
 if (/\+TSRA|\bTSRA\b|\bVCTS\b|\bTS\b/.test(upper)) items.push('trovoada na área');
 if (/\bCB\b/.test(upper)) items.push('cumulonimbus');
 if (/\bTCU\b/.test(upper)) items.push('torre de cúmulus');
 if (/\+RA/.test(upper)) items.push('chuva forte');
 else if (/\bRA\b/.test(upper)) items.push('chuva');
 if (/\bDZ\b/.test(upper)) items.push('garoa');
 if (/\bFG\b/.test(upper)) items.push('nevoeiro');
 if (/\bBR\b/.test(upper)) items.push('névoa úmida');
 if (/\bHZ\b/.test(upper)) items.push('névoa seca');
 if (/\bNSW\b/.test(upper)) items.push('sem tempo significativo');
 return items.length ? `Fenômenos reportados: ${items.join(', ')}.` : '';
}

function crewWeatherTafGlossary(raw = ''): string[] {
 const upper = String(raw || '').toUpperCase();
 const parts: string[] = [];
 const issued = upper.match(/\b(\d{2})(\d{2})(\d{2})Z\b/);
 const validity = upper.match(/\b(\d{2})(\d{2})\/(\d{2})(\d{2})\b/);
 if (issued) parts.push(`TAF emitido no dia ${Number(issued[1])}, às ${issued[2]}:${issued[3]} UTC.`);
 if (validity) parts.push(`Validade do dia ${Number(validity[1])} às ${validity[2]} UTC até o dia ${Number(validity[3])} às ${validity[4]} UTC.`);
 if (/\bTEMPO\b/.test(upper)) parts.push('TEMPO indica condição temporária dentro do período informado.');
 if (/\bBECMG\b/.test(upper)) parts.push('BECMG indica mudança gradual prevista.');
 if (/\bPROB30\b/.test(upper)) parts.push('PROB30 indica probabilidade de 30 por cento.');
 if (/\bPROB40\b/.test(upper)) parts.push('PROB40 indica probabilidade de 40 por cento.');
 if (/\bFM\d{6}\b/.test(upper)) parts.push('FM indica mudança a partir do horário especificado.');
 return parts;
}

function translateAviationWeatherText(raw?: string | null, type: 'METAR' | 'TAF' = 'METAR', icao?: string | null): string {
 const text = String(raw || '').trim();
 if (!text) return type === 'METAR' ? 'Sem METAR disponível.' : 'Sem TAF disponível.';
 const tokens = text.split(/\s+/);
 const station = normalizeAviationAirport(icao || tokens.find((token) => /^[A-Z]{4}$/.test(token)) || '') || String(icao || '').toUpperCase();
 const title = `${crewWeatherAirportTitle(station)}${station ? `, ${crewWeatherIcaoPhonetic(station)}.` : '.'}`;
 const lines = [title];
 const dayTime = crewWeatherMetarDayTime(text);
 if (dayTime) lines.push(dayTime);
 const core = [crewWeatherWindText(text), crewWeatherVisibilityText(text), crewWeatherCloudText(text), crewWeatherPhenomenaText(text), crewWeatherTemperatureText(text), crewWeatherQnhText(text)].filter(Boolean);
 lines.push(...core);
 if (type === 'TAF') lines.push(...crewWeatherTafGlossary(text));
 lines.push('Informação operacional extra, como pista em uso, nível de transição, NOTAM e letra ATIS, só deve aparecer quando vier de fonte oficial integrada.');
 return lines.join(' ');
}

function weatherDisplayText(raw?: string | null, mode: AviationWeatherTextMode = 'raw', type: 'METAR' | 'TAF' = 'METAR', icao?: string | null): string {
 const text = String(raw || '').trim();
 if (mode === 'raw') return text || (type === 'METAR' ? 'Sem METAR bruto.' : 'Sem TAF bruto.');
 return translateAviationWeatherText(text, type, icao);
}

const CREWCHECK_IATA_TO_ICAO: Record<string, string> = {
...
function normalizeAviationAirport(value?: string | null): string {
 const code = String(value || '').toUpperCase().replace(/[^A-Z]/g, '');
 if (code.length === 4) return code;
 if (code.length === 3) return CREWCHECK_IATA_TO_ICAO[code] || code;
 return '';
}

function collectRosterWeatherAirports(bundle: SessionRosterBundle | null): string[] {
 const codes: string[] = [];
 const push = (value?: string | null) => {
  const normalized = normalizeAviationAirport(value);
  if (normalized && !codes.includes(normalized)) codes.push(normalized);
 };
 const today = new Date();
 today.setHours(0, 0, 0, 0);
 const max = today.getTime() + 10 * 24 * 60 * 60 * 1000;
 for (const day of bundle?.roster?.days || []) {
  const date = rosterDateFromString(day.date);
  const ms = date.getTime();
  if (Number.isFinite(ms) && (ms < today.getTime() - 24 * 60 * 60 * 1000 || ms > max)) continue;
  push(day.base || bundle?.roster?.base);
  for (const leg of day.legs || []) {
   push(leg.origin);
   push(leg.destination);
  }
 }
 if (!codes.length) push(bundle?.roster?.base || localStorage.getItem('crewcheck_preferred_airport') || 'BSB');
 return codes.slice(0, 8);
}

function MeteorologiaDefesaCivilScreen({ bundle, onBack }: { bundle: SessionRosterBundle | null; onBack: () => void }) {
 const rosterAirports = useMemo(() => collectRosterWeatherAirports(bundle), [bundle?.roster?.days?.length, bundle?.roster?.month, bundle?.roster?.year, bundle?.roster?.base]);
 const [airportText, setAirportText] = useState(() => {
  try { return localStorage.getItem('crewcheck_aviation_weather_airports') || rosterAirports.join(', '); } catch { return rosterAirports.join(', '); }
 });
 const [payload, setPayload] = useState<AviationWeatherAlertsResponse | null>(null);
 const [loading, setLoading] = useState(false);
 const [weatherTextMode, setWeatherTextMode] = useState<AviationWeatherTextMode>(() => {
  try { return (localStorage.getItem('crewcheck_aviation_weather_text_mode') as AviationWeatherTextMode) || 'translated'; } catch { return 'translated'; }
 });
 const changeWeatherTextMode = useCallback((mode: AviationWeatherTextMode) => {
  setWeatherTextMode(mode);
  try { localStorage.setItem('crewcheck_aviation_weather_text_mode', mode); } catch {}
 }, []);
 const airports = (airportText.match(/[A-Z]{3,4}/gi) || []).map(normalizeAviationAirport).filter(Boolean).slice(0, 8);
 const alerts = Array.isArray(payload?.alerts) ? payload.alerts : [];
 const observations = Array.isArray(payload?.observations) ? payload.observations : [];
 const forecasts = Array.isArray(payload?.forecasts) ? payload.forecasts : [];

 const applyRosterAirports = useCallback(() => {
  const next = rosterAirports.join(', ');
  setAirportText(next);
  try { localStorage.setItem('crewcheck_aviation_weather_airports', next); } catch {}
 }, [rosterAirports.join('|')]);

 const loadAlerts = useCallback(async () => {
  setLoading(true);
  try {
   const selected = airports.length ? airports : rosterAirports;
   const nextText = selected.join(', ');
   try { localStorage.setItem('crewcheck_aviation_weather_airports', nextText); } catch {}
   const params = new URLSearchParams();
   params.set('airports', selected.join(','));
   params.set('scope', 'roster');
   ...
   if (!response.ok || !data) throw new Error(data?.message || `HTTP ${response.status}`);
   setPayload(data);
   if (Array.isArray(data.alerts) && data.alerts.length) toast.warning(`${data.alerts.length} alerta(s) meteorológico(s) aeronáutico(s) encontrado(s).`);
   else toast.success(data.message || 'Sem alerta aeronáutico relevante nos aeroportos da escala agora.');
  } catch (error: any) {
   const message = error?.message || 'Falha ao consultar meteorologia aeronáutica.';
   setPayload({ ok: false, alerts: [], message });
   toast.error(message);
  } finally {
   setLoading(false);
  }
 }, [airportText, rosterAirports.join('|')]);

 useEffect(() => {
  if (!airportText.trim() && rosterAirports.length) setAirportText(rosterAirports.join(', '));
 }, [rosterAirports.join('|')]);
 ...
 return (
  <div className="min-h-screen bg-[#07111F] px-5 py-8 text-white">
   <div className="mx-auto max-w-5xl">
    <HeaderBack title="Meteorologia" subtitle="Alertas aeronáuticos por escala" onBack={onBack} action={<button type="button" onClick={() => void loadAlerts()} disabled={loading} className="rounded-2xl bg-cyan-300 px-4 py-3 text-sm font-black text-[#07111f] disabled:opacity-60"><RefreshCw className={`mr-2 inline h-4 w-4 ${loading ? 'animate-spin' : ''}`} />Atualizar</button>} />

    <section className="mt-6 rounded-[2rem] border border-cyan-300/15 bg-cyan-300/10 p-5 shadow-2xl shadow-cyan-950/20">
     <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
      <div>
       <p className="text-xs font-black uppercase tracking-[0.22em] text-cyan-100/65">Foco operacional</p>
       <h2 className="mt-1 text-3xl font-black tracking-tight">Tempestade, CB, vento, teto e visibilidade por origem/destino</h2>
       <p className="mt-2 max-w-3xl text-sm leading-6 text-slate-300">O Alertas+ foi removido. Esta tela consulta METAR/TAF oficiais via backend e destaca risco meteorológico útil para aviação nos aeroportos encontrados na sua escala. METAR é observação atual; TAF é previsão e deve ser conferido com a fonte oficial da operação.</p>
      </div>
      <div className="rounded-2xl border border-emerald-300/20 bg-emerald-300/10 px-4 py-3 text-sm font-black text-emerald-50"><Plane className="mr-2 inline h-4 w-4" />Baseado na escala</div>
     </div>
     <div className="mt-5 grid gap-3 md:grid-cols-[1fr_auto_auto]">
      <input value={airportText} onChange={(event) => setAirportText(event.target.value.toUpperCase())} placeholder="Ex.: SBBR, SBGR, SBSP" className="rounded-2xl border border-white/10 bg-[#07111F]/70 px-4 py-3 text-sm font-black text-white outline-none focus:border-cyan-300" />
      <button type="button" onClick={applyRosterAirports} className="rounded-2xl border border-cyan-300/25 bg-cyan-300/10 px-4 py-3 text-sm font-black text-cyan-50"><Radar className="mr-2 inline h-4 w-4" />Usar escala</button>
      <button type="button" onClick={() => void loadAlerts()} disabled={loading} className="rounded-2xl bg-cyan-300 px-4 py-3 text-sm font-black text-[#07111f] disabled:opacity-60">Consultar</button>
     </div>
     <div className="mt-4 flex flex-wrap items-center gap-2 rounded-2xl border border-white/10 bg-black/15 p-2">
      <span className="px-2 text-[11px] font-black uppercase tracking-[0.18em] text-cyan-100/70">Exibição</span>
      <button type="button" onClick={() => changeWeatherTextMode('translated')} className={`rounded-xl px-3 py-2 text-xs font-black transition ${weatherTextMode === 'translated' ? 'bg-cyan-300 text-[#07111f]' : 'bg-white/10 text-cyan-50 hover:bg-white/15'}`}>Traduzido</button>
      <button type="button" onClick={() => changeWeatherTextMode('raw')} className={`rounded-xl px-3 py-2 text-xs font-black transition ${weatherTextMode === 'raw' ? 'bg-cyan-300 text-[#07111f]' : 'bg-white/10 text-cyan-50 hover:bg-white/15'}`}>Raw</button>
      <span className="px-2 text-xs font-semibold text-cyan-50/65">Traduzido segue glossário operacional: KT em nós, FT em pés, M em menos/metros conforme contexto e ICAO em alfabeto fonético.</span>
     </div>
     <p className="mt-3 text-xs font-semibold text-cyan-50/70">Aeroportos detectados: {(rosterAirports.length ? rosterAirports : ['SBBR']).join(' · ')}</p>
    </section>

    <section className="mt-6 grid gap-4">
     {loading && <StateCard icon={Loader2} title="Consultando meteorologia aeronáutica" text="Lendo METAR/TAF e procurando sinais de tempestade, CB, vento forte, teto baixo e visibilidade reduzida." spin />}
     {!loading && payload && !payload.sourceConfigured && <div className="rounded-[2rem] border border-amber-300/25 bg-amber-300/10 p-6"><div className="flex items-start gap-4"><AlertTriangle className="mt-1 h-7 w-7 shrink-0 text-amber-200" /><div><h3 className="text-xl font-black text-amber-50">Fonte aeronáutica indisponível</h3><p className="mt-2 text-sm leading-6 text-amber-50/85">{payload.message || 'Não consegui consultar a fonte oficial agora. O app não inventa alerta meteorológico.'}</p></div></div></div>}
     {!loading && payload?.sourceConfigured && !alerts.length && <div className="rounded-[2rem] border border-emerald-300/20 bg-emerald-300/10 p-6 text-center"><CheckCircle2 className="mx-auto h-10 w-10 text-emerald-200" /><h3 className="mt-3 text-xl font-black text-emerald-50">Sem alerta aeronáutico relevante</h3><p className="mx-auto mt-2 max-w-2xl text-sm leading-6 text-emerald-50/80">{payload.message || 'Não identifiquei TS/CB, vento forte, teto baixo ou visibilidade crítica nos aeroportos selecionados.'}</p><p className="mt-3 text-xs font-semibold text-emerald-100/70">Atualizado em {formatDefesaCivilDate(payload.updatedAt)}</p></div>}
     {!loading && alerts.map((alert) => <article key={alert.id || `${alert.airport}-${alert.title}`} className={`rounded-[2rem] border p-5 ${aviationSeverityClass(alert.severity)}`}>
      <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between"><div><p className="text-xs font-black uppercase tracking-[0.18em] opacity-75">{alert.source || 'AviationWeather.gov'}</p><h3 className="mt-1 text-2xl font-black">{alert.airport || alert.icao} · {alert.title || 'Alerta meteorológico aeronáutico'}</h3>{alert.summary && <p className="mt-2 text-sm leading-6 opacity-90">{alert.summary}</p>}</div><span className="rounded-full border border-white/15 bg-white/15 px-3 py-1 text-xs font-black uppercase tracking-[0.12em]">{aviationSeverityLabel(alert.severity)}</span></div>
      {!!alert.details?.length && <div className="mt-4 grid gap-2 sm:grid-cols-2">{alert.details.map((detail, idx) => <div key={`${alert.id}-detail-${idx}`} className="rounded-2xl border border-white/10 bg-black/15 px-4 py-3 text-sm font-semibold">{detail}</div>)}</div>}
      {alert.rawMetar && <div className="mt-4 rounded-2xl border border-white/10 bg-black/20 p-4"><p className="text-xs font-black uppercase tracking-[0.16em] opacity-70">METAR {weatherTextMode === 'raw' ? 'RAW' : 'TRADUZIDO'}</p><p className={`mt-1 text-xs leading-5 ${weatherTextMode === 'raw' ? 'font-mono' : 'font-semibold'}`}>{weatherDisplayText(alert.rawMetar, weatherTextMode, 'METAR', alert.icao || alert.airport)}</p></div>}
      {alert.rawTaf && <div className="mt-3 rounded-2xl border border-white/10 bg-black/20 p-4"><p className="text-xs font-black uppercase tracking-[0.16em] opacity-70">TAF {weatherTextMode === 'raw' ? 'RAW' : 'TRADUZIDO'}</p><p className={`mt-1 text-xs leading-5 ${weatherTextMode === 'raw' ? 'font-mono' : 'font-semibold'}`}>{weatherDisplayText(alert.rawTaf, weatherTextMode, 'TAF', alert.icao || alert.airport)}</p></div>}
      <div className="mt-4 grid gap-2 text-xs font-semibold opacity-80 sm:grid-cols-3"><span>Categoria: {alert.category || 'terminal'}</span><span>Emitido: {formatDefesaCivilDate(alert.issuedAt)}</span><span>Válido até: {formatDefesaCivilDate(alert.validUntil)}</span></div>
     </article>)}
     {!loading && !payload && <StateCard icon={Plane} title="Pronto para consultar" text="Use os aeroportos da escala ou informe ICAO/IATA para buscar avisos meteorológicos voltados à operação." />}
    </section>

    <section className="mt-6 rounded-[2rem] border border-white/10 bg-white/[0.045] p-5">
     <p className="text-xs font-black uppercase tracking-[0.22em] text-cyan-100/60">METAR/TAF {weatherTextMode === 'raw' ? 'raw' : 'traduzido'}</p>
     <div className="mt-4 grid gap-3 md:grid-cols-2">
      {observations.map((item: any) => <div key={`metar-${item.icao || item.stationId || item.rawOb}`} className="rounded-2xl border border-white/10 bg-black/15 p-4"><p className="text-sm font-black text-white">{item.icao || item.stationId || 'METAR'}</p><p className={`mt-2 text-xs leading-5 text-slate-300 ${weatherTextMode === 'raw' ? 'font-mono' : 'font-semibold'}`}>{weatherDisplayText(item.rawOb || item.raw || item.text, weatherTextMode, 'METAR', item.icao || item.stationId)}</p></div>)}
      {forecasts.map((item: any) => <div key={`taf-${item.icao || item.stationId || item.rawTAF}`} className="rounded-2xl border border-white/10 bg-black/15 p-4"><p className="text-sm font-black text-white">{item.icao || item.stationId || 'TAF'}</p><p className={`mt-2 text-xs leading-5 text-slate-300 ${weatherTextMode === 'raw' ? 'font-mono' : 'font-semibold'}`}>{weatherDisplayText(item.rawTAF || item.rawTaf || item.raw || item.text, weatherTextMode, 'TAF', item.icao || item.stationId)}</p></div>)}
     </div>
    </section>
   </div>
  </div>
  ...
  <div className="min-h-screen bg-[#08111f] px-5 py-8 text-white">
   <div className="mx-auto max-w-4xl">
    ...
    <div className="mt-6 grid gap-3 sm:grid-cols-2">
     ...
     <MenuAction icon={History} title="Gerenciador de Escalas" text="Ver escala ativa, abrir versões antigas ou excluir erro" onClick={() => onNavigate('history')} />
     <MenuAction icon={ShieldAlert} title="Regulamentação e Jornada" text="Limites de jornada, acionamento, reserva e sobreaviso" onClick={() => onNavigate('regulation')} />
     ...
     <MenuAction icon={ShieldAlert} title="Meteorologia · Alertas" text="METAR/TAF, tempestade e risco aeronáutico por escala" onClick={() => onNavigate('weather')} />
     ...
     <MenuAction icon={FileText} title="Checklist Médico" text="Formulário de ocorrência, PDF e compartilhamento" onClick={() => onNavigate('medical')} />
     <MenuAction icon={Sparkles} title="Sugestões dos colegas" text="Formulário interno para melhorias do CrewCheck" onClick={() => onNavigate('bids')} />
...
function buildImplicitAwayFullDayLayoverSegments(roster: CrewRoster, anchors: PerDiemAnchor[] = buildFlightAnchors(roster)): PerDiemOperationalSegment[] {
 const base = normalizedAirportCode(roster.base || 'BSB').slice(0, 3) || 'BSB';
 const out: PerDiemOperationalSegment[] = [];
 const existingDates = new Set((roster.days || []).filter((day) => isInactiveLayoverLikeDay(day)).map((day) => day.date));
 for (const day of roster.days || []) {
  if (day.legs?.length || existingDates.has(day.date)) continue;
  const code = String(day.type || day.pairingCode || '').toUpperCase();
  if (/^(DO|DOF|DOP|DOPR|OFF|VC|FERIAS|FÉRIAS)$/.test(code)) continue;
  const startAbs = absoluteDayBase(day.date);
  const endAbs = startAbs + 1440;
  const previous = [...anchors].reverse().find((anchor) => anchor.endAbs <= startAbs + 180);
  const next = anchors.find((anchor) => anchor.startAbs >= endAbs - 180);
  const away = normalizedAirportCode(previous?.destination || '').slice(0, 3);
  const nextOrigin = normalizedAirportCode(next?.origin || '').slice(0, 3);
  if (!away || away === base || away !== nextOrigin || isSuppressedBaseVirtualLayover(away)) continue;
  const internationalRule = internationalPerDiemRuleForAirport(away);
  const meta = perDiemMetaFromRule(internationalRule);
  out.push(...splitAbsoluteSegment(startAbs, endAbs, `PERNOITE ${away}`, 'layover', 'estimada', `Pernoite recuperado automaticamente em ${away}: dia sem voo entre chegada e próxima apresentação na mesma estação fora de casa. Gera almoço/jantar; café e ceia não entram por pernoite puro.`, { ...meta, flightKey: `PERNOITE ${away}` }));
 }
 return out;
}

function isAutomaticCrewLayoverGap(current: PerDiemAnchor, next: PerDiemAnchor, gap: number): boolean {
 if (!Number.isFinite(gap) || gap <= 0 || gap > 6 * 24 * 60) return false;
 const currentDay = Math.floor(current.endAbs / 1440);
 const nextDay = Math.floor(next.startAbs / 1440);
 const crossesLocalMidnight = nextDay > currentDay;
 // Pernoite real não depende de 20h: se termina fora da base e a próxima apresentação sai da mesma localidade no dia seguinte,
 // o intervalo já deve gerar almoço/jantar do pernoite. O limite mínimo de 6h evita tratar solo/turnaround como hotel.
 return gap >= 20 * 60 || (crossesLocalMidnight && gap >= 6 * 60);
}


function buildOpenEndedLastDestinationLayoverSegments(roster: CrewRoster, anchors: PerDiemAnchor[] = buildFlightAnchors(roster)): PerDiemOperationalSegment[] {
 const base = normalizedAirportCode(roster.base || 'BSB').slice(0, 3) || 'BSB';
 const out: PerDiemOperationalSegment[] = [];
 for (let i = 0; i < anchors.length; i += 1) {
  const current = anchors[i];
  const away = normalizedAirportCode(current.destination).slice(0, 3);
  if (!away || away === base || isSuppressedBaseVirtualLayover(away)) continue;
  const hasKnownReturnFromAway = anchors.slice(i + 1).some((candidate) => candidate.startAbs > current.endAbs && normalizedAirportCode(candidate.origin).slice(0, 3) === away);
  if (hasKnownReturnFromAway) continue;
  const sameDayEnd = Math.floor(current.endAbs / 1440) * 1440 + 1440;
  // Quando a chave/mês termina fora de casa, a escala seguinte pode não estar carregada.
  // Projetamos 24h conservadoras a partir da chegada para recuperar almoço/jantar do pernoite perdido.
  const projectedEnd = Math.max(sameDayEnd, current.endAbs + 24 * 60);
  if (projectedEnd <= current.endAbs) continue;
  const internationalRule = internationalPerDiemRuleForAirport(away);
  const meta = perDiemMetaFromRule(internationalRule);
  const reason = internationalRule
   ? `Pernoite em aberto fora do Brasil em ${away}: a escala termina fora de casa e a próxima chave pode estar no mês seguinte. Projeção conservadora de 24h para não perder almoço/jantar do pernoite; confirmar no demonstrativo oficial.`
   : `Pernoite em aberto fora de casa em ${away}: último destino do dia/chave e ainda não há próxima saída carregada dessa localidade. O CrewCheck projeta 24h para recuperar almoço/jantar do pernoite perdido; café e ceia não entram por pernoite puro.`;
  out.push(...splitAbsoluteSegment(current.endAbs, projectedEnd, `PERNOITE ${away}`, 'layover', 'estimada', reason, { ...meta, flightKey: current.flightKey || current.code }));
 }
 return out;
}
...
function buildLayoverPerDiemSegments(roster: CrewRoster): PerDiemOperationalSegment[] {
 ...
 for (let i = 0; i < anchors.length; i += 1) {
  ...
  if (!isAutomaticCrewLayoverGap(current, next, gap)) continue;
  const internationalRule = internationalPerDiemRuleForAirport(away);
  const meta = perDiemMetaFromRule(internationalRule);
  const crossesLocalMidnight = Math.floor(next.startAbs / 1440) > Math.floor(current.endAbs / 1440);
  const layoverReason = internationalRule
   ...
   : `Pernoite fora da base em ${away}${crossesLocalMidnight ? ', com virada de dia entre a chegada e a próxima apresentação' : ', com permanência operacional superior a 20h'}. Demonstrativos LATAM pagam almoço/jantar do pernoite automaticamente; café e ceia não são gerados por pernoite puro.`;
 ...
 return [...out, ...buildOpenEndedLastDestinationLayoverSegments(roster, anchors), ...buildExplicitLayoverDayPerDiemSegments(roster, anchors), ...buildImplicitAwayFullDayLayoverSegments(roster, anchors)];
}
...
async function fetchCrewExchangeRates(base: string, symbols: string[]): Promise<ExchangeRatesResponse> {
 ...
 const cleanSymbols = symbols.map((item) => item.toUpperCase());
 const params = new URLSearchParams({ base: cleanBase, symbols: cleanSymbols.join(',') });
 ...
 const missing = cleanSymbols.filter((symbol) => symbol !== cleanBase && !Number(payload?.rates?.[symbol]));
 if (!response.ok || !payload?.ok || !payload.rates || missing.length) throw new Error(payload?.message || `Cotação indisponível para ${missing.join(', ') || cleanSymbols.join(', ')}.`);
 return payload;
}
...
function CurrencyConverterScreen({ bundle, onBack, onOpenPerDiem }: { bundle: SessionRosterBundle | null; onBack: () => void; onOpenPerDiem: () => void }) {
 ...
 const directRate = fromCurrency === toCurrency ? 1 : (ratePayload?.rates?.[toCurrency] || 0);
 const conversionReady = directRate > 0;
 const converted = conversionReady ? amount * directRate : 0;
 ...
 return (
  <div className="min-h-screen bg-[#08111f] px-5 py-8 pb-32 text-white crewcheck-finance-screen crewcheck-tool-screen crewcheck-currency-screen">
   <div className="mx-auto max-w-6xl">
    <HeaderBack title="Conversor de moedas" subtitle="cotação do dia · diárias internacionais" onBack={onBack} action={<button onClick={onOpenPerDiem} className="cc-currency-action cc-currency-action-secondary rounded-2xl px-4 py-3 text-xs font-black">Abrir diárias</button>} />
    ...
    <section className="mt-6 grid gap-4 lg:grid-cols-[minmax(0,1fr)_minmax(20rem,.55fr)]">
     <div className="rounded-[2rem] border border-white/10 bg-white/[0.06] p-5 shadow-2xl shadow-black/20">
      <div className="grid gap-3 md:grid-cols-[1fr_12rem_auto_12rem] md:items-end">
       ...
       <button onClick={swapCurrencies} className="cc-currency-action cc-currency-action-secondary h-14 rounded-2xl px-4 text-xl font-black" title="Inverter moedas">⇄</button>
      ...
      <div className="mt-5 rounded-[1.6rem] border border-cyan-300/20 bg-cyan-300/10 p-5">
       ...
       <p className="mt-2 text-4xl font-black text-cyan-50">{loading ? 'Consultando...' : conversionReady ? formatCurrencyAmount(converted, toCurrency) : '—'}</p>
       <p className="mt-2 text-sm font-semibold text-cyan-50/75">{amount.toLocaleString('pt-BR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })} {fromCurrency} {conversionReady ? `× ${directRate.toLocaleString('pt-BR', { maximumFractionDigits: 6 })}` : ' · cotação aguardando'}</p>
       ...
       <button type="button" onClick={() => void loadRate()} disabled={loading} className="cc-currency-action cc-currency-action-primary mt-4 rounded-2xl px-4 py-3 text-xs font-black">{loading ? 'Atualizando...' : 'Atualizar cotação'}</button>
      </div>
      <div className="mt-4 flex flex-wrap gap-2">
       {['USD','EUR','ARS','GBP'].map((currency) => <button key={currency} onClick={() => { setFromCurrency(currency as CrewCurrencyCode); setToCurrency('BRL'); }} className="cc-currency-chip rounded-full px-4 py-2 text-xs font-black">{currency} → BRL</button>)}
      </div>
     </div>
