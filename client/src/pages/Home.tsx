...
function CrewCheckMobileMenu({ active, onNavigate }: { active: CrewCheckBottomActive; onNavigate: (view: string) => void }) {
 ...
 const items: { id: 'home' | 'roster' | 'vivo' | 'flightboard' | 'settings'; label: string; fullLabel: string; icon: LucideIcon; view: string }[] = [
  ...
  { id: 'vivo', label: 'Extra', fullLabel: 'Extra na Saída Inteligente', icon: Plane, view: 'departureExtra' },
...
function SmartDepartureScreen({ nextEvent, bundle, userBase, hasRoster, canAccessAdmin = false, onBack, onOpenRoster }: { nextEvent?: DashboardNextEvent; bundle?: SessionRosterBundle | null; userBase?: string; hasRoster: boolean; canAccessAdmin?: boolean; onBack: () => void; onOpenRoster: () => void }) {
 ...
 const tomTomRouteUrl = plan?.mapsUrl || '';
...
const supportIsPremium = Boolean(billingStatus?.premiumAccess);
 ...
 async function connectTelegram() {
  ...
  try {
   ...
   if (payload.connected) {
    toast.success('Telegram Premium já está conectado.');
    return;
   }
 ...
 async function saveTelegramPreference(key: string, value: boolean) {
  const next = { ...(telegramStatus?.preferences || {}), [key]: value };
  ...
  try {
   ...
   const payload = await response.json().catch(() => null) as TelegramStatus | null;
   if (payload?.ok) applyTelegramPayload(payload);
  } catch {
   toast.error('Não foi possível salvar a preferência do Telegram Premium.');
  }
 }
  ...
  if (activeCategory === 'notifications') {
   ...
   return (
    <div className="space-y-5">
     ...
     <SettingsCategoryCard eyebrow="Telegram Premium" title="Alertas importantes fora do app" description="Canal executivo de alertas: Saída Inteligente, escala, radar e irregularidades com mensagens mais claras, silenciosas e selecionadas pelo usuário.">
      <div className="flex flex-col gap-3 rounded-[1.6rem] border border-cyan-200/20 bg-cyan-300/10 p-4 sm:flex-row sm:items-center sm:justify-between">
       <div>
        ...
        <h3 className="mt-1 text-2xl font-black text-white">{telegramConnected ? 'Telegram Premium conectado' : telegramConfigured ? 'Telegram Premium disponível' : 'Telegram não configurado'}</h3>
      ...
      <div className="mt-4 grid gap-3 sm:grid-cols-2">
       {[
        ...
        ['radar', 'Radar de voo', 'portão, status e última chamada'],
      ...
      <p className="mt-4 text-xs leading-5 text-slate-400">Para evitar excesso de mensagens, o CrewCheck envia apenas alertas relevantes. O Telegram é um canal extra Premium e pode ser ajustado a qualquer momento.</p>
     </SettingsCategoryCard>
    </div>
   );
  }
...
const INACTIVE_LAYOVER_AIRPORTS = new Set(['JPA', 'CPV', 'MAB', 'VCP']);
...
function detectInactiveLayoverAirport(day: CrewRoster['days'][number], base: string): string {
 ...
 const direct = normalizedAirportCode(day.hotel || day.pairingCode || '');
 if (direct && direct !== base && INACTIVE_LAYOVER_AIRPORTS.has(direct)) return direct;
 const found = source.match(/\b(JPA|CPV|MAB|VCP)\b/);
 return found && found[1] !== base ? found[1] : '';
}

function isInactiveLayoverLikeDay(day: CrewRoster['days'][number]): boolean {
 const source = `${day.type || ''} ${day.pairingCode || ''} ${day.hotel || ''} ${day.rawText || ''}`.toUpperCase();
 return !day.legs?.length && /(INATIV|PERNOITE|LAYOVER|HOTEL|ACOMOD)/.test(source);
}
function buildExplicitLayoverDayPerDiemSegments(roster: CrewRoster): PerDiemOperationalSegment[] {
 ...
 const out: PerDiemOperationalSegment[] = [];
 for (const day of roster.days || []) {
  const explicitLayover = String(day.type || '').toUpperCase() === 'LAYOVER';
  const inactiveLayover = isInactiveLayoverLikeDay(day);
  if (!explicitLayover && !inactiveLayover) continue;
  const away = explicitLayover
   ? normalizedAirportCode(day.hotel || day.pairingCode || '')
   : detectInactiveLayoverAirport(day, base);
  if (!away) continue;
  ...
  const reason = internationalRule
   ...
   : inactiveLayover
    ? `Inativo/pernoite fora da base em ${away}. Tratar como pernoite publicado e gerar almoço/jantar quando o período cobre as janelas; café e ceia não entram em pernoite puro.`
...
function buildPreciseMarkerUrl(lat?: number | null, lng?: number | null): string {
 ...
 const label = encodeURIComponent('Carro salvo pelo CrewCheck');
 return `https://www.openstreetmap.org/?mlat=${latitude.toFixed(6)}&mlon=${longitude.toFixed(6)}#map=18/${latitude.toFixed(6)}/${longitude.toFixed(6)}&layers=M&marker=${latitude.toFixed(6)},${longitude.toFixed(6)}&query=${label}`;
}
...
function buildTomTomGoAndroidIntentUrl(lat?: number | null, lng?: number | null): string {
 ...
 const longitude = Number(lng);
 if (!Number.isFinite(latitude) || !Number.isFinite(longitude)) return '';
 const fallback = encodeURIComponent(buildPreciseMarkerUrl(latitude, longitude));
...
function buildTomTomPlanSearchUrl(lat?: number | null, lng?: number | null): string {
 // O Plan.TomTom bloqueia URLs /search/<coordenadas> em alguns WebViews Android (AccessDenied).
 // Para não abrir mapa em branco, o fallback web do CrewCheck agora sempre usa marcador coordenado.
 return buildPreciseMarkerUrl(lat, lng) || 'https://plan.tomtom.com/';
}
   ...
   <div className="mx-auto max-w-6xl">
    ...
    <section className="mt-6 grid gap-5 lg:grid-cols-[minmax(0,.82fr)_minmax(22rem,.55fr)]">
     ...
     <div className="rounded-[2rem] border border-white/10 bg-white/[0.06] p-5 shadow-2xl shadow-black/20">
      ...
      {coordinateLabel ? <div className="mt-5 rounded-3xl border border-cyan-300/20 bg-cyan-300/10 p-4 text-sm font-semibold leading-6 text-cyan-50/90"><b className="block text-xs uppercase tracking-[0.18em] text-cyan-100/70">Coordenadas do carro</b><span className="mt-1 block font-mono text-base font-black text-white">{coordinateLabel}</span><span className="mt-1 block text-xs text-cyan-50/70">Se o TomTom GO não estiver instalado, o CrewCheck abre um marcador web preciso com as coordenadas salvas.</span></div> : null}
