...
type HomeView = 'home' | 'import' | 'history' | 'calendar' | 'iflight' | 'flightboard' | 'departure' | 'routine' | 'perdiem' | 'salary' | 'currency' | 'parking' | 'c32f' | 'settings' | 'support' | 'notes' | 'chief' | 'medical' | 'bids' | 'admin' | 'systemstatus' | 'more';
...
function dashboardRelationForTarget(target: Date, endTarget: Date, todayStart: Date, now: Date, afterRest: boolean): { relation: DashboardNextEvent['relation']; relationLabel: string; dayContext: string; isToday: boolean; isTomorrow: boolean; isInProgress: boolean; isAfterRest: boolean } {
 ...
 if (afterRest) {
  ...
  return { relation: 'rest_followup', relationLabel: isTomorrow ? 'A seguir · amanhã' : `A seguir · ${target.toLocaleDateString('pt-BR', { day: '2-digit', month: '2-digit' })}`, dayContext: `Hoje consta folga/monofolga. A programação exibida é ${when}; o CrewCheck antecipa a leitura para você se organizar sem confundir com hoje.`, isToday, isTomorrow, isInProgress, isAfterRest: true };
 }
 ...
 return { relation: 'future', relationLabel: `A seguir · ${target.toLocaleDateString('pt-BR', { day: '2-digit', month: '2-digit' })}`, dayContext: `A próxima programação operacional está em ${daysAway} dias.`, isToday, isTomorrow, isInProgress, isAfterRest: false };
}
...
export default function Home() {
 ...
 useEffect(() => {
  try {
   ...
   if (target && ['home','import','history','calendar','iflight','flightboard','departure','routine','perdiem','salary','currency','parking','c32f','settings','support','notes','more'].includes(target)) {
        ...
        <div onDragOver={onDragOver} onDragLeave={onDragLeave} onDrop={onDrop} className={`crewcheck-light-card crewcheck-import-dropzone-v1109 relative rounded-[1.35rem] border-2 border-dashed p-8 text-center transition-all duration-300 ${isDragging ? 'scale-[1.015] border-cyan-300 bg-cyan-300/10' : 'border-white/15 bg-white/[0.04] hover:border-fuchsia-300/60 hover:bg-white/[0.07]'} ${isProcessing ? 'pointer-events-none opacity-70' : ''}`}>
         {isProcessing ? (
          ...
          <div className="flex flex-col items-center gap-5 py-4">
           ...
           <p className="max-w-md text-xs leading-5 text-slate-400">No iPhone/iPad, prefira Arquivos → No iPhone/iCloud Drive. Se veio do WhatsApp, salve em Arquivos antes para o PDF ser lido corretamente.</p>
          </div>
         )}
        </div>
...
function ParkingSpotScreen({ onBack }: { onBack: () => void }) {
 ...
 const mapsUrl = spot?.lat && spot?.lng ? `https://www.google.com/maps/search/?api=1&query=${spot.lat},${spot.lng}` : '';
 ...
 const copyParking = async () => {
  const text = [`Meu carro está salvo no CrewCheck.`, terminal && `Terminal: ${terminal}`, floor && `Piso: ${floor}`, sector && `Setor: ${sector}`, row && `Vaga/fileira: ${row}`, note && `Observação: ${note}`, mapsUrl && `Mapa: ${mapsUrl}`].filter(Boolean).join('\n');
  try {
   if (navigator.share) await navigator.share({ title: 'Local do carro', text, url: mapsUrl || undefined });
    ...
    <section className="mt-6 grid gap-5 lg:grid-cols-[minmax(0,.82fr)_minmax(22rem,.55fr)]">
     ...
     <div className="rounded-[2rem] border border-white/10 bg-white/[0.06] p-5 shadow-2xl shadow-black/20">
      ...
      <div className="mt-4 grid gap-3">
       ...
       <button onClick={() => mapsUrl ? window.open(mapsUrl, '_blank', 'noopener,noreferrer') : toast.info('Salve uma localização GPS primeiro.')} className="rounded-3xl border border-white/10 bg-white/[0.07] px-5 py-4 text-left text-sm font-black text-white hover:bg-white/[0.11]"><Navigation className="mr-2 inline h-5 w-5 text-cyan-200" />Abrir no Google Maps</button>
