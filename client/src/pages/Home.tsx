...
import { getDatabaseStatus, listSavedRosters, openSavedRoster, getLatestSavedRoster, type DatabaseStatus, type SavedRosterSummary } from '@/lib/databaseClient';
...
function writeCurrentRosterBundle(bundle: SessionRosterBundle, sourceFileName = 'Última escala salva'): void {
 try {
  sessionStorage.setItem('crewcheck_roster', JSON.stringify(normalizeRosterSchedule(detectAndMarkLayovers(bundle.roster))));
  ...
  // Se a sessão estiver indisponível, o app continua permitindo nova importação.
 }
}
...
function Home() {
 ...
 useEffect(() => {
  if (autoLatestLoaded || savedLoading || hasCurrentRosterSession()) return;
  let cancelled = false;
  setAutoLatestLoaded(true);
  getLatestSavedRoster()
   .then((latest) => {
    if (cancelled || !latest?.roster) return;
    writeCurrentRosterBundle({ roster: latest.roster, compliance: latest.compliance, gym: latest.gym || [] }, latest.summary?.sourceFileName || 'Última escala salva');
    sessionStorage.setItem('crewcheck_role_selection', roleSelection);
    try {
     localStorage.setItem(`crewcheck_latest_roster_bundle_${crewcheckCurrentUserScope()}`, JSON.stringify({ roster: latest.roster, compliance: latest.compliance, gym: latest.gym || [], sourceFileName: latest.summary?.sourceFileName || 'Última escala salva', updatedAt: new Date().toISOString(), source: 'database-latest-on-open', userScope: crewcheckCurrentUserScope() }));
    } catch {}
    toast.success('Última escala salva carregada para esta conta.');
    setDashboardClockTick(Date.now());
   ...
   .catch(() => {
    ...
    openSavedRoster(latest.id)
     .then((data) => {
      if (cancelled) return;
      writeCurrentRosterBundle({ roster: data.roster, compliance: data.compliance, gym: data.gym || [] }, latest.sourceFileName || 'Última escala salva');
      sessionStorage.setItem('crewcheck_role_selection', roleSelection);
      setDashboardClockTick(Date.now());
      window.setTimeout(() => window.dispatchEvent(new CustomEvent('crewcheck:refresh-smart-departure')), 450);
     })
...
function CrewCheckMobileMenu({ active, onNavigate }: { active: CrewCheckBottomActive; onNavigate: (view: string) => void }) {
 ...
 const items: { id: 'home' | 'roster' | 'vivo' | 'flightboard' | 'settings'; label: string; fullLabel: string; icon: LucideIcon; view: string }[] = [
  ...
  { id: 'vivo', label: 'Extra', fullLabel: 'Voo de Extra na Saída Inteligente', icon: Plane, view: 'departureExtra' },
...
function CrewCheckFloatingSpeedMenu({ active, onNavigate, canAccessC32FAcademy, hasRoster }: { active: 'home' | 'roster' | 'iflight' | 'departure' | 'settings' | 'more'; onNavigate: (view: string) => void; canAccessC32FAcademy: boolean; hasRoster: boolean }) {
 ...
 const quickItems: Array<{ label: string; hint: string; icon: LucideIcon; view: string; premium?: boolean }> = [
  ...
  { label: 'Voo de Extra', hint: 'grátis quando usar dados locais', icon: Radar, view: 'departureExtra' },
...
function BillingComparisonTable({ free, monthly, annual, trialDays }: { free: CrewCheckPlanLike; monthly: CrewCheckPlanLike; annual: CrewCheckPlanLike; trialDays: number }) {
 const rows = [
  ...
  { label: 'Voo de Extra', free: 'Cálculo local e simulações', monthly: 'Fontes externas quando disponíveis', annual: 'Fontes externas quando disponíveis' },
  ...
  <div className="min-h-screen bg-[#08111f] px-5 py-8 text-white">
   <div className="mx-auto max-w-4xl">
    ...
    <div className="mt-6 grid gap-3 sm:grid-cols-2">
     ...
     <MenuAction icon={Radar} title="Voo de Extra" text="Melhor opção, voos reais e conexões cabíveis" onClick={() => onNavigate('departureExtra')} />
