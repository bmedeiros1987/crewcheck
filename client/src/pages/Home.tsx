...
function CrewCheckFloatingSpeedMenu({ active, onNavigate, canAccessC32FAcademy, hasRoster }: { active: 'home' | 'roster' | 'iflight' | 'departure' | 'settings' | 'more'; onNavigate: (view: string) => void; canAccessC32FAcademy: boolean; hasRoster: boolean }) {
 ...
 const quickItems: Array<{ label: string; hint: string; icon: LucideIcon; view: string; premium?: boolean }> = [
  ...
  { label: 'Vivo de Extra', hint: 'grátis quando usar dados locais', icon: Radar, view: 'departureVivo' },
   ...
   {open && (
    <div className="crewcheck-speed-dial-panel mb-3 w-[min(21rem,calc(100vw-2rem))] rounded-[1.55rem] border p-3 shadow-2xl backdrop-blur-2xl">
     <div className="mb-2 flex items-center justify-between px-2">
      ...
      <span className="rounded-full px-2 py-1 text-[9px] font-black uppercase tracking-[0.12em]">v11.0.11</span>
     </div>
  ...
  <SettingsCategoryCard eyebrow="Assinatura" title="Premium CrewCheck" description={`Mais do que ver escala: organização de rotina, descanso priorizado e tempo de qualidade. Mensal promocional por ${promoMonths} meses para contratações até 19 de julho.`}>
   ...
   <div className="mt-5 grid gap-4 xl:grid-cols-2">
    ...
    <Version11FeatureBox
     ...
     title="Premium para manter APIs e estrutura"
     description="O plano gratuito recebe tudo que não depende de custo recorrente. O Premium financia consultas pagas, servidores, suporte e evolução contínua."
     items={[
      'Consultas de voo/portão quando houver API paga',
      'Radar com cache premium e maior disponibilidade',
      'Suporte direto e prioridade de correções',
      'Infraestrutura, banco e segurança no ar',
      'Integrações externas com custo recorrente',
...
function BillingComparisonTable({ free, monthly, annual, trialDays }: { free: CrewCheckPlanLike; monthly: CrewCheckPlanLike; annual: CrewCheckPlanLike; trialDays: number }) {
 const rows = [
  ...
  { label: 'Saída Inteligente', free: 'Completa com recursos locais/GPS', monthly: 'APIs externas quando disponíveis', annual: 'APIs externas quando disponíveis' },
  ...
  { label: 'Rotina e descanso', free: 'Sugestões completas sem API paga', monthly: 'Prioridade em melhorias', annual: 'Prioridade em melhorias' },
  ...
  { label: 'Vivo de Extra', free: 'Cálculo local e simulações', monthly: 'Fontes externas quando houver custo', annual: 'Fontes externas quando houver custo' },
  { label: 'Radar de voos', free: 'Resumo local/manual', monthly: 'Portão, status e cache via API paga', annual: 'Portão, status e cache via API paga' },
  { label: 'Diárias, salário e moedas', free: 'Cálculo e conversor sem API paga', monthly: 'Sustenta servidores e cache', annual: 'Sustenta servidores e cache' },
  { label: 'Agenda e histórico', free: 'Histórico por usuário e exportação local', monthly: 'Sincronizações externas quando disponíveis', annual: 'Sincronizações externas quando disponíveis' },
  ...
  <div className="mt-6 rounded-[1.8rem] border border-cyan-200/18 bg-white/[0.055] p-4 shadow-2xl shadow-black/15 backdrop-blur-2xl sm:p-5">
   <div className="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
    <div>
     ...
     <p className="mt-2 max-w-3xl text-sm leading-6 text-slate-400">O plano gratuito passa a incluir todas as ferramentas que não geram custo recorrente de API. O Premium sustenta servidor, consultas de voo, integrações externas, suporte e melhorias — com foco em previsibilidade, descanso e qualidade de vida.</p>
    </div>
