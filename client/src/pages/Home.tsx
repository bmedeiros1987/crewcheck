        ...
        <div onDragOver={onDragOver} onDragLeave={onDragLeave} onDrop={onDrop} className={`crewcheck-light-card crewcheck-import-dropzone-v1109 relative rounded-[1.35rem] border-2 border-dashed p-8 text-center transition-all duration-300 ${isDragging ? 'scale-[1.015] border-cyan-300 bg-cyan-300/10' : 'border-white/15 bg-white/[0.04] hover:border-fuchsia-300/60 hover:bg-white/[0.07]'} ${isProcessing ? 'pointer-events-none opacity-70' : ''}`}>
         {isProcessing ? (
          ...
          <div className="flex flex-col items-center gap-5 py-4">
           ...
           <p className="max-w-md text-xs leading-5 text-slate-400">No iPhone/iPad, prefira Arquivos → No iPhone/iCloud Drive. Se veio do WhatsApp, salve em Arquivos antes para o PDF ser lido corretamente. A demonstração usa dados fictícios e é desativada assim que você importa sua primeira escala real.</p>
          </div>
         )}
        </div>
 ...
 return cleanAirportMonitorRows(rows)
  ...
  .map((row) => {
   ...
   return {
    ...
    title: 'Voo de Extra direto · monitor/radar',
    route: `${origin} → ${target}`,
  ...
  if (activeCategory === 'notifications') {
   return (
    <SettingsCategoryCard eyebrow="Permissões" title="Localização, notificações e Saída Inteligente" description="O CrewCheck usa sua localização apenas sob permissão para calcular hora de sair, transporte público, carro e corrida por app.">
     ...
     <div className="mt-5 grid gap-3 sm:grid-cols-3"><button onClick={() => window.dispatchEvent(new CustomEvent('crewcheck:request-native-location'))} className="rounded-2xl bg-cyan-300 px-5 py-4 text-sm font-black text-[#07111f]">Verificar localização</button><button onClick={() => window.dispatchEvent(new CustomEvent('crewcheck:request-notifications'))} className="rounded-2xl border border-cyan-300/30 bg-cyan-300/10 px-5 py-4 text-sm font-black text-cyan-50">Verificar notificações</button><button onClick={() => window.dispatchEvent(new CustomEvent('crewcheck:request-background-mode'))} className="rounded-2xl border border-emerald-300/30 bg-emerald-300/10 px-5 py-4 text-sm font-black text-emerald-50">Permitir segundo plano</button></div>
  ...
  <SettingsCategoryCard eyebrow="Assinatura" title="Premium CrewCheck" description={`Mais do que ver escala: organização de rotina, descanso priorizado e tempo de qualidade. Mensal promocional por ${promoMonths} meses para contratações até 19 de julho.`}>
   ...
   <div className="mt-5 grid gap-4 lg:grid-cols-3">
    ...
    <PlanCard
     ...
     features={["Tudo do Premium mensal", "Melhor custo-benefício", "Ajuda a manter servidor e consultas externas", "Prioridade em melhorias premium", "Menos cobranças durante o ano"]}
   ...
   <div className="mt-5 grid gap-4 xl:grid-cols-2">
    ...
    <Version11FeatureBox
     ...
     title="Premium para manter consultas e estrutura"
     ...
     items={[
      'Consultas de voo/portão quando houver serviço externo pago',
...
function BillingComparisonTable({ free, monthly, annual, trialDays }: { free: CrewCheckPlanLike; monthly: CrewCheckPlanLike; annual: CrewCheckPlanLike; trialDays: number }) {
 const rows = [
  ...
  { label: 'Saída Inteligente', free: 'Completa com recursos locais/GPS', monthly: 'serviços externos quando disponíveis', annual: 'serviços externos quando disponíveis' },
  ...
  { label: 'Diárias, salário e moedas', free: 'Cálculo e conversor sem serviço pago', monthly: 'Sustenta servidores e cache', annual: 'Sustenta servidores e cache' },
  ...
  <div className="mt-6 rounded-[1.8rem] border border-cyan-200/18 bg-white/[0.055] p-4 shadow-2xl shadow-black/15 backdrop-blur-2xl sm:p-5">
   <div className="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
    <div>
     ...
     <p className="mt-2 max-w-3xl text-sm leading-6 text-slate-400">O plano gratuito passa a incluir todas as ferramentas que não geram custo recorrente com serviços externos. O Premium sustenta servidor, consultas de voo, integrações externas, suporte e melhorias — com foco em previsibilidade, descanso e qualidade de vida.</p>
    </div>
