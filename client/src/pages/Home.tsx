...
type HomeView = 'home' | 'import' | 'history' | 'calendar' | 'billing' | 'iflight' | 'flightboard' | 'departure' | 'routine' | 'perdiem' | 'salary' | 'currency' | 'parking' | 'weather' | 'regulation' | 'c32f' | 'settings' | 'support' | 'notes' | 'chief' | 'medical' | 'bids' | 'admin' | 'systemstatus' | 'more';
...
function persistFreshRosterBundle(rosterInput: CrewRoster, roleSelection: CrewRoleSelection, sourceFileName: string) {
 ...
 persistRosterPayloadLocally({ ...bundle, sourceFileName, roleSelection, updatedAt: new Date().toISOString(), source: 'crewcheck-ios-persistent-fallback-v11108', userScope: crewcheckCurrentUserScope() });
 return bundle;
}
...
function regDefaultDutyLimitHours(crewType: RegulationCrewType, stages: number, flightKind: RegulationFlightKind, acclimated: boolean, extension: boolean): number {
 let base = 11;
...
function regDefaultFlightLimitHours(crewType: RegulationCrewType, stages: number, flightKind: RegulationFlightKind): number {
 if (crewType === 'simples') return stages >= 4 ? 8 : 9;
...
function RegulationCalculatorPanel({ onBack, onOpenScaleAlerts }: { onBack: () => void; onOpenScaleAlerts: () => void }) {
 ...
 const [combinedLimit, setCombinedLimit] = useState(16);

 const baseDutyLimit = regDefaultDutyLimitHours(crewType, stages, flightKind, acclimated, extension);
 const dutyLimitHours = manualLimit.trim() ? Math.max(1, Number(manualLimit) || baseDutyLimit) : baseDutyLimit;
 const flightLimitHours = regDefaultFlightLimitHours(crewType, stages, flightKind);
 ...
 const dutyEndNoExtension = regAddMinutes(presentation, regDefaultDutyLimitHours(crewType, stages, flightKind, acclimated, false) * 60);
 const dutyEndWithExtension = regAddMinutes(presentation, regDefaultDutyLimitHours(crewType, stages, flightKind, acclimated, true) * 60);
 ...
 const standbyCombinedDeadline = regAddMinutes(standbyStart, combinedLimit * 60);
 ...
 const reserveCombinedDeadline = regAddMinutes(reserveStart, combinedLimit * 60);
   ...
   <div className="mx-auto max-w-6xl space-y-4">
    <div className="rounded-[2rem] border border-white bg-white/90 p-5 shadow-[0_18px_60px_rgba(20,54,84,0.10)] backdrop-blur-xl">
     ...
     <div className="grid gap-4 lg:grid-cols-[minmax(0,1fr)_22rem] lg:items-end">
      <div>
       <p className="text-xs font-black uppercase tracking-[0.22em] text-sky-600">Regulamentação CrewCheck</p>
       <h1 className="mt-2 text-3xl font-black leading-tight sm:text-4xl">Calculadora de jornada, reserva e sobreaviso.</h1>
       <p className="mt-2 max-w-3xl text-sm font-semibold leading-6 text-[#60758a]">Este menu agora funciona como calculadora operacional: você informa o horário de apresentação/início e o sistema mostra até que horas pode ir a jornada, o corte dos motores e o encerramento. A aba de alertas da escala fica separada para não misturar auditoria automática com consulta manual.</p>
      </div>
      <div className="rounded-3xl border border-sky-100 bg-sky-50 p-4 text-sm font-semibold leading-6 text-sky-900">
       ...
       Sobreaviso/reserva com acionamento compara o limite normal da jornada com o teto combinado parametrizado. Por padrão, o teto combinado usado é <b>{combinedLimit}h</b>.
      </div>
     </div>
    </div>
    ...
    <div className="grid gap-4 xl:grid-cols-[minmax(0,1fr)_24rem]">
     <section className="rounded-[2rem] border border-white bg-white p-5 shadow-[0_18px_60px_rgba(20,54,84,0.08)]">
      ...
      <div className="grid gap-3 md:grid-cols-2">
       ...
       <RegulationField label="Etapas">
        <input type="number" min={1} max={8} value={stages} onChange={(event) => setStages(Math.max(1, Math.min(8, Number(event.target.value) || 1)))} className={inputClass} />
       </RegulationField>
       ...
       <RegulationField label="Teto combinado SAV/Reserva">
        <input type="number" step="0.5" min={8} max={24} value={combinedLimit} onChange={(event) => setCombinedLimit(Math.max(8, Math.min(24, Number(event.target.value) || 16)))} className={inputClass} />
       </RegulationField>
      </div>
     ...
     <aside className="space-y-4">
      <div className="rounded-[2rem] border border-white bg-white p-5 shadow-[0_18px_60px_rgba(20,54,84,0.08)]">
       <div className="flex items-center gap-3"><div className="flex h-12 w-12 items-center justify-center rounded-2xl bg-sky-100 text-sky-700"><ShieldAlert className="h-6 w-6" /></div><div><p className="text-xs font-black uppercase tracking-[0.16em] text-sky-600">Modo confiável</p><h3 className="text-lg font-black">Consulta manual primeiro</h3></div></div>
       <p className="mt-3 text-sm font-semibold leading-6 text-[#60758a]">Os alertas automáticos da escala ficam em segundo plano. Para uma dúvida real de jornada, informe a apresentação e use o resultado desta calculadora.</p>
...
function Home() {
 ...
 const handleDashboardNavigate = useCallback((view: string) => {
  const premiumViews = new Set(['flightboard','departure','smartDeparture','departureExtra','perdiem','salary','currency','parking','weather','chief','medical','bids','reports','alerts']);
...
function CrewCheckMobileMenu({ active, onNavigate, canAccessAdmin = false, drawerOpen = false, onClose }: { active: CrewCheckBottomActive; onNavigate: (view: string) => void; canAccessAdmin?: boolean; drawerOpen?: boolean; onClose?: () => void }) {
 ...
 const groups: Array<{ key: string; label: string; icon: LucideIcon; items: Array<{ label: string; icon: LucideIcon; view: string; activeId?: CrewCheckBottomActive; admin?: boolean; hint?: string }> }> = [
  ...
  { key: 'relatorios', label: 'Relatórios', icon: BarChart3, items: [
   ...
   { label: 'Chefe de Cabine', icon: ClipboardList, view: 'chief', activeId: 'more', hint: 'caderno e bordo' },
  ]},
   ...
   {open && (
    <div className="crewcheck-speed-dial-panel mb-3 w-[min(21rem,calc(100vw-2rem))] rounded-[1.55rem] border p-3 shadow-2xl backdrop-blur-2xl">
     <div className="mb-2 flex items-center justify-between px-2">
      ...
      <span className="rounded-full px-2 py-1 text-[9px] font-black uppercase tracking-[0.12em]">v11.0.95</span>
     </div>
...
function SmartDepartureScreen({ nextEvent, bundle, userBase, hasRoster, canAccessAdmin = false, onBack, onOpenRoster }: { nextEvent?: DashboardNextEvent; bundle?: SessionRosterBundle | null; userBase?: string; hasRoster: boolean; canAccessAdmin?: boolean; onBack: () => void; onOpenRoster: () => void }) {
 ...
 useEffect(() => {
  const items = awayHomePlan.layovers.slice(0, 6);
