...
function authRoleSelectionFromRank(rank?: string | null) {
  ...
  if (/(CCM|CC|CCF|CMS|COMISS|CABIN|CHEFE|PUR|SENIOR)/.test(clean)) return 'cabin';
  if (/(PILOTO|PILOT|COMANDANTE|CMTE|CMT|CMD|CAPT|CAP|COPILOTO|CO PILOTO|CO-PILOTO|FO|F\/O|FIRST OFFICER|SIC|P1|P2)/.test(clean)) return 'pilot';
  return 'auto';
}
...
export default function AuthPage() {
  ...
  function validateBeforeSubmit(): boolean {
    ...
    if (mode === 'register') {
      ...
      if (form.phoneLocal.trim() && normalizeAuthPhoneDigits(form.phoneLocal).length < 8) missing.add('phoneLocal');
    }
              ...
              <form onSubmit={submit} noValidate className="space-y-3">
                ...
                {mode === 'register' && (
                  <Field icon={Plane} label="Função / regulamentação automática">
                    <select value={form.rank} onChange={(e) => update('rank', e.target.value)} className="field-input">
                      {AUTH_FUNCTION_OPTIONS.map((option) => <option key={option.value || 'auto'} value={option.value}>{option.label}</option>)}
                    </select>
                    <p className="mt-2 text-[11px] font-semibold leading-5 text-cyan-50/75">
                      O CrewCheck usa essa função para aplicar automaticamente a ACT correta, liberar menus específicos e evitar cálculo com regra errada. Se deixar em branco, tentaremos detectar pela escala.
                    </p>
                  </Field>
                )}
                ...
                {mode === 'register' && (
                  <div className="rounded-2xl border border-amber-200/20 bg-amber-300/10 p-4">
                    <div className="flex items-center gap-2 text-xs font-black uppercase tracking-[0.16em] text-amber-100/80"><PhoneCall className="h-3.5 w-3.5" /> Telefone para despertar inteligente</div>
                    <p className="mt-1 text-xs leading-5 text-amber-50/80">Usado somente para ligação/alerta do Despertador Premium. Pode ser preenchido agora ou depois em Perfil.</p>
                    <div className="mt-3 grid gap-2 sm:grid-cols-[.9fr_1.1fr]">
                      <select value={form.phoneCountryIso} onChange={(e) => { const country = authCountryByIso(e.target.value); setForm((prev) => ({ ...prev, phoneCountryIso: country.iso, phoneCountryCode: country.code })); }} className="field-input">
                        {AUTH_COUNTRY_PHONE_OPTIONS.map((country) => <option key={country.iso} value={country.iso}>{country.name} {country.code}</option>)}
                      </select>
                      <input inputMode="tel" value={form.phoneLocal} onChange={(e) => update('phoneLocal', e.target.value)} className={`field-input ${invalidFields.has('phoneLocal') ? 'ring-2 ring-rose-300/80' : ''}`} placeholder={authCountryByIso(form.phoneCountryIso).example} aria-invalid={invalidFields.has('phoneLocal')} />
                    </div>
                    {invalidFields.has('phoneLocal') && <p className="mt-2 text-[11px] font-bold text-rose-100">Confira o número com DDD/local. O país já adiciona o código internacional.</p>}
                    <p className="mt-2 text-[11px] font-bold text-amber-50/70">Formato salvo: {composeAuthPhoneE164(authCountryByIso(form.phoneCountryIso).code, form.phoneLocal) || `${authCountryByIso(form.phoneCountryIso).code} ...`}</p>
                  </div>
                )}
