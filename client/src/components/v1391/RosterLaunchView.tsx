  ...
  return <>
    ...
    <section className="cc139-list">
      {ordered.map((event) => {
        ...
        return <article key={event.id} className={`cz-roster-card cc139-card ${mode === 'operating' ? 'cc-flight-operating' : mode === 'extra' ? 'cc-flight-extra' : mode === 'stay' ? 'cc-roster-stay' : ''}`} data-work-mode={mode} data-event-kind={event.kind || ''}>
          <header><span><strong>{cardTitle(event, mode)}</strong><small>{formatDate(dateOf(event))} · {modeLabel(mode)}</small></span>{mode === 'operating' || mode === 'extra' ? <Plane/> : mode === 'stay' ? atBase ? <Home/> : <BedDouble/> : <ShieldCheck/>}</header>
          {(mode === 'operating' || mode === 'extra') && <p>{event.presentation && event.presentation !== 'Conexão/Solo' ? `Apresentação ${event.presentation} · ` : ''}{event.departure || '—'} → {event.arrival || '—'} · {event.subtitle || ''}</p>}
          {mode === 'stay' && <p>{hours ? `${hours.toFixed(1)} h entre o fim da jornada e a próxima apresentação.` : 'Intervalo de continuidade entre jornadas.'} {event.hotel ? `Hotel: ${event.hotel}.` : atBase ? 'Use o endereço de casa salvo no Gerenciador de apresentação.' : 'Hotel ainda não informado.'}</p>}
          {mode === 'rest' && <p>{event.subtitle || 'Código preservado conforme a escala publicada.'}</p>}
          {mode === 'duty' && <p>{event.subtitle || `${event.departure || '—'} → ${event.arrival || '—'}`}</p>}
          <div className="cc-roster-card-details">
      ...
      {!ordered.length && <article className="cc139-card cc139-empty"><CalendarDays/><h2>Nenhuma escala carregada</h2><p>Importe o PDF ou sincronize o calendário autorizado do iFlight.</p></article>}
    </section>
  </>;
}
