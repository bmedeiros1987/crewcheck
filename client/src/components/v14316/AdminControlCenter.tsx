...
export default function AdminControlCenter() {
  ...
  const mapsNeedsConfiguration = Boolean(maps && (!maps.googleConfigured || !maps.tomtomConfigured));
  ...
  return <section className="cc-control-shell">
    ...
    <section className="cc-control-card cc-maps-budget" data-maps-budget-status={budget?.blocked || maps?.ok === false ? 'fallback' : 'google'}>
      ...
      {budget ? <>
        <div className="cc-maps-budget-head"><Navigation/><div><strong>{budget.blocked ? 'TomTom ativo automaticamente' : maps?.googleConfigured ? 'Google Routes é o provedor principal' : 'Google Routes requer configuração'}</strong><span>{maps?.message}</span></div><b>{budget.used.toLocaleString('pt-BR')} / {budget.limit.toLocaleString('pt-BR')}</b></div>
        ...
        <div className="cc-maps-budget-facts"><span><small>Restante</small><strong>{budget.remaining.toLocaleString('pt-BR')}</strong></span><span><small>Uso</small><strong>{budgetPercent.toLocaleString('pt-BR')}%</strong></span><span><small>Google Routes</small><strong>{maps?.googleConfigured ? 'Configurado' : 'Não configurado'}</strong></span><span><small>TomTom</small><strong>{maps?.tomtomConfigured ? 'Pronto para contingência' : 'Não configurado'}</strong></span><span><small>Fallback</small><strong>{maps?.fallback || 'TomTom'}</strong></span><span><small>Reinício</small><strong>{dateTimeLabel(budget.resetAt)}</strong></span><span><small>Controle</small><strong>{budget.persistence === 'database' ? 'Banco persistente' : 'Contingência em memória'}</strong></span></div>
        {mapsNeedsConfiguration && <div className="cc-maps-budget-alert"><ShieldCheck/><span>Configuração incompleta: {maps?.googleConfigured ? 'Google Routes pronto' : 'Google Routes sem chave server-side válida'} · {maps?.tomtomConfigured ? 'TomTom pronto' : 'TomTom sem chave de contingência'}. Os valores das chaves permanecem ocultos.</span></div>}
        {maps?.ok === false && !mapsNeedsConfiguration && <div className="cc-maps-budget-alert"><ShieldCheck/><span>Os provedores estão configurados, mas o diagnóstico de rotas não está saudável. Verifique autorização, API habilitada, faturamento, quota e resposta do provedor antes de atribuir a falha à interface.</span></div>}
