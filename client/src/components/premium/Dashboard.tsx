...
const Dashboard: React.FC<DashboardProps> = ({
  ...
  const menuItems = [
    ...
    { icon: CreditCard, label: 'Assinatura', caption: 'gerenciar Premium e cobrança', color: 'from-amber-200 to-orange-500', onClick: () => onNavigate('billing') },
    ...(canAccessC32FAcademy ? [{ icon: Plane, label: 'iFlight Admin', caption: 'AutoPull admin', color: 'from-orange-300 to-amber-600', onClick: () => onNavigate('iflight') }] : []),
    ...
    { icon: TrendingUp, label: 'Salário', caption: 'chefia, bruto e líquido', color: 'from-lime-300 to-emerald-600', onClick: () => onNavigate('salary') },
        ...
        <aside className={`crewcheck-v10855-home-sidebar hidden rounded-[2rem] border border-white/10 bg-white/[0.045] p-3 shadow-2xl shadow-black/30 backdrop-blur-2xl lg:flex lg:flex-col ${sidebarCollapsed ? 'is-collapsed' : ''}`}>
          <div className={`flex items-center gap-3 ${sidebarCollapsed ? 'justify-center' : 'justify-between'}`}>
            <button onClick={() => onNavigate('home')} className="flex min-w-0 items-center gap-3 rounded-3xl p-2 text-left transition hover:bg-white/[0.07]" title="CrewCheck Home">
              <img src="/icons/crewcheck-icon.svg" alt="CrewCheck" className="h-12 w-12 rounded-2xl shadow-2xl shadow-cyan-950/40" />
        ...
        <main className="min-w-0 space-y-5">
            ...
            <div className="relative z-10 max-w-2xl">
              <div className="flex items-start gap-4">
                ...
                <div className="min-w-0">
                  ...
                  <p className="mt-3 text-sm font-semibold text-slate-300 sm:text-base">Home Ultra Premium · {user.base || 'BSB'} · escala, rotina reliable, radar, saída inteligente e alertas em leitura rápida.</p>
                </div>
              </div>
          ...
          <section className="crewcheck-cockpit-module-center rounded-[2rem] border border-cyan-200/12 bg-white/[0.04] p-4 shadow-2xl shadow-black/20 backdrop-blur-2xl">
            ...
            <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4">
              {menuItems.filter((item) => item.label !== 'Cockpit').map((item) => <MenuCard key={item.label} {...item} compact />)}
            </div>
          </section>
        </main>
      </div>

      <nav className="crewcheck-dashboard-bottom-nav fixed bottom-0 left-0 right-0 z-50 px-4 pb-[calc(0.9rem+env(safe-area-inset-bottom,0px))] pt-2.5 lg:hidden">
        <div className="crewcheck-bottom-menu-shell mx-auto flex max-w-md items-center justify-between gap-1">
          ...
          <NavItem icon={Monitor} label="Radar" onClick={() => onNavigate('flightboard')} />
          <NavItem icon={Navigation} label="Saída" onClick={() => onNavigate('departure')} />
          <NavItem icon={Settings} label="Conta" onClick={() => onNavigate('settings')} />
        </div>
      </nav>
    </div>
  );
...
function NextProgramBoardingPass({ nextEvent, hasRoster, canAccessAdmin, onOpenRoster, onImport, onSync }: { nextEvent?: DashboardProps['nextEvent']; hasRoster: boolean; canAccessAdmin?: boolean; onOpenRoster: () => void; onImport: () => void; onSync: () => void }) {
  ...
  const sideText = nextEvent?.dayContext || (requiresDeparture
    ? (isFlight ? `Radar privado: ${nextEvent?.source || 'backend CrewCheck'}` : 'Use o botão “Mais informações” para ver a programação completa.')
      ...
      <div className="grid gap-0 xl:grid-cols-[minmax(0,1fr)_15.5rem]">
        <div className="p-5 sm:p-6">
          ...
          <div className="mt-5 flex flex-wrap gap-2">
            ...
            {canAccessAdmin ? <button onClick={onSync} className="cc-button-secondary rounded-2xl px-4 py-3 text-sm font-black transition active:scale-95">iFlight Admin</button> : <button onClick={onImport} className="cc-button-secondary rounded-2xl px-4 py-3 text-sm font-black transition active:scale-95">Enviar PDF oficial</button>}
          </div>
        </div>
...
function SmartDepartureSummaryCard({ nextEvent, hasRoster, userBase, onOpen }: { nextEvent?: DashboardProps['nextEvent']; hasRoster: boolean; userBase?: string; onOpen?: () => void }) {
  ...
  const bestMode = String(plan?.recommendedMode || plan?.fastestMode || selectedRoute?.mode || 'DRIVE').toUpperCase();
  const modeLabel = bestMode === 'TRANSIT' ? 'Transporte público' : bestMode === 'RIDE_APP' ? 'Uber/99' : bestMode === 'DRIVE' ? 'Carro' : 'Auto';
  const selectedLabel = selectedMode === 'TRANSIT' ? 'transporte público' : selectedMode === 'RIDE_APP' ? 'Uber/99' : selectedMode === 'DRIVE' ? 'carro' : 'auto';
  const firstTransit = transitRoute?.firstTransitDeparture || transitRoute?.transitSteps?.find((step) => step.departureLabel)?.departureLabel || plan?.transitBoard?.find((step) => step.departureLabel)?.departureLabel;
  ...
  const leaveAt = selectedRoute?.leaveAtLabel || plan?.leaveAtLabel || 'Calcular';
  const notToday = Boolean(nextEvent?.relation === 'tomorrow' || nextEvent?.relation === 'future' || nextEvent?.relation === 'rest_followup');
  const departureHeadline = nextEvent?.isInProgress
    ...
    : nextEvent?.requiresDeparture === false
      ...
      : notToday
        ...
        : leaveAt;
  const friendlySource = selectedRoute?.humanSourceLabel || plan?.humanSourceLabel || (String(plan?.source || selectedRoute?.source || '').match(/Google|Uber|Dados reais/i) ? 'Dados reais' : 'Estimativa CrewCheck');
  ...
  return (
    <section className="crewcheck-v10855-smart-summary rounded-[1.8rem] border border-cyan-200/15 bg-[linear-gradient(135deg,rgba(34,211,238,.12),rgba(255,255,255,.055),rgba(16,185,129,.075))] p-5 shadow-2xl shadow-cyan-950/10 backdrop-blur-2xl">
      <div className="flex items-start justify-between gap-4">
        <div className="min-w-0">
          ...
          <p className="mt-2 text-sm font-semibold leading-6 text-slate-300">
            {nextEvent?.isInProgress ? 'A programação já começou; confira detalhes e alterações na escala oficial.' : notToday ? (nextEvent?.dayContext || 'A programação exibida não é de hoje.') : nextEvent?.requiresDeparture === false ? 'A próxima programação não exige deslocamento de casa.' : (plan?.ok ? `Melhor: ${modeLabel}. Chegue em ${plan.airport || airport} até ${plan.arrivalDeadlineLabel || nextEvent?.time || '—'}.` : `Resumo rápido até ${airport}. Ajuste margem, modal e preferências no painel.`)}
          </p>
        </div>
      ...
      <div className="mt-5 grid gap-2 sm:grid-cols-3">
        ...
        <GlassPill label="Tempo" value={selectedRoute?.durationMinutes ? `${selectedRoute.durationMinutes} min` : plan?.durationMinutes ? `${plan.durationMinutes} min` : '—'} />
        <GlassPill label="Fonte" value={friendlySource} />
      </div>
      ...
      <div className="mt-4 flex flex-wrap gap-2">
        <button onClick={onOpen} className="cc-button-primary rounded-2xl px-4 py-3 text-sm font-black shadow-lg transition active:scale-95">Painel</button>
...
function SmartDepartureCard({ nextEvent, hasRoster, userBase, onOpen }: { nextEvent?: DashboardProps['nextEvent']; hasRoster: boolean; userBase?: string; onOpen?: () => void }) {
  ...
  const [travelMode, setTravelMode] = useState<'AUTO' | 'DRIVE' | 'TRANSIT' | 'RIDE_APP'>(() => (localStorage.getItem('crewcheck_departure_mode') as 'AUTO' | 'DRIVE' | 'TRANSIT' | 'RIDE_APP') || 'AUTO');
  ...
  const bestModeLabel = bestMode === 'TRANSIT' ? 'Transporte público' : bestMode === 'RIDE_APP' ? 'Corrida por app' : 'Carro';
  ...
  const fetchSmartPlan = React.useCallback(async (coords: NativeLocationPayload) => {
    ...
    return payload;
  }, [airport, margin, nextEvent?.targetIso, travelMode]);
  ...
  const updateMode = (mode: 'AUTO' | 'DRIVE' | 'TRANSIT' | 'RIDE_APP') => {
    setTravelMode(mode);
  ...
  const modeButtons: Array<{ value: 'AUTO' | 'DRIVE' | 'TRANSIT' | 'RIDE_APP'; label: string }> = [
    ...
    <div className="crewcheck-smart-departure-premium crewcheck-smart-departure-minimal rounded-[1.8rem] border border-cyan-200/15 bg-[linear-gradient(135deg,rgba(34,211,238,.12),rgba(255,255,255,.055),rgba(16,185,129,.075))] p-5 shadow-2xl shadow-cyan-950/10 backdrop-blur-2xl">
      <div className="grid gap-5 lg:grid-cols-[minmax(0,1fr)_18rem]">
        <div className="min-w-0">
          <div className="flex flex-wrap items-start justify-between gap-4">
            <div className="min-w-0">
              <p className="text-[0.68rem] font-black uppercase tracking-[0.24em] text-cyan-100/60">Saída inteligente · Home Ultra Premium</p>
              <h3 className="mt-2 text-4xl font-black tracking-tight text-white sm:text-5xl">{plan?.leaveAtLabel || 'Calcular'}</h3>
              <p className="mt-2 max-w-2xl text-sm font-semibold leading-6 text-slate-300">
                {plan?.ok ? `Chegue em ${plan.airport || airport} até ${arrivalLabel}. Melhor opção: ${bestModeLabel}.` : `Ative a localização para comparar carro, transporte público e Uber/99 até ${airport}.`}
              </p>
            </div>
          ...
          {routeOptions.length ? (
            <div className="mt-5 grid gap-3 md:grid-cols-3">
              {routeOptions.slice(0, 3).map((option, index) => (
                <div key={`${option?.mode}-${option?.leaveAtLabel}-${index}`} className={`rounded-3xl border p-4 ${index === 0 ? 'border-emerald-300/25 bg-emerald-300/[0.10]' : 'border-white/10 bg-white/[0.045]'}`}>
                  <p className="text-[10px] font-black uppercase tracking-[0.18em] text-cyan-100/55">{index === 0 ? 'melhor opção' : option?.mode === 'TRANSIT' ? 'transporte público' : option?.mode === 'RIDE_APP' ? 'corrida por app' : 'carro'}</p>
          ...
          {rideBoard.length ? (
            <div className="mt-4 rounded-[1.5rem] border border-cyan-300/20 bg-cyan-300/[0.07] p-4">
              <div className="flex flex-wrap items-center justify-between gap-2">
                ...
                <span className="rounded-full border border-white/10 bg-white/[0.08] px-2 py-1 text-[10px] font-black uppercase tracking-[0.12em] text-cyan-50">API oficial + fallback</span>
              </div>
              <div className="mt-3 flex flex-wrap gap-2">
        ...
        <aside className="rounded-[1.5rem] border border-white/10 bg-white/[0.045] p-4 shadow-xl shadow-black/10">
          ...
          <div className="mt-3 space-y-2">
            ...
            <GlassPill label="Fonte" value={plan?.estimateDisclosure || sourceLabel} />
          </div>
          <div className="mt-3 flex flex-wrap gap-2">
          ...
          <div className="mt-4 grid gap-2">
            ...
            {plan?.mapsUrl ? <button onClick={() => window.open(plan.mapsUrl, '_blank')} className="cc-button-secondary rounded-2xl px-4 py-3 text-sm font-black transition active:scale-95">Abrir Maps</button> : null}
      ...
      <p className="mt-4 text-xs leading-5 text-slate-500">Reliable: o CrewCheck não inventa tarifa oficial. Ele tenta Uber Guest Trip Estimates, Uber Price Estimates quando autorizado e, se a API negar escopo, mostra estimativa auditável + abertura do app oficial.</p>
    </div>
  );
}
...
function MenuCard({ icon: Icon, label, caption, color, onClick, compact = false }: { icon: LucideIcon; label: string; caption: string; color: string; onClick: () => void; compact?: boolean }) {
  return (
    <button onClick={onClick} className={`cc-menu-card group rounded-[1.45rem] border border-white/10 bg-white/[0.045] ${compact ? 'min-h-0 p-3' : 'min-h-[7.2rem] p-3 sm:p-4'} text-left shadow-xl shadow-black/15 transition hover:-translate-y-0.5 hover:bg-white/[0.075] active:scale-95`}>
