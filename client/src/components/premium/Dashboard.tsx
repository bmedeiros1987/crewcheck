...
type SmartTravelMode = 'AUTO' | 'DRIVE' | 'TRANSIT' | 'RIDE_APP' | 'DRIVE_FLIGHT' | 'RIDE_APP_FLIGHT' | 'MOTO_FLIGHT' | 'TRANSIT_FLIGHT';
...
function SmartDepartureCard({ nextEvent, hasRoster, userBase, onOpen }: { nextEvent?: DashboardProps['nextEvent']; hasRoster: boolean; userBase?: string; onOpen?: () => void }) {
  ...
  const fetchSmartPlan = React.useCallback(async (coords: NativeLocationPayload) => {
    ...
    return payload;
  }, [airport, margin, nextEvent?.targetIso, (nextEvent as any)?.smartDepartureTargetIso, nextEvent?.origin, nextEvent?.destination, travelMode]);
  ...
  const modeButtons: Array<{ value: SmartTravelMode; label: string }> = [
    ...
    { value: 'RIDE_APP_FLIGHT', label: 'Uber + voo' },
    { value: 'MOTO_FLIGHT', label: 'Moto + voo' },
            ...
            <div className="mt-5 grid gap-3 md:grid-cols-3">
              {routeOptions.slice(0, 3).map((option, index) => (
                <div key={`${option?.mode}-${option?.leaveAtLabel}-${index}`} className={`rounded-3xl border p-4 ${index === 0 ? 'border-emerald-300/25 bg-emerald-300/[0.10]' : 'border-white/10 bg-white/[0.045]'}`}>
                  <p className="text-[10px] font-black uppercase tracking-[0.18em] text-cyan-100/55">{index === 0 ? 'melhor opção' : String(option?.mode || '').includes('FLIGHT') ? 'com voo até a origem' : option?.mode === 'TRANSIT' ? 'transporte público' : option?.mode === 'RIDE_APP' ? 'corrida por app' : 'carro'}</p>
