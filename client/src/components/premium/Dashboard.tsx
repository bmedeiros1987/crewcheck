    ...
    <div className="min-h-screen overflow-x-hidden overflow-y-auto bg-[#030914] text-white crewcheck-commercial-dashboard">
      ...
      <div className={`relative z-10 mx-auto grid min-h-[100dvh] w-full max-w-[118rem] grid-cols-1 gap-5 px-4 pb-32 pt-[calc(1rem+env(safe-area-inset-top,0px))] sm:px-6 ${sidebarCollapsed ? 'lg:grid-cols-[6.2rem_minmax(0,1fr)]' : 'lg:grid-cols-[18rem_minmax(0,1fr)]'} lg:pb-10 lg:pt-6 crewcheck-home-ultra-minimal`}>
        ...
        <main className="min-w-0 space-y-5">
          ...
          <SeatForecastCockpitCard forecast={seatForecast} onOpen={() => onNavigate('departure')} />
...
function seatForecastCockpitTone(symbol?: string) {
  if (symbol === '🟢') return 'border-emerald-200/30 bg-emerald-300/12 text-emerald-50';
  if (symbol === '🟡') return 'border-yellow-200/30 bg-yellow-300/12 text-yellow-50';
  if (symbol === '🟠') return 'border-orange-200/30 bg-orange-300/12 text-orange-50';
  if (symbol === '🔴') return 'border-rose-200/35 bg-rose-300/12 text-rose-50';
  return 'border-cyan-200/18 bg-white/[0.045] text-cyan-50';
}

function SeatForecastCockpitCard({ forecast, onOpen }: { forecast?: DashboardProps['seatForecast']; onOpen: () => void }) {
  const symbol = forecast?.symbol || '⚪';
  const empty = Boolean(forecast?.empty);
  return (
    <section className={`rounded-[2rem] border p-4 shadow-2xl shadow-black/20 backdrop-blur-2xl ${seatForecastCockpitTone(symbol)}`}>
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div className="min-w-0">
          <p className="text-[0.66rem] font-black uppercase tracking-[0.28em] opacity-70">Vivo de Extra · previsão de assento</p>
          <h3 className="mt-2 truncate text-2xl font-black tracking-tight sm:text-3xl">{empty ? 'Calcule a ocupação do voo' : `${symbol} ${forecast?.title || 'Situação de ocupação'}`}</h3>
          <p className="mt-2 text-sm font-semibold leading-6 opacity-80">{empty ? 'Abra o Vivo de Extra, escolha origem/destino e selecione um voo. O Cockpit passa a mostrar a chance de embarque por semáforo.' : `${forecast?.route || 'rota selecionada'}${forecast?.flight ? ` · ${forecast.flight}` : ''}${forecast?.score ? ` · score ${forecast.score}` : ''}`}</p>
        </div>
        <div className="grid min-w-[14rem] grid-cols-2 gap-2 text-xs font-black">
          <div className="rounded-2xl border border-white/10 bg-black/15 px-3 py-2"><p className="opacity-55">Status</p><p className="mt-1 text-base">{forecast?.riskLabel || (empty ? 'selecionar voo' : 'a validar')}</p></div>
          <div className="rounded-2xl border border-white/10 bg-black/15 px-3 py-2"><p className="opacity-55">Fonte</p><p className="mt-1 text-base">{forecast?.source || 'CrewCheck'}</p></div>
        </div>
        <button onClick={onOpen} className="rounded-2xl border border-white/10 bg-white/10 px-4 py-3 text-xs font-black uppercase tracking-[0.12em] text-white transition hover:bg-white/15">Abrir Vivo de Extra</button>
      </div>
      <div className="mt-3 grid gap-2 text-[11px] font-bold leading-5 opacity-80 sm:grid-cols-4">
        <span>🟢 boa chance</span><span>🟡 atenção</span><span>🟠 baixa chance</span><span>🔴 muito cheio</span>
      </div>
    </section>
  );
}
...
function NextProgramBoardingPass({ nextEvent, hasRoster, canAccessAdmin, onOpenRoster, onImport, onSync }: { nextEvent?: DashboardProps['nextEvent']; hasRoster: boolean; canAccessAdmin?: boolean; onOpenRoster: () => void; onImport: () => void; onSync: () => void }) {
  ...
  const hasLandingEstimate = Boolean(isFlight && (nextEvent?.landingCountdown || nextEvent?.estimatedArrival));
  const sideHeading = hasLandingEstimate ? 'pouso estimado' : nextEvent?.isInProgress ? 'status agora' : requiresDeparture ? 'tempo restante' : 'sem deslocamento';
  const sideValue = hasLandingEstimate ? (nextEvent?.landingCountdown || nextEvent?.estimatedArrival || '—') : nextEvent?.isInProgress ? 'em andamento' : requiresDeparture ? liveCountdown : '—';
  const sideText = nextEvent?.dayContext || (requiresDeparture
    ? (isFlight ? `Radar privado: ${nextEvent?.source || 'CrewCheck'}${nextEvent?.registration ? ` · matrícula ${nextEvent.registration}` : ''}` : 'Use o botão “Mais informações” para ver a programação completa.')
    ...
    <div className={`crewcheck-next-program-v108151 overflow-hidden rounded-[1.9rem] border border-white/12 bg-[linear-gradient(135deg,rgba(255,255,255,0.092),rgba(255,255,255,0.048))] shadow-2xl shadow-black/20 backdrop-blur-2xl ${notToday ? 'cc-not-today' : ''}`}>
      <div className="grid gap-0 xl:grid-cols-[minmax(0,1fr)_15.5rem]">
        <div className="p-5 sm:p-6">
          ...
          <div className={isFlight && requiresDeparture ? "mt-5 grid grid-cols-2 gap-2 sm:grid-cols-6" : "mt-5 grid grid-cols-2 gap-2 sm:grid-cols-4"}>
            ...
            {isFlight && requiresDeparture ? (
              <>
                ...
                <GlassPill label="Pouso" value={nextEvent?.landingCountdown || nextEvent?.estimatedArrival || '—'} />
                <GlassPill label="Matrícula" value={nextEvent?.registration || '—'} />
