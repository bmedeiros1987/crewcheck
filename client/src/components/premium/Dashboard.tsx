...
function routeParts(evt?: DashboardProps['nextEvent']) {
  ...
  return { origin: clean(evt?.origin || m?.[1], 'FOR').slice(0, 3).toUpperCase(), destination: clean(evt?.destination || m?.[2], 'GRU').slice(0, 3).toUpperCase() };
}
...
function flightNumber(evt?: DashboardProps['nextEvent']) { const m = String(evt?.flightNumber || evt?.title || '').toUpperCase().match(/\b[A-Z]{2}\s?\d{3,5}\b/); return m ? m[0].replace(/\s+/g, '') : 'LA3455'; }
function eventDate(evt?: DashboardProps['nextEvent'], fallback = '06/07') {
...
function leaveTime(evt?: DashboardProps['nextEvent']) {
  ...
  const dep = time(evt?.presentationTime || evt?.departureTime || evt?.time, '04:15');
  const [h,m] = dep.split(':').map(Number); if (!Number.isFinite(h)) return '02:45';
...
function BottomNav({ active, onNavigate }: { active: string; onNavigate: (view: string) => void }) {
  ...
  return <nav className="cc1267-bottom-nav">{items.map(([label, view, Icon]) => <button key={view} onClick={() => onNavigate(view)} className={active === view ? 'is-active' : ''}><Icon size={23}/><span>{label}</span>{view === 'alerts' && <b>3</b>}</button>)}</nav>;
}

export default function Dashboard({ user, nextEvent, onNavigate, onOpenMenu, onToggleTheme, perDiemForecastLabel = 'R$ 957,60', weeklyDutyLabel = '33h 37min', alertsCount = 0, canAccessC32FAcademy = false }: DashboardProps) {
  const route = useMemo(() => routeParts(nextEvent), [nextEvent]);
  const flight = flightNumber(nextEvent);
  const presentation = time(nextEvent?.presentationTime || nextEvent?.time, '04:15');
  const departure = time(nextEvent?.departureTime || nextEvent?.time, '04:45');
  const arrival = time(nextEvent?.estimatedArrival || nextEvent?.endTime, '08:20');
  ...
  return (
    <main className="cc1267-app-shell cc1267-cockpit" data-version={APP_VERSION}>
      ...
      <section className="cc1267-kpi-row">
        ...
        <button onClick={() => onNavigate('alerts')}><Bell/><span>Alerts</span><strong>{alertsCount || 3}</strong><small>Não lidos</small><ChevronRight className="chev"/></button>
      </section>
      ...
      <section className="cc1267-flight-card">
        ...
        <div className="cc1267-flight-inner">
          <div className="cc1267-airline-row"><div><span className="cc1267-latam-mark">▰</span><strong>LATAM</strong><em>{flight}</em></div><div><CalendarDays size={20}/><b>{eventDate(nextEvent)}</b><small>Sábado</small></div></div>
