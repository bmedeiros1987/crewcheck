...
function defaultGeneralFlightState(): GeneralFlightState {
  return { seatsAtLanding: '', minors: '', maas: '', wheelchairs: '', wheelchairType: '', comments: '' };
}
...
function upcomingFlight(flights: FlightOption[]) {
  const now = Date.now();
  return flights.find((item) => withTime(parseRosterDate(item.date), item.departureTime).getTime() >= now) || flights[0];
}
...
function buildReportText(flight: FlightOption | undefined, log: CabinLogState) {
  ...
  const otpLines = TIMELINE_STEPS
    ...
    .join('\n');
  const specialAlerts = [
    Number(log.general.minors || 0) > 0 ? `${log.general.minors} menor(es)` : '',
    Number(log.general.maas || 0) > 0 ? `${log.general.maas} MAAS` : '',
    Number(log.general.wheelchairs || 0) > 0 ? `${log.general.wheelchairs} cadeirante(s)${log.general.wheelchairType ? ` · ${log.general.wheelchairType}` : ''}` : '',
  ].filter(Boolean).join(' · ');
...
export default function CabinChiefAssistant({ bundle, onBack, onOpenRoster, initialFocus = 'chief' }: CabinChiefAssistantProps) {
  ...
  function markOfficial(stepKey: string, label: string, time?: string | null) {
    ...
    setTimelineValue(stepKey, label, time, 'escala/API');
    if (stepKey === 'landing') notifyLandingSpecials();
  ...
  function fillFromFlight() {
    ...
    setLog((current) => ({
      ...
      timeline: {
        ...current.timeline,
        ...(flight.dutyReport ? { report: { key: 'report', label: 'Apresentação tripulação', value: flight.dutyReport, note: 'escala/API' } } : {}),
        ...(flight.departureTime ? { takeoff: { key: 'takeoff', label: 'Decolagem', value: flight.departureTime, note: 'escala/API' } } : {}),
        ...(flight.arrivalTime ? { landing: { key: 'landing', label: 'Pouso', value: flight.arrivalTime, note: 'escala/API' } } : {}),
      },
    }));
    toast.success('Horários principais preenchidos pela escala/API.');
  }
          ...
          <div className="p-5 sm:p-7">
            <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
              <div className="min-w-0">
                ...
                <p className="mt-2 text-sm font-bold text-slate-300">{flight ? `${flight.dayOfWeek || ''} ${flight.date} · ${flight.departureTime}–${flight.arrivalTime}${flight.aircraft ? ` · A/C ${flight.aircraft}` : ''}` : 'Abra/importe uma escala para iniciar o relatório.'}</p>
              </div>
              <select value={log.flightId || flight?.id || ''} onChange={(event) => updateLog({ flightId: event.target.value })} className="rounded-2xl border border-cyan-200/20 bg-[#061426] px-4 py-3 text-sm font-black text-cyan-50 outline-none">
