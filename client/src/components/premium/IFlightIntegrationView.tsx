...
const IFlightIntegrationView: React.FC<IFlightIntegrationViewProps> = ({ onBack, onSuccess, onFileUpload, onRosterImport }) => {
  ...
  const [message, setMessage] = useState('iFlight AutoPull restrito ao administrador. Usuários finais continuam com importação manual de PDF oficial. No Android, o CrewCheck usa WebView interno; no Web/PWA, por segurança do navegador, não há navegador interno real para contornar 403/iframe/CORS.');
  ...
  async function handleOpenPortalFlow() {
    ...
    const webMessage = 'No Web/PWA, o navegador não permite criar um navegador interno real para o iFlight quando o portal bloqueia iframe/CORS/SSO. Abra o portal oficial em sessão autorizada e importe o PDF; no Android o CrewCheck usa WebView interno para capturar o download autorizado.';
    ...
    <div className="min-h-screen overflow-hidden bg-[#030914] pb-24 text-white crewcheck-iflight-screen">
      ...
      <div className="relative z-10 mx-auto max-w-5xl px-5 py-6">
        ...
        <section className="mt-5 rounded-[2rem] border border-white/10 bg-white/[0.065] p-5 shadow-2xl shadow-black/25 backdrop-blur-2xl sm:p-6">
          <div className="grid gap-5 lg:grid-cols-[minmax(0,1fr)_20rem]">
            <div>
              ...
              <p className="mt-3 max-w-2xl text-sm leading-6 text-slate-300">Esta função fica somente no perfil administrador. Usuários finais continuam com importação manual de PDF; aqui o CrewCheck testa o download automático autorizado no WebView Android e mantém um modo universal limitado para Web/PWA.</p>
              ...
              <div className="mt-4 rounded-[1.4rem] border border-white/10 bg-white/[0.055] p-3">
                ...
                <p className="mt-3 text-[0.72rem] font-semibold leading-5 text-slate-400">Se o portal Crew retornar 403 para a conta autorizada, teste o CWP. O CrewCheck não burla permissão: ele só reutiliza sessão oficial válida.</p>
              </div>
                ...
                <div className="mt-4 rounded-[1.5rem] border border-cyan-300/20 bg-cyan-300/[0.07] p-4">
                  <div className="flex flex-wrap items-start justify-between gap-3">
                    <div>
                      ...
                      <p className="mt-2 text-xs leading-5 text-slate-300">No navegador comum, não é possível embutir o iFlight como navegador interno real se o portal negar iframe/CORS/SSO. No Android, o AutoPull usa WebView interno e tenta capturar o PDF autorizado; como fallback administrativo, importe o PDF oficial.</p>
                    </div>
        ...
        <section className="mt-4 grid gap-4 md:grid-cols-3">
          ...
          <div className="rounded-[1.5rem] border border-emerald-300/20 bg-emerald-400/10 p-4 text-sm leading-6 text-emerald-50/80 md:col-span-3"><ShieldCheck className="mb-2 h-5 w-5" /><strong>LGPD:</strong> o CrewCheck não recebe senha nem MFA no backend. Essas informações ficam na tela oficial do iFlight dentro do WebView. O módulo permanece restrito ao administrador até validação completa de estabilidade, permissão e conformidade.</div>
        </section>
