...
type IFlightPortalOptions = {
  ...
  calendarSweep: boolean;
  collectEventDetails: boolean;
  openEventDetails: boolean;
  collectTripulationDetails: boolean;
  pdfFallback: boolean;
...
const IFlightIntegrationView: React.FC<IFlightIntegrationViewProps> = ({ onBack, onSuccess, onFileUpload, onRosterImport }) => {
  ...
  const [message, setMessage] = useState('iFlight Calendar Sweep restrito ao administrador. O objetivo agora é varrer o Roster Calendar autorizado dentro do iFlight, coletar programação, horários, pernoites, reservas e tripulação, sem depender de baixar PDF.');
  ...
  const portalOptions: IFlightPortalOptions = {
    ...
    universalPdfCapture: false,
    calendarSweep: true,
    collectEventDetails: true,
    openEventDetails: true,
    collectTripulationDetails: true,
    pdfFallback: false,
    ...
    clickSend: false,
    privacyMode: 'lgpd_no_credentials',
    actions: ['open_official_iflight_webview', 'wait_user_login_password_mfa_on_official_portal', 'mask_portal_after_auth', 'reuse_authorized_session', 'auto_resume_after_login', 'open_menu', 'open_roster_calendar', 'sweep_month_calendar', 'open_visible_event_details', 'capture_duty_pairing_times', 'capture_crew_list', 'capture_layovers_reserves_training', 'intercept_calendar_xhr', 'return_to_crewcheck', 'broadcast_roster_to_all_modules'],
  };
  ...
  const finishRosterImport = async (roster: CrewRoster, sourceFileName: string) => {
    ...
    setStatus('fetching');
    setMessage('Dados do calendário iFlight recebidos. Interpretando programação, tripulação e montando painel...');
  ...
  async function handleOpenPortalFlow() {
    if (hasNativeBridge && window.CrewCheckIFlight?.openPortalAndImport) {
      setStatus('portal');
      setMessage('Portal oficial aberto no modo administrador. Faça login/MFA com conta autorizada; depois o CrewCheck tenta abrir o Roster Calendar e varrer o mês, eventos, detalhes de programação e tripulação.');
      try {
        ...
        if (!payload?.ok) throw new Error(payload?.error || payload?.message || 'O portal foi fechado sem retornar os dados do calendário iFlight.');
        if (payload.roster) {
          await finishRosterImport(payload.roster, payload.sourceFileName || payload.filename || `iFlight_CalendarSweep_${periodYear}_${periodMonth}.txt`);
          return;
        }
        ...
        if (payload.dataBase64) {
          ...
          setMessage('Fallback PDF recebido. Interpretando escala porque o calendário não trouxe dados suficientes...');
    ...
    const webMessage = 'No web, o navegador pode impedir a abertura interna do iFlight quando o portal bloqueia a sessão. No Android, o CrewCheck usa navegador interno para varrer o Roster Calendar autorizado; no iPad/web, cole o texto do calendário como fallback.';
  ...
  async function handleUniversalPasteImport() {
    ...
    if (text.length < 80) {
      toast.error('Cole o texto do calendário/roster do iFlight para converter sem PDF.');
      return;
    }
    setStatus('fetching');
  ...
  return (
    <div className="min-h-screen overflow-hidden pb-24 crewcheck-iflight-screen" style={{background:"var(--background)",color:"var(--foreground)"}}>
      ...
      <div className="relative z-10 mx-auto max-w-5xl px-5 py-6">
        <header className="flex items-center justify-between gap-4 rounded-[1.6rem] border border-white/10 bg-white/[0.065] px-4 py-3 shadow-2xl shadow-black/25 backdrop-blur-2xl">
          ...
          <img src="/icons/crewcheck-icon-v2.png" alt="CrewCheck" className="crewcheck-brand-icon-image h-11 w-11 rounded-2xl" />
        </header>
        ...
        <section className="mt-5 rounded-[2rem] border border-white/10 bg-white/[0.065] p-5 shadow-2xl shadow-black/25 backdrop-blur-2xl sm:p-6">
          <div className="grid gap-5 lg:grid-cols-[minmax(0,1fr)_20rem]">
            <div>
              ...
              <h1 className="mt-2 text-3xl font-black tracking-tight sm:text-4xl">Calendar Sweep iFlight</h1>
              ...
              <div className="mt-4 rounded-[1.4rem] border border-white/10 bg-[#07111f]/75 p-4">
                ...
                <div className="mt-3 grid gap-2 text-xs font-bold leading-5 text-slate-300 sm:grid-cols-2">
                  ...
                  <span className="rounded-2xl bg-white/[0.07] px-3 py-2">Pernoites, horários, detalhes e tripulação</span>
                </div>
              </div>
              ...
              <div className="mt-5 grid gap-3 sm:grid-cols-[minmax(0,1fr)_14rem]">
                <button onClick={handleOpenPortalFlow} disabled={status === 'fetching'} className="flex min-h-[4rem] items-center justify-center gap-3 rounded-2xl bg-cyan-300 px-5 py-4 text-base font-black text-[#07111f] shadow-lg shadow-cyan-950/30 transition hover:bg-cyan-200 disabled:cursor-wait disabled:opacity-70">
                  <DownloadCloud className="h-5 w-5" /> {hasNativeBridge ? 'Varrer calendário no Android' : 'Abrir portal oficial'}
                </button>
                <button onClick={() => fileInputRef.current?.click()} disabled={status === 'fetching'} className="flex min-h-[4rem] items-center justify-center gap-3 rounded-2xl border border-white/10 bg-white/10 px-5 py-4 text-sm font-black text-white transition hover:bg-white/15 disabled:cursor-wait disabled:opacity-70">
                  <Upload className="h-5 w-5" /> Fallback PDF
                </button>
                ...
                <div className="mt-4 rounded-[1.5rem] border border-cyan-300/20 bg-cyan-300/[0.07] p-4">
                  <div className="flex flex-wrap items-start justify-between gap-3">
                    <div>
                      ...
                      <p className="mt-2 text-xs leading-5 text-slate-300">No iPad e no navegador comum, cole o texto da tela Roster Calendar como fallback. No Android, o módulo administrativo tenta varrer o calendário mensal e abrir detalhes para capturar programação e tripulação.</p>
                    </div>
                    <button onClick={() => fileInputRef.current?.click()} className="rounded-2xl border border-white/10 bg-white/10 px-4 py-2 text-xs font-black text-white transition hover:bg-white/15">Fallback PDF</button>
                  </div>
                  <textarea
                    value={universalPasteText}
                    onChange={(event) => setUniversalPasteText(event.target.value)}
                    placeholder="Opcional: cole aqui o texto da tela Roster Calendar ou detalhes de tripulação do iFlight para converter sem PDF."
            ...
            <div className="rounded-[1.5rem] border border-cyan-300/15 bg-[#07111f]/70 p-4">
              ...
              <div className="grid gap-3">
                ...
                <div className="rounded-2xl border border-white/10 bg-white/[0.055] p-3 text-xs leading-5 text-slate-300">
                  <strong className="text-white">{fromDate}</strong> até <strong className="text-white">{toDate}</strong><br />Formato: <strong className="text-white">Calendar Sweep</strong> · Fuso: <strong className="text-white">LT</strong>
                </div>
              </div>
            </div>
          </div>
        ...
        <section className="mt-4 grid gap-4 md:grid-cols-3">
          <ToggleCard checked={autoClicks} onChange={setAutoClicks} title="Varredura automática" text="Abre o Roster Calendar, lê o mês, tenta abrir detalhes dos eventos e captura tripulação quando visível." />
        ...
        <section className="mt-4 rounded-[1.7rem] border border-white/10 bg-white/[0.055] p-4 text-sm leading-6 text-slate-300 shadow-xl shadow-black/20 backdrop-blur-2xl">
          ...
          Fluxo administrativo: use apenas para testes controlados e contas autorizadas. O CrewCheck não baixa nem lê senha/MFA; ele varre apenas dados visíveis na sessão oficial do iFlight.
        </section>
