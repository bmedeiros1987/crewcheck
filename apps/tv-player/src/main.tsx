...
const time=(value?:string|null)=>value?new Date(value).toLocaleTimeString('pt-BR',{hour:'2-digit',minute:'2-digit',timeZone:'America/Sao_Paulo'}):'—';
function App(){
  const [demo,setDemo]=useState(initialDemo);
  const [snapshot,setSnapshot]=useState<TvSnapshot|null>(()=>demo?demoSnapshot():null);
  const [view,setView]=useState<View>('Agora'),[day,setDay]=useState(new Date().toLocaleDateString('en-CA'));
  const [status,setStatus]=useState(demo?'Demonstração · dados fictícios':'Vincule sua TV');
  ...
  async function begin(){
    clear(); const run=generation.current;
    try {const p=await session.call('pair',{platform});if(run!==generation.current)return;
      setPairing({...p,deadline:Date.now()+p.expiresIn*1000});setQr(await QRCode.toDataURL(p.verificationUri));setStatus('Confirme no celular');
  ...
  useEffect(()=>{
    ...
    const timer=setInterval(async()=>{
      if(Date.now()>pairing.deadline){setPairing(null);setStatus('Código expirado. Gere outro.');return;}
      if(busy)return;busy=true;
      try{const result=await session.call('poll',{deviceCode:pairing.deviceCode});if(cancelled)return;
        if(!result.pending){session.pair(result);setPairing(null);setSnapshot(await session.sync());setStatus('Sincronizado');}
      }catch{if(!cancelled)setStatus('Aguardando confirmação ou conexão.');}finally{busy=false;}
  ...
  useEffect(()=>{
    let busy=false;
    async function sync(){
      if(demo||busy||!session.credential||document.hidden)return;busy=true;
      try{setSnapshot(await session.sync());setStatus('Sincronizado');await session.call('heartbeat',{});
        try{const feed=await session.call('news');setNews(feed.items||[]);}catch{setNews([]);}
      }catch{setSnapshot(session.offline());setStatus(session.credential?'Sem conexão · última informação':'Vincule sua TV');}
      finally{busy=false;}
    }
    const timer=setInterval(()=>{setClock(new Date());if(Date.now()-lastInput.current>120000){setView('Agora');setMode('ambient');}
      if(!demo&&session.credential){const cached=session.offline();if(!cached)setSnapshot(null);}},1000);
    const refresh=setInterval(sync,30000);
    document.addEventListener('visibilitychange',sync);window.addEventListener('online',sync);window.addEventListener('webOSRelaunch',sync);
  ...
  useEffect(()=>{
    const key=(event:KeyboardEvent)=>{
      const action=remoteAction(event.key)||remoteAction(event.keyCode);if(!action)return;
      lastInput.current=Date.now();
      if(action==='back'){event.preventDefault();if(view==='Dia'){setView('Mês');return;}if(view!=='Agora'){setView('Agora');return;}
        const w=window as any;
        if(w.tizen)w.tizen.application.getCurrentApplication().exit();else if(w.webOS?.platformBack)w.webOS.platformBack();else w.dispatchEvent(new Event('tv-exit'));return;}
      if(action==='ok')return;
      event.preventDefault();const buttons=Array.from(document.querySelectorAll<HTMLElement>('button:not(:disabled),a[href]'));
      const current=document.activeElement as HTMLElement;const rect=current?.getBoundingClientRect();
      if(!rect){buttons[0]?.focus();return;}
      const x=rect.left+rect.width/2,y=rect.top+rect.height/2;
      const candidates=buttons.filter(b=>b!==current).map(b=>{const r=b.getBoundingClientRect();return{b,dx:r.left+r.width/2-x,dy:r.top+r.height/2-y};})
        .filter(p=>action==='left'?p.dx< -2:action==='right'?p.dx>2:action==='up'?p.dy< -2:p.dy>2)
        .sort((a,b)=>{const score=(p:typeof a)=>(action==='left'||action==='right')?Math.abs(p.dx)+Math.abs(p.dy)*4:Math.abs(p.dy)+Math.abs(p.dx)*4;return score(a)-score(b);});
  ...
  return <main ref={main} onPointerDown={()=>{lastInput.current=Date.now();}}>
    ...
    {(view==='Mês'||view==='Semana')&&<section className="calendar"><div className="calendar-title"><h1>{view==='Mês'?'Sua escala completa':'Sua semana'}</h1><p>Selecione um dia para ver os detalhes</p>{view==='Semana'&&<><button onClick={()=>setDay(snapshot.days[Math.max(0,snapshot.days.findIndex(d=>d.date===day)-7)].date)}>← Semana anterior</button><button onClick={()=>setDay(snapshot.days[Math.min(snapshot.days.length-1,Math.max(0,snapshot.days.findIndex(d=>d.date===day))+7)].date)}>Próxima semana →</button></>}</div><div className="weekdays">{['SEG','TER','QUA','QUI','SEX','SÁB','DOM'].map(d=><span key={d}>{d}</span>)}</div><div className="days">{view==='Mês'&&Array.from({length:(new Date(snapshot.summary.month+'-01T12:00:00Z').getUTCDay()+6)%7},(_,i)=><div key={`blank-${i}`}/>)}{snapshot.days.filter((_,i)=>view==='Mês'||Math.floor((i+(new Date(snapshot.summary.month+'-01T12:00:00Z').getUTCDay()+6)%7)/7)===Math.floor((Math.max(0,snapshot.days.findIndex(d=>d.date===day))+(new Date(snapshot.summary.month+'-01T12:00:00Z').getUTCDay()+6)%7)/7)).map(d=><button key={d.date} className={d.activities.length?'has-flight':''} onClick={()=>{setDay(d.date);setView('Dia');}}><b>{Number(d.date.slice(-2))}</b><span>{d.activities[0]?.flight||labels[d.activities[0]?.kind]||'Sem programação'}</span><small>{d.activities[0]?.presentation||'—'}</small></button>)}</div><p className="note">Dias sem programação não significam folga confirmada.</p></section>}
    {view==='Dia'&&<section className="detail"><button onClick={()=>setView('Mês')}>← Voltar ao mês</button><h1>{day.split('-').reverse().join('/')}</h1><div className="activities">{selected?.activities.length?selected.activities.map(activity):<p>Nenhuma programação publicada para este dia.</p>}</div></section>}
    {view==='Mudanças'&&<section className="detail"><h1>O que mudou</h1>{snapshot.changes.length?snapshot.changes.map((c,i)=><article key={i}>{c}</article>):<p>Nenhuma atualização confirmada disponível.</p>}</section>}
    {view==='Configurações'&&<section className="detail"><h1>Sua TV</h1><p>Privacidade: {snapshot.privacy==='family'?'Família':'Privado'}. Para alterar, autorize novamente pelo celular.</p><button onClick={()=>{setMode('ambient');setView('Agora');}}>Modo Ambient</button><button onClick={()=>{setMode('live');setView('Agora');}}>Modo Live</button><button onClick={()=>{setMode('briefing');setView('Agora');}}>Modo Briefing</button><button onClick={clear}>{demo?'Sair da demonstração':'Desvincular desta TV'}</button><p>A autorização expira em até 24 horas. Uma nova sessão exige pareamento.</p></section>}
