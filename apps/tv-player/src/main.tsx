...
function App() {
  ...
  function openDay(date: string, from: View) { dayReturn.current = from; setActivityId(null); setDay(date); setView('Dia'); }
  function openActivity(id: string) { setActivityId(id); setView('Programação'); }
  function goBack() {
  ...
  const selectedActivity = selected?.activities.find(a => a.id === activityId) || null;
  ...
  const activity = (a: TvActivity) => {
    const role=a.display?.role || (a.kind==='flight'?'flight':a.kind==='stay'?'stay':'duty');
    const Icon=role==='stay'?BedDouble:role==='rest'?Moon:role==='reserve'?BriefcaseBusiness:role==='standby'?Clock3:Plane;
    return <button className={'activity activity-role-'+role} key={a.id} onClick={()=>openActivity(a.id)}>
      <span className="activity-kind"><Icon aria-hidden="true"/>{a.display?.title || activityLabel(a)}</span>
      {a.origin && <h2>{a.origin} <ArrowRight aria-hidden="true"/> {a.destination}</h2>}
      {role==='flight' && <p>Apresentação <strong>{a.presentation || 'Não informada'}</strong></p>}
      {role==='stay' && <p>Estadia <strong>{a.destination || a.origin || 'Local não informado'}</strong></p>}
      <p>Horários <strong>{time(a.startAt)} — {time(a.endAt)}</strong></p>
      {a.groundBeforeMinutes !== null && role==='flight' && <p>Em solo <strong>{a.groundBeforeMinutes} min</strong></p>}
      <small>OK para ver detalhes <ChevronRight/></small>
    </button>;
  };
  return <><main ref={main} className={'tv-app theme-' + theme} data-motion={effectiveMotion} data-paused={paused || care.covered ? 'true' : 'false'} data-screen-care={care.covered ? 'covered' : 'active'} aria-hidden={care.covered} style={{...themeStyle, left: care.shift.x, top: care.shift.y}} onMouseDown={() => { lastInput.current = Date.now(); }}>
    ...
    {!enabled ? <section className="pair"><div><p className="eyebrow">CREWCHECK TV</p><h1>Piloto ainda não disponível.</h1><p>A liberação da sua conta será feita no aplicativo.</p></div></section> : !snapshot ? <section className="pair view-enter"><div><p className="eyebrow"><ShieldCheck/> BEM-VINDO A BORDO</p><h1>Sua próxima jornada.<br/>Na sua TV.</h1><p>Autorize esta tela pelo CrewCheck no celular.</p><button className="primary-button" onClick={begin}>{demo ? 'Voltar à demonstração' : pairing ? 'Gerar novo código' : 'Vincular TV'} <ArrowRight/></button><p role="status">{status}</p><small>Nenhuma senha da sua conta fica nesta televisão.</small></div>{pairing && <aside><img src={qr} alt="QR Code para autorizar a televisão"/><h2>{pairing.userCode}</h2><p>Válido por 5 minutos</p></aside>}</section> : <>
      ...
      {view === 'Dia' && <section className="detail"><button className="back-button" onClick={() => setView(dayReturn.current)}><ChevronLeft/> Voltar</button><p className="eyebrow">PROGRAMAÇÃO COMPLETA</p><h1>{day.split('-').reverse().join('/')}</h1><p className="detail-lead">Todas as atividades publicadas deste dia aparecem separadamente. Selecione uma para abrir os detalhes.</p><div className="activities">{selected?.activities.length ? selected.activities.map(activity) : <article className="empty-card">Nenhuma programação publicada para este dia.</article>}</div></section>}
      {view === 'Programação' && <section className="detail program-detail"><button className="back-button" onClick={() => setView('Dia')}><ChevronLeft/> Dia</button>{selectedActivity ? <><p className="eyebrow">{selectedActivity.display?.role==='stay'?'PERNOITE':selectedActivity.display?.role==='flight'?'VOO / PROGRAMAÇÃO':'PROGRAMAÇÃO'}</p><h1>{selectedActivity.display?.title || activityLabel(selectedActivity)}</h1>{selectedActivity.origin&&<div className="program-route"><b>{selectedActivity.origin}</b><ArrowRight/><b>{selectedActivity.destination}</b></div>}<div className="program-detail-grid"><article><small>Apresentação</small><strong>{selectedActivity.presentation||'Não informada'}</strong></article><article><small>Início</small><strong>{time(selectedActivity.startAt)}</strong></article><article><small>Fim</small><strong>{time(selectedActivity.endAt)}</strong></article><article><small>Código publicado</small><strong>{selectedActivity.publishedCode||'—'}</strong></article></div><div className="sensitive-placeholder"><ShieldCheck/><div><h2>Detalhes adicionais</h2><p>Tripulação, financeiro e outros dados sensíveis só aparecerão quando o titular autorizar essas categorias no celular.</p></div></div></> : <article className="empty-card">Programação não encontrada.</article>}</section>}
