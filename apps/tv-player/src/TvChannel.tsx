...
export function ChannelSettings({channel}:{channel:TvChannel}) {
  ...
  return <section className="channel-settings" aria-label="Canal automático e música">
    ...
    </div><p className="channel-audio-status" role="status">{channel.sound.status==='error'?'Não foi possível tocar as faixas deste pacote. Sua escala continua funcionando.':channel.sound.status==='blocked'?'A TV não iniciou o áudio. Pressione Tocar música para tentar novamente.':channel.sound.status==='playing'?'Reproduzindo · playlist em sequência e repetição':channel.sound.status==='starting'?'Carregando a faixa local…':'O áudio começa somente quando você escolher ligar.'}</p><small>Música pausada ao ocultar o app, perder o vínculo ou entrar na proteção de tela. O volume da TV continua independente.</small></article></div>
