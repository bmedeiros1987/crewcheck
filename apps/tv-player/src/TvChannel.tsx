...
export function useTvChannel(options:{view:string;setView:(view:ChannelView)=>void;hasSnapshot:boolean;hasChanges:boolean;hasNews:boolean;covered:boolean;hidden:boolean;exitRequested:boolean}) {
  ...
  useEffect(()=>{
    ...
    const audio=document.createElement('audio'); audio.id='crewcheck-soundtrack'; audio.style.display='none'; audio.setAttribute('aria-hidden','true');
    ...
    document.addEventListener('visibilitychange',context);window.addEventListener('pagehide',hide);
    let last=now();
    const tick=window.setInterval(()=>{
      const instant=now(),delta=instant-last;last=instant;const c=current.current;
      const deck=channelDeck(c.hasSnapshot,c.hasChanges,c.hasNews);
    ...
    context();
    return()=>{clearInterval(tick);engine.dispose();audio.remove();player.current=null;events.forEach(event=>document.removeEventListener(event,interaction,true));document.removeEventListener('visibilitychange',context);window.removeEventListener('pagehide',hide);};
  },[]);
  ...
  const deck=channelDeck(options.hasSnapshot,options.hasChanges,options.hasNews);
...
const soundLabel=(state:SoundState)=>state.status==='playing'?CREW_TRACKS[state.index].title:state.status==='starting'?'Iniciando música…':state.status==='error'?'Áudio indisponível':state.status==='blocked'?'OK para tocar':state.status==='paused'?'Música pausada':'Ligar música';
export function ChannelDock({channel}:{channel:TvChannel}) {
  return <div className="channel-dock">
    ...
    <button className="channel-music" onClick={channel.music} aria-label={channel.sound.wanted?'Pausar música de fundo':'Ligar música de fundo'}>{channel.sound.status==='playing'?<Pause/>:<Music2/>}<span>{soundLabel(channel.sound)}</span></button>
  </div>;
}
export function ChannelSettings({channel}:{channel:TvChannel}) {
  ...
  return <section className="channel-settings" aria-label="Canal automático e música">
    <div className="channel-settings-row"><article><h2><Radio/> Telas automáticas</h2><p>Agora → Semana → Mês. Mudanças e Notícias entram quando houver conteúdo.</p>
      ...
      <button aria-label="Faixa anterior" onClick={()=>channel.move(-1)}><SkipBack/></button><button className="channel-play" onClick={channel.music}>{channel.sound.wanted?<Pause/>:<Play/>}{channel.sound.wanted?'Pausar música':'Ligar música'}</button><button aria-label="Próxima faixa" onClick={()=>channel.move(1)}><SkipForward/></button>
    ...
    </div><p className="channel-audio-status" role="status">{channel.sound.status==='error'?'Não foi possível tocar as faixas deste pacote. Sua escala continua funcionando.':channel.sound.status==='blocked'?'A TV não iniciou o áudio. Pressione Ligar música para tentar novamente.':channel.sound.status==='playing'?'Reproduzindo · playlist em sequência e repetição':channel.sound.status==='starting'?'Carregando a faixa local…':'O áudio começa somente quando você escolher ligar.'}</p><small>Música pausada ao ocultar o app, perder o vínculo ou entrar na proteção de tela. O volume da TV continua independente.</small></article></div>
