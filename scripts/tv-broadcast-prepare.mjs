...
if(!main.includes("from './BroadcastPanels'")){
 ...
 main=replace(main,'hasChanges:!!snapshot?.changes.length, hasNews:news.length>0, covered:',
   'hasChanges:displayPrefs.value.changes&&!!snapshot?.changes.length, hasNews:displayPrefs.value.news&&news.length>0, quick:{...quickAvailability(snapshot,clock.getTime()),weather:displayPrefs.value.weather&&quickAvailability(snapshot,clock.getTime()).weather}, covered:');
