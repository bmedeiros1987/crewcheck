...
assert(/const DEFAULT_VERSION = '(?:13\.9\.\d+|14\.0\.0)'/.test(home), 'Home versionada em versão CrewCheck suportada');
...
assert(/"version"\s*:\s*"(?:13\.9\.\d+|14\.0\.0)"/.test(manifest), 'manifest PWA em versão CrewCheck suportada');
assert(/crewcheck-icon-v2\.png\?v=(?:139\d+|14000)/.test(manifest), 'logo atual no PWA');
assert(androidManifest.includes('@drawable/crewcheck_icon_site'), 'logo atual no launcher Android');
assert(/versionName '(?:13\.9\.\d+|14\.0\.0)'/.test(androidGradle), 'Android versionName suportado');
