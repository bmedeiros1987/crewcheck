...
assert(/const DEFAULT_VERSION = '13\.9\.\d+'/.test(home), 'Home versionada na linha 13.9.x');
...
assert(/\"version\"\s*:\s*\"13\.9\.\d+\"/.test(manifest), 'manifest PWA na linha 13.9.x');
assert(/crewcheck-icon-v2\.png\?v=139\d/.test(manifest), 'logo atual no PWA');
assert(androidManifest.includes('@drawable/crewcheck_icon_site'), 'logo atual no launcher Android');
assert(/versionName '13\.9\.\d+'/.test(androidGradle), 'Android versionName na linha 13.9.x');
