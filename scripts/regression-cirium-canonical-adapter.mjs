...
const normalized = normalizeCiriumFlightStatus({
  ...
  airportResources: { departureTerminal: '1', departureGate: '9', arrivalTerminal: '2', arrivalGate: '11', baggage: '7' },
  operationalTimes: {
  ...
  flightEquipment: { scheduledEquipmentIataCode: '320', actualEquipmentIataCode: '32B', tailNumber: 'PT-TEST' },
  delays: { departureGateDelayMinutes: 8, arrivalGateDelayMinutes: 2 },
  flightDurations: { scheduledBlockMinutes: 180, blockMinutes: 174 },
  codeshares: [{ fsCode: 'JJ', flightNumber: '3377', relationship: 'L' }],
});

assert.equal(normalized.ok, true);
assert.equal(normalized.provider, 'cirium-sky');
...
assert.equal(normalized.arrivalGate, '11');
assert.equal(normalized.arrivalTerminal, '2');
assert.equal(normalized.baggage, '7');
...
assert.equal(normalized.aircraft, '32B', 'actual equipment must win over scheduled when provided');
assert.equal(normalized.scheduledAircraft, '320');
assert.equal(normalized.actualAircraft, '32B');
assert.equal(normalized.registration, 'PT-TEST');
assert.equal(normalized.delays.departureGateMinutes, 8);
assert.equal(normalized.durations.blockMinutes, 174);
assert.equal(normalized.codeshares[0].flightNumber, '3377');
assert.equal(normalized.updatedAt, '2026-08-09T10:00:00Z');
assert.equal(ciriumCoverage(normalized).percent, 100);

const legacyEquipmentShape = normalizeCiriumFlightStatus({
  carrierFsCode: 'LA', flightNumber: '3377', departureAirportFsCode: 'REC', arrivalAirportFsCode: 'GRU',
  flightEquipment: { scheduledEquipment: { iata: '320' }, actualEquipment: { iata: '321' }, tailNumber: 'PT-OLD' },
});
assert.equal(legacyEquipmentShape.aircraft, '321');
assert.equal(legacyEquipmentShape.registration, 'PT-OLD');
...
assert.equal(noDepartureResources.arrivalGate, '11');
assert.equal(noDepartureResources.arrivalTerminal, '2');
...
console.log('Cirium Sky canonical adapter regression OK');
