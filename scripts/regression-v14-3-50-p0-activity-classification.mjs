...
const fixtures = [
  ...
  {
    // #303: DR é Day Off Requested — folga pedida, não descanso. rosterCodes.ts
    // já declarava { code: 'DR', description: 'Folga Pedida', category: 'DAY_OFF' };
    // era o classificador que divergia da tabela canônica.
    name: 'folga pedida (DR)',
    activity: { kind: 'duty', title: 'DR', canonical: { kind: 'rest' } },
    category: 'FOLGA',
    smartDeparture: false,
  },
  {
    // Repouso verdadeiro segue coberto, para que a correção acima não reduza a
    // cobertura de REPOUSO deste fixture.
    name: 'descanso sem código formal de folga',
    activity: { kind: 'duty', title: 'Repouso', canonical: { kind: 'rest' } },
    category: 'REPOUSO',
    smartDeparture: false,
  },
  {
...
expect(counts.daysOff === 2, `Folgas esperadas: 2 (DO + DR); recebido: ${counts.daysOff}.`);
expect(counts.requestedDaysOff === 1, `Folgas pedidas esperadas: 1 (DR); recebido: ${counts.requestedDaysOff}.`);
