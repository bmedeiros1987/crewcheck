...
assert.match(server, /selected: 'infobip', configured: infobip\.configured/);
assert.match(server, /async function sendAdminPhoneCall\(phone, text\) \{\s+return sendInfobipPhoneCall\(phone, text\);/);
assert.doesNotMatch(server, /callmebot-phone/);
assert.doesNotMatch(server, /CALLMEBOT_API_KEY/);
