...
const gymProviderBefore = `<>{/* cc-wellhub-v14406-anchor */}</>`;
...
function patchHome(source) {
  ...
  if (!next.includes(wellhubImport)) {
    ...
    if (next.includes(preferredAnchor)) next = insertAfterRequired(next, preferredAnchor, wellhubImport, 'import Wellhub depois da origem de locais');
    else next = insertAfterRequired(next, fallbackAnchor, wellhubImport, 'import Wellhub de fallback');
  }
  ...
  next = replaceRequired(
    ...
    'redes reconhecidas para check-in',
  );
  ...
  next = replaceRequired(
    ...
    'seletores de provedor e plano',
  );

  next = replaceRequired(next, gymSearchBefore, gymSearchAfter, 'busca Wellhub sem inferencia de Maps');
  ...
  next = insertAfterRequired(
    ...
    'metadados oficiais da unidade selecionada',
  );

  if (!next.includes('Wellhub verificado</button>')) {
    next = replaceRequired(next, originalGymProvider, verifiedGymProvider, 'controles Wellhub verificados');
  }
  ...
  if (!next.includes('Horários publicados pelo Wellhub')) next = replaceRequired(next, openFilter, openFilterAfter, 'filtro de aberto sem falsa precisao');
  ...
  const selectedAnchor = `    {selected && <section className="cz-place-detail-card">`;
  ...
  if (!next.includes('FONTE OFICIAL WELLHUB')) next = insertAfterRequired(next, selectedAnchor.replace('    ', ''), sourceCard.trimStart(), 'cartao de fonte oficial');
