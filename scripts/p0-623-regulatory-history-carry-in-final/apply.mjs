...
export async function recomputeComplianceWithRegulatoryHistory(
  ...
  const activeSummary = {
    id: 'active',
    crewId: String((roster as any).crewId || '') || null,
    crewName: String((roster as any).crewName || '') || null,
    year: Number(roster.year || 0),
    month: Number(roster.month || 0),
  };
  const activeIdentity = regulatoryCrewIdentity(activeSummary);
  if (!activeIdentity) {
    return {
  ...
  const selection = selectRegulatoryCarryIn({
    summaries: accountSummaries,
    authority: 'account_verified',
    authenticated: true,
    accountQuerySucceeded: true,
  }, activeSummary);
  const previousSummary = selection.historyProven ? selection.summary : null;
  if (!previousSummary) {
    return {
      compliance: withoutHistory(),
      history: {
        complete: false,
        source: selection.reason === 'active_identity_unverified' ? 'identity_unverified' : 'account',
  ...
  try {
    // Account summary proves existence/identity first; openSavedRoster may use an
    // exact local byte copy only as a content cache for that already-proven id.
  ...
  const previousIdentity = previousRoster ? regulatoryCrewIdentity({
    crewId: String((previousRoster as any).crewId || '') || null,
    crewName: String((previousRoster as any).crewName || '') || null,
  }) : null;
  const exactPreviousPeriod = Boolean(previousRoster)
    && Number(previousRoster?.year) === selection.expectedYear
    && Number(previousRoster?.month) === selection.expectedMonth;
  if (!previousRoster || previousIdentity !== selection.crewIdentity || !exactPreviousPeriod) {
    return {
      compliance: withoutHistory(),
  ...
  const snapshotKey = regulatoryHistorySnapshotKey(roster, activeIdentity);
...
for (const fragment of [
  ...
  'selectRegulatoryCarryIn({',\n  "authority: 'account_verified'",\n  'regulatoryCrewIdentity(activeSummary)',\n  'analyzeCompliance(roster, roleSelection, [previousRoster])',
