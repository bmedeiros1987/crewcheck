...
const hashFiles = () => crypto.createHash('sha256')
  ...
  .digest('hex');
execFileSync(process.execPath, ['scripts/v14409/apply.mjs'], { stdio: 'pipe' });
const firstHash = hashFiles();
execFileSync(process.execPath, ['scripts/v14409/apply.mjs'], { stdio: 'pipe' });
const secondHash = hashFiles();
assert.equal(secondHash, firstHash, 'v14.4.09 precisa ser idempotente em server.mjs e WhatsApp');
...
assert.equal((materializedServer.match(/cc-v14409:concierge-gyms-plan-location/g) || []).length, 1, 'marcador Wellhub não pode duplicar');
...
console.log('CrewCheck v14.4.09 Wellhub plan + geo fail-closed + WhatsApp location parity: PASS');
