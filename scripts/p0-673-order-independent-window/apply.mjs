...
if (!source.includes(marker)) {
  ...
  const saveWithSettledRefresh = `saveRosterAnalysis({ roster, compliance: newCompliance, gym: newGym, sourceFileName: file.name } as any).finally(() => {
          // The first history event is emitted synchronously by databaseClient while
          // React may still hold the previous bundle. Rebuild once more on the next
          // task, after the newly imported competence has become the active UI bundle.
          window.setTimeout(() => {
