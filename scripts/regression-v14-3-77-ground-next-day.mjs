...
const clientParser = fs.readFileSync('client/src/lib/aimsParser.ts', 'utf8');
...
assert.match(
  clientParser,
  /isNextDay:\s*Boolean\(startTime && endTime && minutesOfDay\(endTime\) <= minutesOfDay\(startTime\)\)/,
  'cliente canônico precisa manter semântica equivalente de virada de dia em atividade de solo',
);
...
try {
  ...
  // Force the v14.3.75 MCK visual fallback by removing only MCK from the normal
  // token-activity allowlist in an isolated test module. The fallback must keep
  // the same 22:00 -> 02:00 next-day semantics.
  ...
  console.log('[v14.3.77] OK — MCK/RCFI noturnos e fallback MCK preservam isNextDay e 4h sem regredir a semântica do cliente.');
} finally {
