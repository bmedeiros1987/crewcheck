...
assert.doesNotMatch(refreshSlice, /setRosterWindow\(primary\);/, 'P0 #673: refresh não pode encolher a Escala para o roster primário antes da materialização histórica');
...
assert.match(home, /saveRosterAnalysis\([\s\S]{0,500}\.finally\(\(\) => \{[\s\S]{0,300}crewcheck:roster-history-updated/, 'P0 #673: import não agenda refresh pós-persistência');
