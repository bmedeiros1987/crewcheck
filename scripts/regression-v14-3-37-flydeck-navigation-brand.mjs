...
assert.ok(chain.includes("await import('../v14337/apply.mjs');"), 'v14.3.37 deve encerrar a preparação canônica');
...
const groupTitles = ['Hoje', 'Preparação', 'Em operação', 'Escala e planejamento', 'Financeiro', 'Rotina e apoio', 'Documentos e conta', 'Administração'];
...
const expectedRoutes = [
  ...
  'perdiem','salary','crew','concierge','hotels','gyms','routine','community',
  'reports','calendar','exports','plans','settings','updates','maintenance','admin',
];
...
assert.ok(groupItemCounts.length >= 7, 'grupos públicos do menu não foram reconhecidos');
...
console.log('v14.3.37 FlyDeck/navigation/brand: chronology, grouped menu, five-item bottom nav, protected engine and canonical mark validated.');
