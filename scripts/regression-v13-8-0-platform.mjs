...
assert.match(platform, /crewcheck_platform_rosters/);
assert.match(platform, /crewcheck_platform_visitors/);
assert.match(platform, /crewcheck_platform_auth_attempts/);
...
assert.match(platform, /saveRosterMysql/);
assert.match(platform, /owner_email=\$1 AND roster_key=\$2/);
assert.match(platform, /ASAAS_CANCELLATION_REQUIRED/);
assert.match(platform, /Nenhum dado foi excluído/);
assert.match(platform, /providerStatus: response\.status/);
assert.doesNotMatch(platform, /externalCancellation\s*=\s*await asaasRequest\([\s\S]{0,300}\.catch\(\(\)\s*=>\s*false\)/);
...
assert.doesNotMatch(platform, /await import\(['"]pg['"]\)|POSTGRES_URL|SUPABASE_DB_URL|ON\\s+CONFLICT|::jsonb|jsonb_to_recordset/);
