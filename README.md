# CrewCheck v11.0.88 — Telegram aparece no Gerenciador + backup Storage
...
- Upload de escala pelo Telegram agora confirma o ID curto da escala no Gerenciador.
- Gerenciador de Escalas força atualização ao abrir a tela.
- API do Gerenciador inclui espelho de escala recebida pelo Telegram caso o registro principal não apareça por cache/sincronização.
- Snapshot do Telegram passa a guardar o ID real do registro, status de backup e metadados de storage.
- Banco atual continua sendo a fonte principal; Supabase segue apenas como backup/redundância do PDF original.
...
- Workflow do GitHub publica o AAB, APK e `crewcheck-mapping-v11.0.88.txt` como assets da release.
- Use o `app-release.aab` na Play Console e envie o arquivo `crewcheck-mapping-v11.0.88.txt` como arquivo de desofuscação da mesma versão.
